-- Strategy3 cumulative bid/ask volume patch, 2026-06-16.
-- Unit contract:
...
alter table public.fugle_quotes_latest
...
comment on column public.fugle_quotes_latest.cumulative_bid_volume is
  'Unit: lots. Inner/bid-side accumulated volume copied from fugle_quotes_live.';

comment on column public.fugle_quotes_latest.cumulative_ask_volume is
  'Unit: lots. Outer/ask-side accumulated volume copied from fugle_quotes_live.';

comment on column public.fugle_quotes_latest.cumulative_bid_ask_volume is
  'Unit: lots. cumulative_bid_volume + cumulative_ask_volume.';

update public.fugle_quotes_latest latest
set
  cumulative_bid_volume = live.cumulative_bid_volume,
  cumulative_ask_volume = live.cumulative_ask_volume,
  cumulative_bid_ask_volume = live.cumulative_bid_ask_volume
from public.fugle_quotes_live live
where live.symbol = latest.symbol
  and (
    coalesce(latest.cumulative_bid_volume, 0) <> coalesce(live.cumulative_bid_volume, 0)
    or coalesce(latest.cumulative_ask_volume, 0) <> coalesce(live.cumulative_ask_volume, 0)
    or coalesce(latest.cumulative_bid_ask_volume, 0) <> coalesce(live.cumulative_bid_ask_volume, 0)
  );
...
grant select on public.fugle_quotes_latest to anon;
grant select on public.fugle_quotes_latest to service_role;
