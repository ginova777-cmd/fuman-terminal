...
$preCandidatesPath = Join-Path $outDir "opening-limit-order-0840-pre-candidates-$compact.json"
if (!$RunId -and (Test-Path -LiteralPath $preCandidatesPath)) {
  try { $preCandidates = Get-Content -LiteralPath $preCandidatesPath -Raw | ConvertFrom-Json; if ($preCandidates.run_id) { $RunId = $preCandidates.run_id } } catch {}
}
...
$verifierArgs = @("--trade-date=$tradeDate")
if ($RunId) { $verifierArgs += "--run-id=$RunId" }
$output = (& $node --use-system-ca $verifier @verifierArgs 2>&1 | Out-String).Trim()
...
$payload | Add-Member -NotePropertyName run_id -NotePropertyValue $(if ($payload.run_id) { $payload.run_id } else { $RunId }) -Force
