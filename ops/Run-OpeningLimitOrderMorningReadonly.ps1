...
if (!$RunId) {
  ...
  $RunId = "opening-limit-order-{0}-{1}" -f $compactDate, ((Get-Date).ToUniversalTime().ToString("yyyyMMddHHmmss"))
}
...
Write-Host ("[morning] 開盤入 read-only 08:40 progressive 總入口 trade_date={0} run_id={1}" -f $TradeDate, $RunId)
Write-Host "[morning] 08:40 靜態條件與日報族群先排序；08:45-08:50 接股期/試撮；08:55 產出加權排名。"

& "C:\Program Files\PowerShell\7\pwsh.exe" -NoProfile -ExecutionPolicy Bypass -File $progressive -TradeDate $TradeDate -Limit $Limit -RunId $RunId -WaitUntil0840 -TerminalDir $TerminalDir -RuntimeDir $RuntimeDir
