...
const APP_AUTH_UNLOCK_KEY = "fuman_app_auth_unlock_until_v1";
const EXPORT_UNLOCK_KEY = "fuman_export_unlock_until_v1";
const APP_AUTH_UNLOCK_MS = 30 * 24 * 60 * 60 * 1000;
const EXPORT_UNLOCK_MS = APP_AUTH_UNLOCK_MS;
...
function isAppAuthenticated() {
  const appUntil = Number(localStorage.getItem(APP_AUTH_UNLOCK_KEY) || 0);
  const exportUntil = Number(localStorage.getItem(EXPORT_UNLOCK_KEY) || 0);
  return Math.max(appUntil, exportUntil) > Date.now();
}

function markAppAuthenticated() {
  const until = String(Date.now() + APP_AUTH_UNLOCK_MS);
  localStorage.setItem(APP_AUTH_UNLOCK_KEY, until);
  localStorage.setItem(EXPORT_UNLOCK_KEY, until);
  updateAuthChrome();
}

async function unlockAppAccess() {
  if (isAppAuthenticated()) return true;
  const input = window.prompt("請輸入登入密碼以查看完整功能：");
  if (!input?.trim()) return false;
  try {
    const result = await verifyExportPassword(input.trim());
    if (!result.ok) {
      if (result.payload?.code === "PASSWORD_NOT_SET") {
        showExportNotice("尚未在 Vercel 設定登入密碼。");
      } else {
        showExportNotice("密碼錯誤，請重新登入。");
      }
      return false;
    }
  } catch (error) {
    showExportNotice("登入驗證失敗，請稍後再試。");
    return false;
  }
  markAppAuthenticated();
  showExportNotice("登入成功，已解鎖完整功能。");
  return true;
}
...
function updateAuthChrome() {
  ...
  viewLinks.forEach((link) => {
    const locked = isProtectedView(link.dataset.view) && !authed;
    link.classList.toggle("locked", locked);
    if (locked) link.setAttribute("aria-label", `${link.textContent.trim()}，登入後可查看`);
    else link.removeAttribute("aria-label");
  });
  const logout = document.querySelector(".logout");
  if (logout) logout.textContent = authed ? "↪ 登出" : "↪ 登入";
}

function ensureAuthOverlay(panel, viewName, activeLink) {
  if (!panel) return;
  const host = viewName === "strategy" ? panel.querySelector(".strategy-results") || panel : panel;
  host.classList.add("auth-lock-host");
  let overlay = host.querySelector(":scope > .auth-lock-overlay");
  if (!overlay) {
    overlay = document.createElement("div");
    overlay.className = "auth-lock-overlay";
    host.appendChild(overlay);
  }
  const title = activeLink?.textContent?.replace(/\s+/g, " ").trim() || "完整功能";
  overlay.innerHTML = `
    <div class="auth-lock-card" role="dialog" aria-modal="true" aria-label="登入解鎖">
      <span class="auth-lock-kicker">PREMIUM ACCESS</span>
      <h2>登入後查看 ${escapeAttr(title)}</h2>
      <p>市場總覽保持公開；策略中心、自選股、盤後籌碼與權證資金走向需登入後解鎖完整資料與操作。</p>
      <div class="auth-lock-stats">
        <article><span>公開頁</span><strong>市場總覽</strong></article>
        <article><span>目前狀態</span><strong>未登入</strong></article>
        <article><span>保留時間</span><strong>30 天</strong></article>
      </div>
      <div class="auth-lock-actions">
        <button type="button" data-auth-login>登入解鎖</button>
        <button type="button" data-auth-contact>聯絡開通權限</button>
      </div>
    </div>
  `;
  overlay.querySelector("[data-auth-login]")?.addEventListener("click", async () => {
    if (await unlockAppAccess()) showView(viewName, activeLink);
  });
  overlay.querySelector("[data-auth-contact]")?.addEventListener("click", () => {
    window.alert("請直接與作者聯繫");
  });
}

function applyProtectedViewState(viewName, activeLink) {
  updateAuthChrome();
  Object.entries(viewPanels).forEach(([name, panel]) => {
    const locked = name === viewName && isProtectedView(name) && !isAppAuthenticated();
    panel?.classList.toggle("auth-locked", locked);
    panel?.classList.toggle("auth-locked-strategy", locked && name === "strategy");
    if (locked) ensureAuthOverlay(panel, name, activeLink);
    else {
      panel?.querySelectorAll(".auth-lock-overlay").forEach((overlay) => overlay.remove());
      panel?.querySelectorAll(".auth-lock-host").forEach((host) => host.classList.remove("auth-lock-host"));
    }
  });
  return isProtectedView(viewName) && !isAppAuthenticated();
}
...
function openExportSettings() {
  ...
  if (action === "2") {
    ...
    localStorage.removeItem(APP_AUTH_UNLOCK_KEY);
    updateAuthChrome();
...
function showView(viewName, activeLink) {
  ...
  const locked = applyProtectedViewState(viewName, activeLink);
  if (locked) return;
...
viewLinks.forEach((link)=>{
  link.addEventListener("click",(e)=>{
    e.preventDefault();
    if (!isProtectedView(link.dataset.view) || isAppAuthenticated()) applyStrategyPresetFromLink(link);
    showView(link.dataset.view, link);
  });
});
document.querySelector(".logout")?.addEventListener("click", async () => {
  if (!isAppAuthenticated()) {
    const activeLink = viewLinks.find((link) => link.classList.contains("active")) || viewLinks.find((link) => link.dataset.view === "market");
    if (await unlockAppAccess()) showView(activeLink?.dataset.view || "market", activeLink);
    return;
  }
  localStorage.removeItem(APP_AUTH_UNLOCK_KEY);
  localStorage.removeItem(EXPORT_UNLOCK_KEY);
  showExportNotice("已登出，市場總覽仍可瀏覽。");
