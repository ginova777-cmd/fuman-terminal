...
function getActiveExportContext() {
  if (isViewActive("chip-trade")) {
    return {
      title: "外資+投信連買",
      table: document.querySelector("#chip-trade-view .chip-table"),
    };
  }
  if (isViewActive("warrant-flow")) {
    return {
      title: "權證資金走向",
      table: document.querySelector("#warrant-flow-view table"),
    };
  }
  if (selectedStrategyIds.has("overnight_chip")) {
    return { title: "策略3-隔日沖", table: strategyTable?.querySelector("table") };
  }
  return {
    title: strategyTitle?.textContent || "fuman",
    table: strategyTable?.querySelector("table"),
  };
}

function getActiveExportRows(limit = Infinity) {
  const context = getActiveExportContext();
  ...
  return { headers, rows, title: context.title };
}
...
function downloadCsv(headers, rows, title = "fuman") {
  const lines = [headers, ...rows].map((row) => row.map(csvCell).join(","));
  const blob = new Blob([`\uFEFF${lines.join("\r\n")}`], { type: "text/csv;charset=utf-8" });
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  const safeTitle = String(title || "fuman").replace(/[^\w\u4e00-\u9fa5+.-]+/g, "-");
  link.href = url;
  link.download = `${safeTitle}-${new Date().toISOString().slice(0, 10)}.csv`;
...
async function handleProtectedExport() {
  ...
  const { headers, rows, title } = getActiveExportRows();
  if (!rows.length) {
...
function renderWarrantFlow() {
  ...
  panel.innerHTML = `
    <section class="swing-dashboard">
      <div class="swing-topbar">
        <div>
          <h2>${titleWithIcon("◒", "權證資金走向")} <span class="swing-live">● 08:00 / 15:00 完整掃</span></h2>
          <p>${helperText}</p>
        </div>
        <div class="swing-controls">
          <label>更新模式：<select><option>08:00 / 15:00 完整掃</option></select></label>
