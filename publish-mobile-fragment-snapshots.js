...
const DEFAULT_TABS = ["strategy2", "strategy3", "strategy4", "strategy5", "chip", "cb", "warrant"];

function requestedTabs() {
  const raw = process.argv.find((arg) => arg.startsWith("--tabs="))?.slice("--tabs=".length);
  if (!raw) return DEFAULT_TABS;
  const tabs = [...new Set(raw.split(",").map((value) => value.trim().toLowerCase()).filter(Boolean))];
  const unknown = tabs.filter((tab) => !DEFAULT_TABS.includes(tab));
  if (unknown.length) throw new Error(`unknown mobile fragment tabs: ${unknown.join(",")}`);
  if (!tabs.length) throw new Error("--tabs must include at least one tab");
  return tabs;
}
...
async function main() {
  ...
  console.log(JSON.stringify({ ok: failures.length === 0, baseUrl: BASE_URL, tabs, count: results.length, results, failures }, null, 2));
