...
try {
  ...
  $strategy2VerifierDue = (($startedAt.DayOfWeek -ne [DayOfWeek]::Saturday) -and ($startedAt.DayOfWeek -ne [DayOfWeek]::Sunday) -and ($startedAt.TimeOfDay -ge [TimeSpan]::Parse("08:45")))
  ...
  [ordered]@{
    ...
    ok = (($verifierExit -eq 0) -and (-not $strategy2VerifierDue -or $strategy2VerifierExit -eq 0) -and (-not $cleanupVerifierDue -or $cleanupVerifierExit -eq 0))
  ...
  $checkpointOk = (($verifierExit -eq 0) -and (-not $strategy2VerifierDue -or $strategy2VerifierExit -eq 0) -and (-not $cleanupVerifierDue -or $cleanupVerifierExit -eq 0))
  if (-not $checkpointOk) {
