...
function getFumanWorker() {
  ...
  try {
    fumanWorker = new Worker("terminal-worker.js?v=mobile-title-card-fixed-20260602");
...
function loadFumanStyle(href, id) {
  ...
  link.id = id;
  link.rel = "stylesheet";
  link.href = href.includes("?") ? href : `${href}?v=${window.FUMAN_TERMINAL_BOOT?.version || "mobile-title-card-fixed-20260602"}`;
...
function loadFumanFeatureModule(name, src, globalName) {
  ...
  const version = window.FUMAN_TERMINAL_BOOT?.version || "mobile-title-card-fixed-20260602";
