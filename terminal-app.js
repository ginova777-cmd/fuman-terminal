...
function getFumanWorker() {
  ...
  try {
    fumanWorker = new Worker("terminal-worker.js?v=mobile-strategy-title-fix-20260601-05");
...
function loadFumanStyle(href, id) {
  ...
  link.id = id;
  link.rel = "stylesheet";
  link.href = href.includes("?") ? href : `${href}?v=${window.FUMAN_TERMINAL_BOOT?.version || "mobile-strategy-title-fix-20260601-05"}`;
...
function loadFumanFeatureModule(name, src, globalName) {
  ...
  const version = window.FUMAN_TERMINAL_BOOT?.version || "mobile-strategy-title-fix-20260601-05";
