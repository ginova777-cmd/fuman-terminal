(function (root, factory) {
  ...
  function render(data) {
    ...
    const message = !data ? "晨報尚未產生，等待 08:20 美股／夜盤及 08:50 日韓股排程。" : !valid ? `晨報資料未完整：${data.reason_code || "產業或排名資料缺漏"}`
      : data.previousTradingDay ? `休市期間，顯示最近交易日 ${data.date} 的晨報。` : rows.length ? "正漲幅優先觀察" : "本階段產業已偵測，沒有符合正漲幅條件的觀察標的（0 筆）。";
    ...
    return `<section class="opening-report-0830-briefing" data-opening-report-0830-briefing="1" data-opening-report-state="${state}" data-run-id="${esc(data?.run_id)}" data-trade-date="${esc(data?.date)}" data-content-hash="${esc(data?.delivery_content_hash)}" style="border:1px solid #64748b;border-radius:12px;padding:16px;margin:12px 0;overflow-wrap:anywhere;line-height:1.7">
      <div data-morning-night data-night-contract="${esc(data?.night_futures?.contract_month)}" data-night-close="${esc(data?.night_futures?.close)}" style="margin-bottom:12px;overflow-wrap:anywhere">${esc(data?.night_futures_summary || "台指期夜盤：資料未確認")}</div>
      <header><h3 style="margin:0">${esc(data?.date || "")} ${esc(data?.report_time || "08:20")} ${data?.stage === "asia_0850" ? "日韓股" : "美股／夜盤"} 晨報</h3><p data-morning-status>${esc(message)}</p><p>資料截點 <span data-morning-cutoff>${esc(data?.source_cutoff || "08:20:59 Asia/Taipei")}</span>｜${data?.recovery ? "當日授權補跑" : "正式排程 "+esc(data?.report_time || "08:20")}｜僅供觀察排序，不構成正式進場訊號</p></header>
