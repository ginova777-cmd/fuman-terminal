...
const checks = [
  ...
  ["api_canonicalizes_formal_authority", api.includes("publishAllowed: blockedEvidence || replay ? false : true") && api.includes("formalDisplayAllowed: blockedEvidence || replay ? false : true")],
  ["api_canonicalizes_formal_quality", api.includes('qualityStatus: blockedEvidence ? "blocked" : replay ? "diagnostic_replay" : "complete"') && api.includes('evidenceStatus: blockedEvidence ? "blocked" : replay ? "diagnostic_replay" : "complete"') && api.includes("run_quality_at_publish:")],
  ...
  ["historical_scorecard_ignores_current_day_blockers", source.includes("isHistoricalSelection") && source.includes("historicalSelection ? payload") && source.includes("allSourceReports.filter")],
  ["generator_passes_explicit_scorecard_date", generator.includes("date: scorecardFallbackDate()")],
  ["generator_has_defined_row_dedupe_key", /const (?:row)?key =/.test(fs.readFileSync(path.join(ROOT, "scripts", "generate-terminal-scorecard-source.js"), "utf8"))],
  ...
  ["generator_preserves_symbol_and_name", /function codeOf\(row, fallback\)\s*\{\s*return cleanText\(/.test(generator) && /function nameOf\(row, code\)\s*\{\s*return cleanText\(/.test(generator)],
