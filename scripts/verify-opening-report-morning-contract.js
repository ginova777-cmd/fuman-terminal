...
function symbolMapChecks(checks) {
  ...
  addCheck(checks, "industry_mapping_evidence_complete", rows.every((row) => row.mapping_contract === "opening-report-0830-industry-map-v2" && /^\d{4}-\d{2}-\d{2}$/.test(String(row.mapping_reviewed_at || "")) && Array.isArray(row.mapping_evidence_authorities) && row.mapping_evidence_authorities.length >= 2 && [...(row.a || []), ...(row.b || [])].every((stock) => stock.mapping_status === "reviewed" && stock.mapping_grade === stock.tier && stock.mapping_industry === row.industry && stock.relationship_type && Array.isArray(stock.evidence_authorities) && stock.evidence_authorities.length >= 2)), "every A/B mapping requires grade, relationship, authority and review date");
  addCheck(checks, "industry_mapping_c_observation_only", rows.every((row) => (row.c || []).every((stock) => stock.tier === "C" && stock.mapping_grade === "C" && stock.mapping_status === "observation_only" && stock.relationship_type === "theme_only_or_unverified" && !(row.a || []).some((item) => item.symbol === stock.symbol) && !(row.b || []).some((item) => item.symbol === stock.symbol))), "C mappings are retained for review but excluded from A/B priority output");
