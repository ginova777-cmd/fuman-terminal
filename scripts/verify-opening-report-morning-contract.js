...
function currentReceiptChecks(checks, tradeDate) {
  ...
  addCheck(checks, "current_report_status_is_report_only", ["REPORT_OK", "REPORT_DEGRADED", "COMPLETE", "complete"].includes(finalReceipt.report_status), finalReceipt.report_status || finalReceipt.status);
  addCheck(checks, "current_final_contract_complete", finalReceipt.contract === "opening_report_0830_complete_v1" && finalReceipt.complete === true && finalReceipt.status === "complete" && finalReceipt.first_blocker == null, JSON.stringify({ contract: finalReceipt.contract, complete: finalReceipt.complete, status: finalReceipt.status, first_blocker: finalReceipt.first_blocker }));
