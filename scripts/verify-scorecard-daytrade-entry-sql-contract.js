...
const checks = [
  ...
  ["signal_type_field", /signal_type\s+text\s+not null\s+default 'formal'/i],
  ["signal_type_constraint", /signal_type[\s\S]*formal[\s\S]*detected/i],
  ...
  ["anon_authenticated_insert_policy", /for insert[\s\S]*to anon,\s*authenticated[\s\S]*with check \(true\)/i],
  ["service_manage_policy", /for all[\s\S]*to service_role[\s\S]*with check \(true\)/i],
  ["grant_select_insert", /grant select,\s*insert on public\.fugle_daytrade_entry_history to anon,\s*authenticated/i],
  ["grant_service_all", /grant all on public\.fugle_daytrade_entry_history to service_role/i],
  ["grant_sequence_usage", /grant usage,\s*select on sequence public\.fugle_daytrade_entry_history_id_seq to anon,\s*authenticated,\s*service_role/i],
];
