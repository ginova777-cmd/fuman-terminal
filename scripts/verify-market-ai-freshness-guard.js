...
assert(productionMonitorSource.includes("notifyIfNeeded"), "production monitor must call notification path for critical live contract issues");
assert(alertPathSource.includes("AI 判讀存在 sourceIssues") && alertPathSource.includes("sourceIssueEmail"), "alert path verifier must prove AI sourceIssues reach Gmail/email dry-run");
