...
function verifyQueue(state, queue, issues) {
  ...
  for (const job of Array.isArray(queue) ? queue : []) {
    ...
    if (stateName.includes("SOURCE")) {
      assert(/daytrade-warmup:self-heal/.test(String(job.command || "")) && /verify:terminal-water-root/.test(String(job.command || "")), "source_job_must_rewater_then_check_water_root", { key, command: job.command }, issues);
    }
...
function main() {
  ...
  const payload = {
    ...
    sourcePolicy: "source jobs must queue idempotent rewater/self-heal and Water Root recheck; executable gating is verified in auto-roll-forward plan",
