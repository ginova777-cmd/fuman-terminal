...
async function main() {
  ...
  const sourceStartupVerifier = readText("scripts/verify-daytrade-fugle-source-startup.js");
  ...
  const mobileFragmentApi = readText("api/mobile-fragment.js");
  const scorecard88Collector = readText("scripts/collect-terminal-scorecard-88.js");
  const rowAuditVerifier = readText("scripts/verify-terminal-row-audit-consistency.js");
  ...
  const masterCheckpointMarkers = [
    ...
    "$checkpointVerifierFailure", "checkpoint_specific_verifier_failed:",
    ...
    "$exactCheckpointId", "$boundedCheckpointIds", "$delayMinutes -ge 0 -and $delayMinutes -le 5", "if (-not $RequestedCheckpoint -and $effectiveMode -eq \"Checkpoint\")",
    "$dailyCheckpointCoverageFailure", "daily_checkpoint_receipts_missing", "dailyCheckpointCoverage", "dailyCheckpointResults", "priorCheckpointFailures", "prior_checkpoint_failed:",
    ...
    "Fuman Daytrade Source Gate 0700", "Fuman Opening Report 0820 Preflight", "Fuman Opening Report 0830 Telegram", "Fuman Opening Limit Order Morning Readonly 0840", "Fuman Daytrade Futopt Preopen Evidence 0845", "Fuman Daytrade Futopt Preopen Evidence 0850", "Fuman Strategy4 Source Prewarm 1535", "Fuman Scorecard88 Collect Strategy2 1240",
    'owner="EXTERNAL_OWNER"', 'disposition="READ_ONLY_BRIDGE"', 'disposition="READ_ONLY_BRIDGE_CLOSURE"', 'same_day_bridge_aggregate_19_of_19_run_id_trade_date_and_allowed_action_no_recalculation_or_retry', 'today_frozen_preflight_and_pre_delivery_contract_gate_complete_before_delivery', '--pre-delivery', '$firstFailedCheckpointVerifier', 'checkpoint_specific_verifier_failed:', 'reportedFirstBlocker', 'reportedFailedChecks', 'checkpointVerifierOutput',
    ...
    'verify-daytrade-mother-pool-contract.js', 'verify-opening-limit-order-closed-loop.js', 'verify-terminal-row-audit-consistency.js', '--strategy=$rowAuditStrategy', 'Fuman Scorecard88 Collect Strategy3 1315', 'Fuman Scorecard88 Collect Strategy4 1700', 'Fuman Scorecard88 Collect Evening 2140',
    'verify-strategy2-v3-unified-contract.js', 'Fuman Strategy2 Unified 0845-1230',
    '$strategy2LiveReceipt', 'scope="not_due_until_12:40"',
    '"12:30" = @{ name="strategy2_finalize"', '$strategy2VerifierDue = @("12:40", "23:10")',
    'Fuman API-Only Retired Artifact Cleanup 1535', 'Fuman Supabase Vercel History Cleanup 1545',
    'Fuman Global Cost Janitor Scorecard 1555', 'Fuman Daytrade Intraday Retention 1605',
    'Fuman Daily Retention Maintenance 1625', '$originalTaskWaitFailure = @($originalTaskWait',
    'verify-daytrade-postclose-1330.js', 'Fuman Daytrade Source Writer 0600-1330',
    'Fuman Fugle Daytrade WebSocket Collector 0600-1330', 'no_residual_lock_or_cross_day_rollback',
    'verify-opening-limit-order-0840-checkpoint-readonly.js', '--slots=0845',
    'verify-strategy2-v3-water.js', 'transport_pool_0845_futopt_trial_strategy2_formal_water_preflight_and_isolation',
  ];
  for (const marker of masterCheckpointMarkers) {
    requireMarker(issues, "run-terminal-master-control.ps1", masterRunner, marker);
  }
  for (const marker of ["fuman-daytrade-fugle-source-startup-v2", "WRITER_TASK", "queryActiveWriterOwners", "writer_active_owner_not_unique", "writer_production_root_mismatch", "startsTask: false"]) {
    requireMarker(issues, "scripts/verify-daytrade-fugle-source-startup.js", sourceStartupVerifier, marker);
  }
  for (const marker of ["data-row-symbol", "data-row-price", "data-row-score", "data-row-rank"]) {
    requireMarker(issues, "api/mobile-fragment.js", mobileFragmentApi, marker);
  }
  for (const marker of ["rowAuditSignatures", "rowAuditSignatureComplete", "canonical_${key}_api_readonly", "row_audit_run_id_mismatch", "row_audit_count_mismatch", "row_audit_fields_incomplete"]) {
    requireMarker(issues, "scripts/collect-terminal-scorecard-88.js", scorecard88Collector, marker);
  }
  for (const marker of ["terminal-four-surface-row-audit-v1", "desktop_rows_match_canonical", "mobile_rows_match_canonical", "scorecard88_rows_match_canonical", "source_reports_rows_match_canonical"]) {
    requireMarker(issues, "scripts/verify-terminal-row-audit-consistency.js", rowAuditVerifier, marker);
  }
  const checkpointIds = ["06:00","06:05","07:00","07:08","08:20","08:29","08:30","08:35","08:36","08:40","08:45","08:50","08:55","09:00","12:30","12:40","12:50","12:55","13:00","13:15","13:30","15:35","16:00","17:00","17:10","17:40","18:10","18:40","19:10","20:05","21:00","21:10","21:15","21:40","22:00","23:10"];
  ...
  if (/\$earliestUnprocessedDue|\$processedCheckpointIds/.test(masterRunner)) addIssue(issues, "master_checkpoint_must_not_retarget_to_earlier_missing_receipt");
  if (countMarker(masterRunner, "$exactCheckpointId") < 2) addIssue(issues, "master_checkpoint_exact_trigger_routing_missing");
  if (countMarker(masterRunner, "$boundedCheckpointIds") < 2) addIssue(issues, "master_checkpoint_bounded_delay_routing_missing");
  ...
  const checkpointContract = ["06:00","06:05","07:00","07:08","08:20","08:29","08:30","08:35","08:36","08:40","08:45","08:50","08:55","09:00","12:30","12:40","12:50","12:55","13:00","13:15","13:30","15:35","16:00","17:00","17:10","17:40","18:10","18:40","19:10","20:05","21:00","21:10","21:15","21:40","22:00","23:10"];
  ...
  const payload = {
    ...
    guarantees: [
      ...
      "08:20-08:30 verify external-owner frozen evidence read-only; 08:35 validates bridge apply and 08:36 validates aggregate closure read-only",
