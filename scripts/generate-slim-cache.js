...
function slimWarrant(payload) {
  ...
  return {
    ...
    matches: matches.map((item) => ({
      ...
      finalScore: cleanNumber(item.finalScore || item.score),
      warrantHeatScore: cleanNumber(item.warrantHeatScore || item.score),
      stockSetupScore: cleanNumber(item.stockSetupScore),
      stockSetupLabel: String(item.stockSetupLabel || ""),
      stockRisk: String(item.stockRisk || ""),
      branchPowerScore: cleanNumber(item.branchPowerScore),
      branchStatus: String(item.branchStatus || ""),
      branchLabel: String(item.branchLabel || ""),
      branchAvailable: item.branchAvailable === true,
      branchTopBuyer: String(item.branchTopBuyer || ""),
      branchTopSeller: String(item.branchTopSeller || ""),
      branchConcentration: cleanNumber(item.branchConcentration),
      branchStreakDays: cleanNumber(item.branchStreakDays),
      signalGrade: String(item.signalGrade || item.level || ""),
