...
function classifyLeaderMarket(symbol) {
  ...
  if (/\.T$/.test(value)) return "japan";
  if (/\.(?:KS|KQ)$/.test(value)) return "korea";
  if (/\.[A-Z]{1,4}$/.test(value)) return "other";
  return "us";
}
...
module.exports = { classifyLeaderMarket, classifyPercent, koreanCode, naverLocalTradedAtMs, parseNaverKoreaBasic, yahooJapanLocalTimeMs, parseYahooJapanQuotePage, yahooJapanSnapshot, yahooChartSnapshot, japanYahooSnapshot, industrySummary };
