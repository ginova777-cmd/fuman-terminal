...
const SQL_FILE = path.join(ROOT, "ops", "public-slot", "DaytradeStarSlotSymbolReadback_20260908.sql");
...
async function main() {
  const guard=spawnSync(process.execPath,[path.join(ROOT,"scripts","supabase-incident-guard.js"),"check","--class=writer","--action=apply-daytrade-star-slot-symbol-readback"],{cwd:ROOT,stdio:"inherit",windowsHide:true});
  ...
  const output={ok:true,status:"complete",contract:"star_slot_symbol_readback_apply_v1",applied_at:new Date().toISOString(),sql_file:SQL_FILE,method,views:["v_fugle_daytrade_star_slot_evidence_source","v_fugle_daytrade_star_slot_verification_readback","v_fugle_daytrade_star_slot_symbol_readback"],natural_evidence_mutated:false};
