...
const snapshot={run_id:'fixture',generation:'fixture',snapshot_sequence:1,trade_date:'2026-09-17',canonical_run_id:'fixture'};
let calls=[],ack={ok:true,run_id:'fixture',generation:'fixture',snapshot_sequence:1},failure=false,readFailure=false,readBlocked=false,receipts=[];
const ctx=vm.createContext({DRY_RUN:false,nowIso:()=> 'fixture',buildMotherPoolSnapshot:()=>snapshot,
 ...
 fs:{existsSync:()=>false},supabaseGetPaged:async(view,query)=>{assert.ok(query.includes("&generation=eq.fixture"));assert.ok(query.includes("&canonical_run_id=eq.fixture"));if(readFailure)throw Error('view HTTP 503 secret-test-marker');return [];},require:id=>id.endsWith('/daytrade-mother-pool-snapshot')?{inspectSnapshot:()=>({ok:true})}:id.endsWith('/mother-pool-snapshot-readback')?{verifySnapshotReadback:()=>({complete:!readBlocked,status:'complete',exit_code:0,read_role:'anon',trade_date:snapshot.trade_date,canonical_run_id:snapshot.canonical_run_id,first_blocker:null,mother_pool_run_id:'fixture',generation:'fixture',snapshot_sequence:1,failed_checks:readBlocked?['member_count_mismatch']:[]})}:require(id),
...
async function main(){
 ...
 for(const invalid of [{},{ok:true,run_id:'other',snapshot_sequence:1},{ok:true,run_id:'fixture',generation:'fixture',snapshot_sequence:2}]){
  ack=invalid;calls=[];
  await assert.rejects(()=>ctx.publishMotherPoolSnapshot([],[],'2026-09-17','fixture'),/DB_ACK_MISMATCH/);
  assert.deepEqual(calls,['DB']);
 }
 ack={ok:true,run_id:'fixture',generation:'fixture',snapshot_sequence:1};readFailure=true;calls=[];receipts=[];
