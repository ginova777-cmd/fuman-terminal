...
const target = path.join(receipts, recoveryReplay ? "strategy3-recovery-replay.json" : "strategy3.json");
...
const complete = !recordFailure && (recoveryReplay ? recoveryComplete : (baseComplete && triComplete));
...
const blockingReason = complete ? "" : recoveryReplay ? "strategy3_recovery_replay_closure_not_complete" : awaitingScorecard ? "scorecard_collection_pending_1315" : !baseComplete ? "strategy3_base_closure_not_complete" : "strategy3_tri_surface_scorecard_not_complete";
const payload = { contract: "strategy-runner-verifier-receipt-v1", strategy: "strategy3", tradeDate: date,
  ...
  blockingReason, fallback: false, recoveryReplay, naturalSlotComplete: !recoveryReplay, warnings: [], triSurfaceStatus: (recoveryReplay ? recoverySurfaceComplete : triComplete) ? "complete" : "pending",
  ...
  runnerStatus: scan?.runner_status || scan?.status || null, verifierOk: water?.verifier_ok === true, receiptWritten: true,
