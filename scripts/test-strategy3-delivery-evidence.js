...
function fixture() {
  ...
  return {
    date: "2026-09-11",
    ui: { ok: true, results: ["desktop", "mobile", "scorecard"].map(kind => ({ kind, routeKey: "strategy3", ok: true, contentAcceptance: { actualRun: id, actualSymbols: ["1560", "2305"], sameSymbols: true } })) },
...
for (const [name, mutate] of [
  ...
  ["missing rendered UI", x => { x.ui = null; }],
  ["locked rendered UI", x => { x.ui.results[0].accessState = "membership_locked"; }],
  ["old rendered UI", x => { x.ui.results[1].contentAcceptance.actualRun = "old"; }],
  ["missing rendered 88", x => { x.ui.results.pop(); }],
...
test("healthy zero results requires explicit zero everywhere", () => {
  ...
  x.ui.results.forEach(row => { row.contentAcceptance.actualSymbols = []; });
