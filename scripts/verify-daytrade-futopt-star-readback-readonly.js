...
async function main(){
  ...
  const cases=rows.map((row)=>{
    const futureUsable=Boolean(row.future_symbol)&&!String(row.future_symbol).startsWith("TXF")&&positive(row.future_0845_open_price)&&positive(row.future_0859_last_price)&&row.empty_shell_row!==true;
    ...
    if(row.star_final_ok===true&&(!futureUsable||!nearOne||row.future_open_retest_ok!==true)) failures.push(`${row.symbol}:star_pass_with_missing_future_retest_evidence`);
    if(!futureUsable&&row.star_final_ok===true) failures.push(`${row.symbol}:missing_natural_future_window_must_fail_closed`);
    return {symbol:row.symbol,future_symbol:row.future_symbol,future_0845_open_price:row.future_0845_open_price,future_0859_last_price:row.future_0859_last_price,future_open_near_percent:row.future_open_near_percent,future_open_retest_ok:row.future_open_retest_ok,future_ok:row.future_ok,preopen_ok:row.preopen_ok,near_one_present:nearOne,preopen_snapshot_count:Number(row.preopen_snapshot_count||0),star_final_ok:row.star_final_ok,data_gap_reason:row.data_gap_reason,display_label:row.display_label};
  });
