...
function computeStats({ activeSymbols, priorityRows, quoteMap, fetchedRows, dailyVolumeMap, intradayMap, futoptRows, websocketFutoptSync = {}, fetchResult, state, supplementalMaps = {} }) {
  ...
  const payload = {
    ...
    formal_entry_allowed: !offSession && after0845 && gateGrade === "A",
    latest_update_allowed: !offSession && after0845 && gateGrade === "A",
...
async function syncWebSocketFutoptQuotes() {
  ...
  if (!rows.length) {
    return { written: 0, skipped: true, cacheCount: cache.quotes.size, stockRows, txfRows };
  }
