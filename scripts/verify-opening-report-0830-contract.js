...
function currentReceiptChecks(tradeDate) {
  ...
  const bridge = readJson(path.join(RUNTIME_DIR, "data", "scan-receipts", "opening-report-0830-priority-bias-bridge-latest.json"));
  ...
  return {
    ...
    bridge: bridge?.contract === "opening-report-0830-priority-bias-bridge-aggregate-v1" && compactDate(bridge?.date) === compact && bridge?.status === "BRIDGE_OK" && bridge?.received === true && Number(bridge?.industry_count || 0) > 0 && Number(bridge?.successful_industry_count || 0) === Number(bridge?.industry_count || 0) && bridge?.forbidden_publish_guard === true && bridge?.formal_candidate_count === 0 && bridge?.formal_candidate_allowed === false,
...
function main() {
  ...
  const checks = preDelivery ? [] : [
    ...
    stage("opening_entry_strength_fields", contains(runner, "overseas_sector_up_1d") && contains(runner, "overseas_sector_up_2d: null") && contains(runner, "us_sector_up_1d: null") && contains(runner, "us_sector_up_2d: null") && contains(runner, "missing_2d_reason")),
    ...
    stage("bridge_after_delivery_required", contains(runner, "const applyBridge = true") && contains(runner, "opening-report-0830-priority-bias-bridge-aggregate-v1") && contains(wrapper, "bridge_handoff_required = (-not $marketClosedSkip)") && contains(wrapper, "bridge_completed_inside_delivery_chain")),
  ...
  if (requireCurrent || preDelivery) {
    ...
    if (requireCurrent) checks.push(stage("current_bridge_aggregate", current.bridge));
