...
function collectMobileStats(route) {
  ...
  if (!membershipLocked && layout.tabsDisplay !== "flex") layoutBlockers.push(`mobile tabs must be flex actual=${layout.tabsDisplay || "<missing>"}`);
  ...
  if (route.fragment === "ai" && !membershipLocked) {
  ...
  if (route.verifyWatchAdd && !membershipLocked) {
  ...
  const keyOk = membershipLocked || route.fragment === "watch" || rootKey === route.fragment;
  const warnings = [];
  ...
  return {
    ...
    blockerMatches: blockers,
    warnings,
    accessState: membershipLocked ? "membership_locked" : "open",
    ok: membershipLocked
...
function collectDesktopMarketClosedStats(route) {
  const banner = document.querySelector("#fuman-market-closed-banner");
  const body = String(banner?.textContent || document.body?.textContent || "").replace(/\s+/g, " ").trim();
  const closed = Boolean(banner) || document.documentElement?.dataset?.marketClosedProtection === "true" || document.body?.dataset?.marketClosedProtection === "true";
  return {
    kind: "desktop", routeKey: route.key, label: route.label, accessState: "market_closed_previous_good",
    activePanelId: route.expectedPanelId || "", activeRouteKey: route.expectedRouteKey || "", activeNavView: "market",
    routeIdentityOk: closed, expectedPanelId: route.expectedPanelId || "", expectedRouteKey: route.expectedRouteKey || "",
    rowsVisible: 0, domRows: 0, canvasRows: 0, canvasCountText: "", canvasPixelDiversity: 0, canvasSize: { width: 0, height: 0 },
    sampleRows: [], filterCounts: [], freshnessText: body.slice(0, 180), emptyStateText: body.slice(0, 260), dateSignals: [], fieldSignals: [],
    blockerMatches: closed ? [] : ["market closed protection banner missing"],
    warnings: ["market closed previous-good UI protection verified"], ok: closed,
  };
}
async function collectDesktopStatsWhenReady(cdp, route, timeoutMs = 22000) {
  if (["market", "market-ai"].includes(route.key)) {
    const marketClosed = await evaluate(cdp, () => Boolean(document.querySelector("#fuman-market-closed-banner") || document.documentElement?.dataset?.marketClosedProtection === "true" || document.body?.dataset?.marketClosedProtection === "true")).catch(() => false);
    if (marketClosed) return evaluate(cdp, collectDesktopMarketClosedStats, route);
  }
  const start = Date.now();
  let last = null;
  while (Date.now() - start < timeoutMs) {
...
function collectDesktopMembershipLockStats(route) {
  const body = document.body;
  const authGate = document.querySelector("#auth-gate");
  const sidebar = document.querySelector("aside.sidebar");
  const memberState = document.querySelector("#member-state");
  const locked = body?.classList?.contains("auth-locked")
    || body?.dataset?.membershipAccess === "locked"
    || (authGate && !authGate.hidden && authGate.getAttribute("aria-hidden") !== "true")
    || Boolean(document.querySelector(".sidebar-foot .logout[data-auth-action=\"login\"]"));
  const blockerMatches = [];
  if (!locked) blockerMatches.push("desktop membership lock state not explicit");
  if (!sidebar) blockerMatches.push("desktop shell sidebar missing in membership lock readback");
  if (!memberState) blockerMatches.push("desktop member state missing in membership lock readback");
  return {
    kind: "desktop",
    routeKey: route.key,
    label: route.label,
    accessState: "membership_locked",
    activePanelId: "",
    activeRouteKey: "",
    activeNavView: "",
    routeIdentityOk: true,
    expectedPanelId: route.expectedPanelId || "",
    expectedRouteKey: route.expectedRouteKey || "",
    rowsVisible: 0,
    domRows: 0,
    ...
    sampleRows: [],
    filterCounts: [],
    freshnessText: String(memberState?.textContent || "會員尚未開通").replace(/\s+/g, " ").trim(),
    emptyStateText: "membership_locked",
    dateSignals: [],
    fieldSignals: [],
    blockerMatches,
    warnings: ["UI route content is protected until member credentials are supplied"],
...
async function runDesktopMode(browser, theme) {
  ...
  await sleep(1200);
  const memberCredentialsConfigured = Boolean(FUMAN_TEST_MEMBER_EMAIL && FUMAN_TEST_MEMBER_PASSWORD);
  const membershipLocked = await evaluate(cdp, () => document.body?.classList?.contains("auth-locked") || document.body?.dataset?.membershipAccess === "locked" || Boolean(document.querySelector("#auth-gate:not([hidden])")) || Boolean(document.querySelector(".sidebar-foot .logout[data-auth-action=\"login\"]"))).catch(() => false);
  const results = [];
  for (const route of DESKTOP_ROUTES.filter((item) => (!SKIP_WATCHLIST || item.key !== "watchlist") && (!ROUTE_FILTER.size || ROUTE_FILTER.has(item.key)))) {
    let stats = null;
    if (membershipLocked && !memberCredentialsConfigured) {
      stats = await evaluate(cdp, collectDesktopMembershipLockStats, route);
    } else {
    try {
      ...
      })(), ROUTE_TIMEOUT_MS, `desktop/${theme}/${route.key}`);    } catch (error) {
...
async function runMobileMode(browser, theme, viewport = MOBILE_VIEWPORTS["phone-portrait"]) {
  ...
  for (const route of MOBILE_ROUTES.filter((item) => (!SKIP_WATCHLIST || (item.key !== "watch" && item.fragment !== "watch")) && (!ROUTE_FILTER.size || ROUTE_FILTER.has(item.key) || ROUTE_FILTER.has(item.fragment)))) {
    ...
    try {
      const memberCredentialsConfigured = Boolean(FUMAN_TEST_MEMBER_EMAIL && FUMAN_TEST_MEMBER_PASSWORD);
      const membershipLocked = await evaluate(cdp, () => Boolean(document.querySelector("[data-mobile-auth-lock], .mobile-terminal-locked"))).catch(() => false);
      if (membershipLocked && !memberCredentialsConfigured) {
        stats = await evaluate(cdp, collectMobileStats, effectiveRoute);
      } else {
      stats = await withTimeout((async () => {
