...
function validateDbRow(row, expectedPayloads) {
  ...
  const allEvidence = row?.payload?.openingReport0830IndustryBias;
  const stageId = payloads[0]?.stage || "us_0820";
  const evidence = allEvidence?.stages?.[stageId] || allEvidence;
  ...
  const requiredEqual = {
    ...
    report_time: payloads[0]?.report_time || "08:20", stage: stageId,
    source: SOURCE,
    mode: MODE,
    boost_once: true,
    reason_code: REASON,
...
function fixture() {
  ...
  const payload = {
    date: "2026-09-08", report_time: morningStages.stage().time, stage: morningStages.stage().id, run_id: `${reportRunId}-ROBOTICS_AUTOMATION`, source: SOURCE, mode: MODE,
