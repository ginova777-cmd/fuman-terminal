...
function writeStrategy2LivePublishReceipt(report, runId, source = "scan-intraday-signals.js") {
  ...
  const receipt = {
    ...
    finishedAt: now,
    status: report.ok === false ? "failed" : "published_pending_closure",
    exitCode: report.ok === false ? 1 : 0,
    scanned: records.length,
    total: records.length,
    matches,
    complete: false,
    qualityStatus: report.ok === false ? (report.qualityStatus || "failed") : "published_pending_closure",
    fallback: false,
    ...
    source,
    warnings: report.ok === false ? [] : ["Supabase publish succeeded; awaiting strict tri-surface closure"],
    blockingReason: report.ok === false ? "" : "awaiting_strict_tri_surface_closure",
