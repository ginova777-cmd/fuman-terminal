...
const ACTIVE_MODULE_REGISTRY = JSON.parse(fs.readFileSync(path.join(ROOT, "scripts", "terminal-active-module-registry.json"), "utf8"));
const ACTIVE_MODULE_KEYS = new Set((ACTIVE_MODULE_REGISTRY.active || []).map((module) => module.key));
const isActiveModule = (key) => ACTIVE_MODULE_KEYS.has(key);
...
const terminalWaterRootContractGuard = spawnSync(process.execPath, ["--use-system-ca", path.join(ROOT, "scripts", "verify-terminal-water-root.js")], {
  cwd: ROOT,
  encoding: "utf8",
});
if (terminalWaterRootContractGuard.status !== 0) {
  issues.push(`verify-terminal-water-root failed: ${terminalWaterRootContractGuard.stderr || terminalWaterRootContractGuard.stdout || ""}`.trim());
}

const terminalFinalAuditContractGuard = spawnSync(process.execPath, ["--use-system-ca", path.join(ROOT, "scripts", "verify-terminal-final-audit-contract.js")], {
  cwd: ROOT,
  encoding: "utf8",
});
if (terminalFinalAuditContractGuard.status !== 0) {
  issues.push(`verify-terminal-final-audit-contract failed: ${terminalFinalAuditContractGuard.stderr || terminalFinalAuditContractGuard.stdout || ""}`.trim());
}
...
if (fumanScheduleRegistry) {
  ...
  const receiptCoverage = policy.receiptCoverage || {};
  for (const [taskName, receiptFile] of [
    ["Fuman Strategy3 Complete Scan 1300", "strategy3.json"],
    ["Fuman Strategy3 Battle Verify 1305", "strategy3.json"],
    ["Fuman Strategy4 Cache 1600", "strategy4.json"],
  ]) {
    const files = Array.isArray(receiptCoverage[taskName]?.files) ? receiptCoverage[taskName].files : [];
    if (!files.includes(receiptFile)) {
      issues.push(`scripts/fuman-schedule-registry.json policy.receiptCoverage must let ${taskName} use ${receiptFile}`);
    }
  }
...
const watchlistAutonomyText = read("scripts/verify-watchlist-autonomy.js");
if (!liveFreshnessGateText.includes("FUMAN_WATCHLIST_SKIP_FAST_GATE_LAST_RESULT")) {
  issues.push("run-live-freshness-gate.ps1 must set FUMAN_WATCHLIST_SKIP_FAST_GATE_LAST_RESULT around verify:watchlist-autonomy");
}
for (const marker of ["FUMAN_WATCHLIST_SKIP_FAST_GATE_LAST_RESULT", "SKIP_FAST_GATE_LAST_RESULT", "last result skipped during active freshness gate"]) {
  if (!watchlistAutonomyText.includes(marker)) issues.push(`verify-watchlist-autonomy.js missing freshness gate self-reference marker ${marker}`);
}
...
if (!String(packageJson.scripts?.["verify:shared-source-readonly"] || "").includes("ops/public-slot/SharedSourceReadOnlyVerifier.ps1")) {
...
const strategy3LatestApiContract = strategy3LatestApi + "\n" + read("api/strategy3-latest.shared-probe-legacy.js");
...
const realtimeRadarScanner = readOptional("scripts/scan-realtime-radar-cache.js");
...
const runOpenBuy = readOptional("run-open-buy.ps1");
const runStrategy1Preopen = readOptional("run-strategy1-preopen-common.ps1");
...
const runRealtimeRadar = readOptional("run-realtime-radar.ps1");
const runRealtimeRadarWatchdog = readOptional("run-realtime-radar-watchdog.ps1");
const realtimeRadarWatchdogInstaller = readOptional("scripts/install-realtime-radar-watchdog-task.ps1");
...
const sharedSourceReadonlyVerifier = read("ops/public-slot/SharedSourceReadOnlyVerifier.ps1");
...
if (isActiveModule("strategy1") && !readOptional("scripts/scan-open-buy-cache.js").includes('FUMAN_PUBLISH_GATE_STRATEGY: "strategy1"')) {
  issues.push("scan-open-buy-cache.js must call publish hard gate with FUMAN_PUBLISH_GATE_STRATEGY=strategy1");
}
for (const [file, text] of Object.entries({
  "api/open-buy-latest.js": readOptional("api/open-buy-latest.js"),
...
for (const [file, text] of Object.entries({
  "scripts/scan-open-buy-cache.js": readOptional("scripts/scan-open-buy-cache.js"),
...
for (const marker of [
  ...
  if (!sharedSourceReadonlyVerifier.includes(marker)) issues.push(`SharedSourceReadOnlyVerifier.ps1 missing read-only verifier marker ${marker}`);
}
if (/Write-PublicSlot|Invoke-PublicSlotUpsert|Invoke-RestMethod[\s\S]{0,120}-Method\s+(Post|Patch|Delete)|Set-Content|Add-Content|Out-File/i.test(sharedSourceReadonlyVerifier)) {
  issues.push("SharedSourceReadOnlyVerifier.ps1 must stay read-only: no writes, no source_status overwrite, no local cache/runtime output");
}
...
for (const marker of ["tvPassCount", "normalizeStrategy3ApiContract", "strategy3TvOk"]) {
  if (!strategy3LatestApiContract.includes(marker)) issues.push(`api/strategy3-latest.js missing Strategy3 API contract marker ${marker}`);
}
for (const [file, source] of [
  ["api/strategy3-latest.js", strategy3LatestApiContract],
...
if (!/compactSnapshotEndpoints/.test(terminalFastBundleApi) || !/Vercel-CDN-Cache-Control",\s*"public, max-age=3, stale-while-revalidate=12"/.test(terminalFastBundleApi)) {
...
if (isActiveModule("realtime-radar")) {
for (const marker of [
...
const openBuyScanner = readOptional("scripts/scan-open-buy-cache.js");
for (const marker of [
...
const strategy1AutonomyVerifier = readOptional("scripts/verify-strategy1-autonomy-readonly.js");
for (const marker of [
  "runSupabasePublishHardGate",
...
for (const legacyScript of [
  ...
  if (["run-open-buy.ps1", "run-realtime-radar.ps1", "run-realtime-radar-watchdog.ps1"].includes(legacyScript)) continue;
...
if (!fs.existsSync(path.join(ROOT, "REALTIME-RADAR-FRESHNESS-GOVERNANCE.md"))) {
  ...
  const realtimeRadarGovernance = readOptional("REALTIME-RADAR-FRESHNESS-GOVERNANCE.md");
  for (const marker of [
