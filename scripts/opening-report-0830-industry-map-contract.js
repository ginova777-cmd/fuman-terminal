...
const OPENING_REPORT_0830_INDUSTRY_MAP = [
  ...
  ...row,
  evidence_summary: row.overseas_leaders.filter(x=>morningStages.allowed(x[1])).length ? row.overseas_leaders.filter(x=>morningStages.allowed(x[1])).map(x=>x[0]).join("、") + "：本階段海外市場指標；只供產業觀察。" : "本階段無對應水源；保留台股對照。",
  detection_enabled: row.overseas_leaders.some(x=>morningStages.allowed(x[1])),
  detection_policy: morningStages.CONTRACT,
  mapping_contract: CONTRACT,
  mapping_reviewed_at: MAPPING_REVIEWED_AT,
  mapping_evidence_authorities: MAPPING_EVIDENCE_AUTHORITIES,
  priority_rank: index + 1,
  overseas_leaders: row.overseas_leaders.map(([name, yahoo_symbol, source_provider = ""]) => ({ name, yahoo_symbol, source_provider: require("../lib/opening-report-japan-realtime").SYMBOLS.includes(yahoo_symbol) ? require("../lib/opening-report-japan-realtime").PROVIDER : source_provider })),
...
function leaderPairs(row) {
  return row.overseas_leaders.filter(leader => morningStages.allowed(leader.yahoo_symbol)).map((leader) => [leader.name, leader.yahoo_symbol, leader.source_provider || ""]);
}
...
function validateIndustryMapContract(rows = OPENING_REPORT_0830_INDUSTRY_MAP) {
  ...
  rows.forEach((row, index) => {
    ...
    if (!Array.isArray(row.overseas_leaders)) issues.push(`overseas_leaders_missing:${row.industry}`);
  ...
  if (passive?.overseas_leaders?.length !== 1 || passive.overseas_leaders[0].yahoo_symbol !== "6981.T") issues.push("passive_components_anchor_invalid");
