...
function manifestPreviousGoodHoldClosure(manifest = {}, modules = []) {
  ...
  return modules.every((row) => row.ok === true
    ...
    && lower(row.evidenceStatus || "complete") === "complete");
}
...
function validateCanary(manifest, scorecard, options = {}) {
  ...
  const allowMarketClosedPublish = closed && (manifestFullyClosed(manifest, modules) || pendingPreviousGood || previousGoodHoldClosure);
  const allowPreviousGoodHoldClosurePublish = !closed && previousGoodHoldClosure && !pendingPreviousGood;
  const guardedPreviousGoodMode = closed || allowPendingRollForward || allowPreviousGoodHoldClosurePublish;
  const enforcePublishable = !guardedPreviousGoodMode;
  if (!guardedPreviousGoodMode && scorecard.qualityStatus && lower(scorecard.qualityStatus) !== "complete") issues.push(`scorecard_quality_not_complete:${scorecard.qualityStatus}`);
  ...
  for (const row of modules) {
    ...
    if (!guardedPreviousGoodMode && report.ok !== true) issues.push(`sourceReport_not_ok:${row.key}`);
