...
async function main() {
  ...
  const timeoutMs = Math.max(10000, Number(arg("timeout-ms", "240000")) || 240000);
  ...
  if (final) {
    ...
    Object.assign(final, {
      ...
      status: finalComplete ? "complete" : "failed",
      report_status: finalComplete ? "COMPLETE" : "FAIL_CLOSED",
      exitCode: finalComplete ? 0 : 1,
      first_blocker: finalComplete ? null : firstBlocker,
      checked_at: new Date().toISOString(),
    });
