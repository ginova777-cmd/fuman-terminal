"use strict";

// Strategy2 V3 only accepts its own receipt and the Fugle deep-scan contract.
// It deliberately has no compatibility route to V2 snapshots or old views.
const fs = require("fs");
const path = require("path");

const ROOT = path.resolve(__dirname, "..");
const RUNTIME_DIR = process.env.FUMAN_RUNTIME_DIR || "C:/fuman-runtime";
const RECEIPT = path.join(RUNTIME_DIR, "data", "scan-receipts", "strategy2-v3-water.json");
const COLLECTOR = path.join(ROOT, "scripts", "fugle-websocket-collector.js");
const COLLECTOR_WRAPPER = path.join(ROOT, "ops", "public-slot", "Run-DaytradeWebSocketCollector.ps1");
const WRITER = path.join(ROOT, "scripts", "run-daytrade-source-writer.js");
const DEPLOYED_SOURCE_WRITER = process.env.FUMAN_DAYTRADE_DEPLOYED_SOURCE_WRITER || "C:/fuman-release-owner/prod81/scripts/run-daytrade-source-writer.js";
const SCANNER = path.join(ROOT, "scripts", "run-strategy2-v3-water-scan.js");
const CONTRACT = "strategy2-v3-fugle-deep-scan-water-v1";
const MIN_FORMAL_WATER_COVERAGE_RATIO = 0.90;

function taipeiDate() {
  const parts = new Intl.DateTimeFormat("en-CA", {
    timeZone: "Asia/Taipei", year: "numeric", month: "2-digit", day: "2-digit",
  }).formatToParts(new Date());
  const get = (type) => parts.find((part) => part.type === type)?.value || "";
  return `${get("year")}-${get("month")}-${get("day")}`;
}

function asNumber(value) {
  const parsed = Number(value);
  return Number.isFinite(parsed) ? parsed : 0;
}

function addCheck(checks, name, ok, detail) {
  checks.push({ name, ok: Boolean(ok), detail: detail || "" });
}

function main() {
  const expectDate = (process.argv.find((value) => value.startsWith("--trade-date=")) || "").split("=")[1] || taipeiDate();
  const checks = [];
  const scanner = fs.readFileSync(SCANNER, "utf8");
  const receipt = JSON.parse(fs.readFileSync(RECEIPT, "utf8"));
  const collector = fs.readFileSync(COLLECTOR, "utf8");
  const collectorWrapper = fs.readFileSync(COLLECTOR_WRAPPER, "utf8");
  const writer = fs.readFileSync(WRITER, "utf8");
  const deployedWriter = fs.existsSync(DEPLOYED_SOURCE_WRITER) ? fs.readFileSync(DEPLOYED_SOURCE_WRITER, "utf8") : "";

  addCheck(checks, "scanner_has_v3_contract", scanner.includes(CONTRACT));
  addCheck(checks, "scanner_reads_priority_pool", scanner.includes('"fugle_daytrade_priority_pool"'));
  addCheck(checks, "scanner_reads_canonical_v2_websocket_status_only", scanner.includes("fugle-daytrade-websocket-status-v2.json") && !scanner.includes('"fugle-daytrade-websocket-status.json"'));
  addCheck(checks, "scanner_reads_fugle_quotes", scanner.includes("readFugleWebSocketQuotes") && scanner.includes("fugle_daytrade_websocket_cache"));
  addCheck(checks, "scanner_reads_formal_1m", scanner.includes("readFugleWebSocketCandles") && scanner.includes("fugle_daytrade_websocket_cache"));
  addCheck(checks, "scanner_rejects_old_view_queries", !/readRows\(source,\s*["']v_fugle_daytrade_mother_pool/i.test(scanner));
  addCheck(checks, "scanner_has_no_top40_gate", scanner.includes("noTop40Gate: true"));
  addCheck(checks, "collector_reserves_dynamic_mother_pool_aggregate_capacity_600", collector.includes("FUGLE_STREAMING_AGGREGATE_SYMBOLS || 600"));
  addCheck(checks, "collector_wrapper_pins_aggregate_capacity_600", /FUGLE_STREAMING_AGGREGATE_SYMBOLS\s*=\s*"600"/.test(collectorWrapper));
  addCheck(checks, "collector_total_subscription_budget_remains_1800", /FUGLE_STREAMING_MAX_TOTAL_SUBSCRIPTIONS\s*=\s*"1800"/.test(collectorWrapper));
  addCheck(checks, "collector_candle_capacity_remains_1000", /FUGLE_STREAMING_CANDLE_SYMBOLS\s*=\s*"1000"/.test(collectorWrapper));
  addCheck(checks, "writer_exports_strategy2_formal_water_symbols", writer.includes("strategy2Symbols: strategy2FormalWaterSymbols") && writer.includes('strategy2FormalWaterSource: "daytrade_deep_scan_pool"'));
  addCheck(checks, "collector_consumes_strategy2_priority_symbols", collector.includes('addMany("strategy2", payload.strategy2 || payload.strategy2Symbols'));
  addCheck(checks, "deployed_source_writer_exports_strategy2_formal_water_symbols", deployedWriter.includes("strategy2Symbols: strategy2FormalWaterSymbols") && deployedWriter.includes('strategy2FormalWaterSource: "daytrade_deep_scan_pool"'), DEPLOYED_SOURCE_WRITER);
  addCheck(checks, "collector_supervisor_poll_retains_sleep", /transient status-file read can never discard the formal candle cache\.\s*\r?\n\s*Start-Sleep -Seconds 5/.test(collectorWrapper));
  addCheck(checks, "scanner_uses_current_water_reason_codes", scanner.includes("strategy2_v3_formal_water_ready") && scanner.includes("strategy2_v3_formal_water_incomplete"));
  addCheck(checks, "scanner_declares_formal_water_coverage_contract_90", scanner.includes("MIN_FORMAL_WATER_COVERAGE_RATIO = 0.90") && scanner.includes("formalWaterCoverageRatio") && scanner.includes("formalWaterCoverageOk"));
  addCheck(checks, "diagnostic_water_receipt_is_isolated", scanner.includes("strategy2-v3-water-diagnostic-latest.json") && scanner.includes('diagnostic ? "latest-diagnostic.json" : "latest.json"'));
  addCheck(checks, "diagnostic_never_writes_formal_water_receipt", scanner.includes("const receiptPath = diagnostic ? diagnosticReceiptPath : formalReceiptPath") && scanner.includes("writeJson(receiptPath, report)"));
  addCheck(checks, "scanner_rejects_retired_rules_not_attached_reason", !scanner.includes("strategy_rules_not_yet_attached"));
  addCheck(checks, "scanner_has_no_previous_good_fallback", scanner.includes("noPreviousGoodFallback: true"));

  addCheck(checks, "receipt_is_v3", receipt.version === "v3" && receipt.strategy === "strategy2");
  addCheck(checks, "receipt_has_expected_contract", receipt.strategyContract === CONTRACT);
  addCheck(checks, "receipt_has_target_trade_date", receipt.dataDate === expectDate && receipt.tradeDate === expectDate, `${receipt.dataDate || "missing"} / ${expectDate}`);
  const coverage = receipt.sourceCoverage || {};
  const expected = asNumber(receipt.expectedCount);
  const ready = asNumber(coverage.formalIntradayOneMinuteReadySymbols || receipt.scannedCount);
  const requiredRatio = asNumber(coverage.requiredFormalWaterCoverageRatio || MIN_FORMAL_WATER_COVERAGE_RATIO);
  const actualRatio = asNumber(coverage.formalWaterCoverageRatio || (expected > 0 ? ready / expected : 0));
  const minimumReady = Math.ceil(expected * requiredRatio);
  addCheck(checks, "receipt_completed_water_check", receipt.ok === true && receipt.complete === true && coverage.formalWaterCoverageOk === true, receipt.status);
  addCheck(checks, "formal_water_coverage_at_least_required", expected > 0 && ready >= minimumReady && actualRatio >= requiredRatio, `${ready}/${expected} ratio=${actualRatio} required=${requiredRatio}`);
  addCheck(checks, "receipt_rejects_retired_rules_not_attached_reason", receipt.reason !== "strategy_rules_not_yet_attached", receipt.reason);
  addCheck(checks, "data_gap_within_formal_water_tolerance", asNumber(receipt.dataGapCount) <= Math.max(0, expected - minimumReady), `${receipt.dataGapCount}/${expected - minimumReady}`);
  addCheck(checks, "no_formal_candidate_before_rules", receipt.publishAllowed === false && receipt.formalDisplayAllowed === false && asNumber(receipt.resultCount) === 0, receipt.reason);
  addCheck(checks, "receipt_declares_new_only_routes", coverage.noLegacyReadbackViews === true && coverage.noTop40Gate === true && coverage.noPreviousGoodFallback === true);

  const rows = Array.isArray(receipt.records) ? receipt.records : [];
  addCheck(checks, "record_count_matches_scan", rows.length === asNumber(receipt.scannedCount), `${rows.length}/${receipt.scannedCount}`);
  addCheck(checks, "every_record_has_formal_quote_and_1m", rows.every((row) => row.formalQuoteReady === true && row.formalOneMinuteReady === true && /^fugle/i.test(String(row.quoteSource || "")) && asNumber(row.candleCount) >= 35));
  addCheck(checks, "every_record_has_today_source_evidence", rows.every((row) => String(row.sourceRunId || "") && String(row.firstCandleTime || "").startsWith(expectDate) && String(row.lastCandleTime || "").startsWith(expectDate)));

  const failures = checks.filter((check) => !check.ok);
  console.log(JSON.stringify({
    ok: failures.length === 0,
    status: failures.length === 0 ? "YES" : "NO",
    strategy: "strategy2",
    version: "v3",
    contract: CONTRACT,
    checkedAt: new Date().toISOString(),
    tradeDate: expectDate,
    runId: receipt.runId || "",
    expectedCount: receipt.expectedCount || 0,
    scannedCount: receipt.scannedCount || 0,
    dataGapCount: receipt.dataGapCount || 0,
    first_blocker: failures[0]?.name || null,
    checks,
  }, null, 2));
  process.exitCode = failures.length ? 1 : 0;
}

main();
