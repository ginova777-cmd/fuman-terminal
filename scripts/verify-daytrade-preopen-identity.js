...
async function main(){
 ...
 for(let attempt=1;attempt<=3;attempt++){
  ...
  const rows=[];let fetchError=null;
  for(let offset=0;offset<10000;offset+=200){
   let page=null,lastError=null;
   for(let pageAttempt=1;pageAttempt<=3;pageAttempt++){
    try{
     const r=await fetch(`${url}/rest/v1/v_fugle_daytrade_mother_pool_v4_1?select=*&trade_date=eq.${date}&order=symbol.asc&limit=200&offset=${offset}`,{headers:{apikey:key,Authorization:`Bearer ${key}`},signal:AbortSignal.timeout(15000)});
     if(!r.ok)throw Error(`anon_readback_http_${r.status}`);
     page=await r.json();break;
    }catch(error){lastError=error; if(pageAttempt<3)await new Promise(resolve=>setTimeout(resolve,1000*pageAttempt));}
   }
   if(!Array.isArray(page)){fetchError=lastError?.message||'anon_readback_fetch_failed';break;}
  ...
  if(fetchError)failures.unshift(fetchError);
