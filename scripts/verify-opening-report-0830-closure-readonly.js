...
function main() {
  ...
  const checks = [];
  checks.push(check("final_receipt", finalReceipt?.report_status === "REPORT_OK" && compact(finalReceipt?.date || finalReceipt?.trade_date) === day && Boolean(runId), "opening_report_final_receipt_invalid", { path: finalPath, report_status: finalReceipt?.report_status || null, run_id: runId || null }));
