...
const OUT_FILE = process.env.STRATEGY2_TRI_SURFACE_RECEIPT_FILE || path.join(OUT_DIR, "strategy2-tri-surface-canonical-latest.json");
...
async function main() {
  ...
  const mobileProtected = (mobile.status === 401 || /data-membership-required="1"|mobile-terminal-locked/i.test(mobile.text || "")) && /membership_required|missing_bearer_token|mobile-terminal-locked|data-membership-required="1"/i.test(mobile.text || "");
  ...
  check(checks, scorecard.status === 200 && (visibleFormal ? scorecardRow?.runId === expectedRunId : scorecardRow?.runId !== expectedRunId || scorecardRow?.publishAllowed === false), "scorecard_strategy2_row_run_id_matches", { status: scorecard.status, expectedRunId, row: scorecardRow });
  check(checks, reports.status === 200 && (visibleFormal ? reportRow?.runId === expectedRunId : reportRow?.runId !== expectedRunId || reportRow?.publishAllowed === false), "source_reports_strategy2_row_run_id_matches", { status: reports.status, expectedRunId, row: reportRow });
  ...
  check(checks, visibleFormal ? Boolean(bundleStrategy2) : !bundleStrategy2 || bundleRunId !== expectedRunId || bundleStrategy2?.publishAllowed === false, "terminal_fast_bundle_contains_strategy2_endpoint", {
  ...
  check(checks, visibleFormal ? Boolean(bundleStrategy2) && bundleRunId === expectedRunId : bundleRunId !== expectedRunId || bundleStrategy2?.publishAllowed === false, "terminal_fast_bundle_strategy2_run_id_matches", { expectedRunId, actual: bundleRunId, endpoint: bundleStrategy2 || null });
  check(checks, visibleFormal ? Number(mobile.status) === 200 && mobileId === expectedRunId : mobileProtected || Number(mobile.status) === 200, "mobile_fragment_strategy2_visible_run_id_matches", {
  ...
  check(checks, visibleBlocked || (expectedDate && bundleStrategy2?.dataDate === expectedDate && scorecardRow?.date === expectedDate && reportRow?.date === expectedDate), "tri_surface_trade_date_matches", { expectedDate, desktop: bundleStrategy2?.dataDate || "", scorecard: scorecardRow?.date || "", sourceReports: reportRow?.date || "" });
  check(checks, visibleBlocked || (Number(bundleStrategy2?.resultCount) === expectedResultCount && Number(bundleStrategy2?.count) === expectedResultCount && Number(scorecardRow?.resultCount) === expectedResultCount && Number(scorecardRow?.count) === expectedResultCount && Number(reportRow?.resultCount) === expectedResultCount && Number(reportRow?.count) === expectedResultCount && mobileResultCount === expectedResultCount), "tri_surface_result_count_matches", { expectedResultCount, desktopCount: bundleStrategy2?.count, desktopResultCount: bundleStrategy2?.resultCount, mobileResultCount, scorecardCount: scorecardRow?.count, sourceReportCount: reportRow?.count });
  check(checks, visibleFormal || visibleBlocked, "blocked_evidence_fail_closed_or_formal_complete", { visibleFormal, visibleBlocked, scorecardPublishAllowed: scorecardRow?.publishAllowed, sourceReportPublishAllowed: reportRow?.publishAllowed, blockedReason: scorecardRow?.blockedReason || "" });
