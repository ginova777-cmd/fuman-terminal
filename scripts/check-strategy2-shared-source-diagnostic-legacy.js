...
async function checkOnce() {
  ...
  const futoptMappingResult = await fetchStockFutureLiveContracts({ limit: 5000 }).catch((error) => ({ ok: false, error: error?.message || String(error), rows: [] }));
  const futoptQuoteResult = await fetchDaytradeFutoptQuotesLive({ limit: 5000 }).catch((error) => ({ ok: false, error: error?.message || String(error), bySymbol: new Map(), count: 0 }));
  ...
  const anonReadFailures = Array.isArray(health.anonRead?.failed) ? health.anonRead.failed : [];
  const onlySlowPriorityReadinessTimeout = anonReadFailures.length > 0
    && anonReadFailures.every((item) => item?.table === "v_fugle_daytrade_priority_readiness"
      && String(item?.error || "").includes("57014"));
  const anonReadIssueSeverity = onlySlowPriorityReadinessTimeout ? "warning" : "critical";
  ...
  const issues = [
    ...
    issue(Boolean(health.anonRead?.ok), anonReadIssueSeverity, "anon-read-failed", onlySlowPriorityReadinessTimeout ? "nonblocking slow priority readiness view timed out; dedicated daytrade gate remains authoritative" : "anon read target check failed", { failed: health.anonRead?.failed || [], nonBlocking: onlySlowPriorityReadinessTimeout }),
