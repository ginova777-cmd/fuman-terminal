...
for(const [time,ok]of [['00:00:00',true],['00:20:00',true],['00:50:59.999',true],['00:51:00',false]]){assert.equal(japan.inWindow(Date.parse(date+'T'+time+'Z'),date),ok);assert.equal(assessLeaderFreshness({yahoo_symbol:'4062.T',source_time:date+'T'+time+'Z'},date).fresh,ok);}
assert.equal(japan.inWindow(Date.parse('2026-09-11T00:50:00Z'),date),false);
...
const check=vm.runInNewContext('('+body+')',{timestamp:()=>new Date().toISOString(),morningRecovery:require("../lib/opening-report-recovery")});
...
const preflight=fs.readFileSync(path.join(root,'scripts/run-opening-report-0830-preflight.js'),'utf8');assert.ok(preflight.includes('taipeiTime >= "08:50" && taipeiTime < "08:51"'));assert.ok(preflight.includes('const selfTest = false'));assert.ok(preflight.includes('--wrapper-owned'));
(async()=>{const good={ok:true,date,run_id:'run-A',cutoff:date+' 08:50:59 Asia/Taipei',industries:Array(15).fill({})};assert.equal((await check(date,'run-A',good)).ok,true);for(const patch of [{run_id:'run-B'},{date:'2026-09-11'},{cutoff:date+' 08:20:59 Asia/Taipei'},{industries:[]},{ok:false}])assert.equal((await check(date,'run-A',{...good,...patch})).ok,false);assert.equal((await check(date,'run-A',null)).ok,false);console.log(JSON.stringify({ok:true,unified_0830:true,old_task_retired:true,source_run_identity:true,late_and_old_source_rejected:true}));})().catch(e=>{console.error(e);process.exitCode=1;});
