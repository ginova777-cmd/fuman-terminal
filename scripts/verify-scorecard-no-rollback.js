...
function verifySchedules(checks) {
  ...
  const closure = queryTask("Fuman Strategy3 V2 Daily Closure Verify 1310");
  ...
  addCheck(checks, Boolean(closure), "schedule-strategy3-v2-closure-exists", "Fuman Strategy3 V2 Daily Closure Verify 1310 must exist", { closure });
  addCheck(checks, !/disabled/i.test(cleanText(closure?.State)), "schedule-strategy3-v2-closure-enabled", "Fuman Strategy3 V2 Daily Closure Verify 1310 must stay enabled", { state: closure?.State });
  addCheck(checks, /verify-strategy3-v2-daily-unattended-closure\.js/i.test(cleanText(closure?.TaskToRun)), "schedule-strategy3-v2-closure-runner", "Strategy3 V2 closure task must run the V2 daily verifier", { taskToRun: closure?.TaskToRun });
  addCheck(checks, /(?:T|\s|^)13:10/i.test(cleanText(closure?.TriggerStart) || cleanText(closure?.TriggerText)), "schedule-strategy3-v2-closure-1310", "Strategy3 V2 closure must trigger at 13:10 Asia/Taipei", { triggerStart: closure?.TriggerStart, triggerText: closure?.TriggerText });
