...
Register-ScheduledTask -TaskName $TaskName -Action $action -Trigger $trigger -Settings $settings -Principal $principal -Description "Unique Strategy2 V3 runner: observe 08:45, scan 09:00-12:30, finalize once at 12:30; one daily runId and fail-closed tri-surface evidence." -Force | Out-Null
foreach ($legacy in @("Fuman Strategy2 V3 Water Gate 0845","Fuman Strategy2 V2 Unattended","Fuman Strategy2 V2 Recovery")) {
