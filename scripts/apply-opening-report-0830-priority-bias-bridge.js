...
function validate(payload, options = {}) {
  ...
  if (!(payload?.mapped_symbols || []).every((row) => row?.mapping_status === "reviewed" && ["A", "B"].includes(row?.mapping_grade) && row?.mapping_grade === row?.tier && row?.mapping_industry === payload.industry && String(row?.mapping_reason || "").includes(String(row?.symbol || "")) && Array.isArray(row?.evidence_authorities) && row.evidence_authorities.length >= 2 && Array.isArray(row?.evidence_urls) && row.evidence_urls.length >= 2)) issues.push("mapped_symbol_evidence_incomplete");
