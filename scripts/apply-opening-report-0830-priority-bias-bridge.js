...
function quoteTimestamp(row) {
  ...
  return payload.quote_seen_at || payload.quoteSeenAt || payload.received_at || payload.receivedAt || payload.lastUpdated || payload.last_updated_at || payload.motherPoolMetrics?.quoteUpdatedAt || payload.motherPoolMetrics?.quote_updated_at || row?.updated_at || "";
}

function priorityPoolPrice(row) {
  const payload = row?.payload && typeof row.payload === "object" ? row.payload : {};
  const value = payload.price ?? payload.last_price ?? payload.lastPrice ?? payload.close ?? payload.motherPoolMetrics?.price ?? payload.mother_pool_metrics?.price;
...
function validate(payload, options = {}) {
  ...
  const priorityRank = Number(payload?.priority_observation_rank);
  const basis = String(payload?.priority_observation_basis || "");
  if (!Number.isInteger(priorityRank) || priorityRank < 1 || priorityRank > 3) issues.push("industry_not_priority_observation_top3");
  if (!["positive_industry_top3", "us_market_closed_asia_positive_leader_top3"].includes(basis)) issues.push("priority_observation_basis_invalid");
  if (basis === "us_market_closed_asia_positive_leader_top3") {
    const leaders = Array.isArray(payload?.priority_overseas_leaders) ? payload.priority_overseas_leaders : [];
    if (!leaders.length || !leaders.every((row) => /\.(?:T|KS|KQ)$/i.test(String(row?.symbol || "")) && Number(row?.percent) > 0)) issues.push("asia_positive_leader_evidence_invalid");
  }
...
function buildReceipt({ inputPath, receiptPath, payload, validation, acceptedSymbols, rejectedSymbols, appliedBoosts, error = "" }) {
  return {
    ...
    priority_observation_rank: payload?.priority_observation_rank ?? null,
    priority_overseas_leaders: Array.isArray(payload?.priority_overseas_leaders) ? payload.priority_overseas_leaders : [],
...
async function main() {
  ...
  try {
    ...
    const rows = acceptedSymbols.map((symbol, index) => {
      ...
      const price = priorityPoolPrice(old);
      ...
      const biasEvidence = mergeOpeningReportEvidence(previousEvidence, payload);
      const linkedIndustries = biasEvidence?.linked_industries || [payload.industry];
      const sourceFlags = [...new Set([...(Array.isArray(oldPayload.source_flags) ? oldPayload.source_flags : []), SOURCE])];
      const priorityReasons = [...new Set([...(Array.isArray(oldPayload.priority_reasons) ? oldPayload.priority_reasons : []), REASON_CODE])];
      ...
      return {
        symbol,
        name: old.name || mappedEntryBySymbol.get(symbol)?.name || symbol,
        market: isTaiwanMarket(old.market) ? String(old.market).toUpperCase() : "TW",
        priority_rank: nextRank,
        hot_extension_rank: Number.isFinite(Number(old.hot_extension_rank)) ? Number(old.hot_extension_rank) : null,
        priority_reason: old.priority_reason || REASON_CODE,
        source: old.source || SOURCE,
        updated_at: now,
        payload: {
          ...oldPayload,
          openingReport0830IndustryBias: biasEvidence,
          opening_report_0830_source: SOURCE,
          opening_report_0830_priority_reason: REASON_CODE,
          source_flags: sourceFlags,
          priority_reasons: priorityReasons,
          priority_status: "watchlist_boosted",
