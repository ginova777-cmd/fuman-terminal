...
const formalEvidence = { source_status: "ok", gate_grade: "A", formal_entry_speed_verdict: "YES", formal_entry_allowed: true };
const fixture = [
  {
    trade_date: today,
...
const checks = [
  ...
  ["sql_entry_time_field", /entry_time\s+time without time zone/i.test(sql)],
  ["sql_strategy_label_field", /strategy_label\s+text/i.test(sql)],
  ["sql_regular_session_constraint", /coalesce\(detect_time,\s*entry_time\)[\s\S]*09:00:00[\s\S]*13:30:00/i.test(sql)],
  ...
  ["sql_grant_select_insert", /grant select,\s*insert on public\.seven_strategy_daily_history to anon,\s*authenticated/i.test(sql)],
  ["sql_insert_policy", /for insert[\s\S]*to anon,\s*authenticated[\s\S]*with check \(true\)/i.test(sql)],
  ["sql_sequence_usage", /grant usage,\s*select on sequence public\.seven_strategy_daily_history_id_seq to anon,\s*authenticated,\s*service_role/i.test(sql)],
