...
async function main(){
 ...
 for(const r of rows){
  ...
  if(r.calculation_version!=="five-minute-indicators-macd-3-9-3-v4")issues.push(`${r.symbol}:calculation_version`);
  if(r.trend_5m_strategy_version!=="golden-cross-any-macd-3-9-3-v4")issues.push(`${r.symbol}:strategy_version`);
 ...
 const branchNames=["rsi3_cross_rsi6_up_5m","kd_5_3_golden_cross_5m","macd_3_9_3_golden_cross_5m","ma5_cross_ma10_up_5m","ma10_cross_ma20_up_5m","ma5_cross_ma20_up_5m"];
 ...
 const uniqueIssues=[...new Set(issues)],out={contract:"daytrade_intraday_5m_history_replay_verifier_v2",status:uniqueIssues.length?"blocked":"complete",complete:!uniqueIssues.length,run_id:runId,trade_date:tradeDate,view:"v_fugle_intraday_5m_history_readback",anon_http_status:all.status,ssl_ok:true,tls_verification_disabled:false,calculation_version:"five-minute-indicators-macd-3-9-3-v4",strategy_version:"golden-cross-any-macd-3-9-3-v4",classification_contract:"daytrade_intraday_5m_branch_independent_strict_wait_v1",summary,issues:uniqueIssues,first_blocker:uniqueIssues[0]||null,checked_at:new Date().toISOString()};
