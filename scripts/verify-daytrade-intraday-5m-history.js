...
async function main(){
 const key=process.env.SUPABASE_ANON_KEY||secret("supabase-anon-key.txt"),runId=arg("run-id"),tradeDate=arg("trade-date"),sample=arg("sample-symbol")||"6290";
 ...
 const all=await paged(key,`${base}&order=symbol.asc,bar_start.asc`),sampleRead=await request(key,`${base}&symbol=eq.${sample}&order=bar_start.asc`),asOf=`${tradeDate}T02:32:00Z`,replay=await request(key,`${base}&symbol=eq.${sample}&bar_end=lte.${encodeURIComponent(asOf)}&bar_complete=eq.true&order=bar_end.asc`),rows=all.rows,issues=[];
