...
function qualifyBaseline(dateKey, productionLive, completionAudit) {
  const summary = readJson(summaryPath(dateKey));
  const manifest = readJson(manifestPath(dateKey));
  const checkpoints = Object.fromEntries(CHECKPOINT_PHASES.map((phase) => [phase, readJson(checkpointPath(dateKey, phase))]));
  const failures = [];
  if (isWeekend(dateKey)) failures.push('baseline_must_be_qualified_on_trading_day');
...
function maybeArmBaseline(todayKey, productionLiveFile, completionAuditFile, baselineState) {
  if (baselineState.valid || isWeekend(todayKey)) return { baseline: baselineState, armedNow: false, qualification: null };
  const productionLive = readJson(productionLiveFile);
  const completionAudit = readJson(completionAuditFile);
  const qualification = qualifyBaseline(todayKey, productionLive, completionAudit);
...
function main() {
  ...
  const baselineResult = maybeArmBaseline(todayKey, productionLiveFile, completionAuditFile, baselineBefore);
  ...
  const days = trackingStarted && observationEnd >= trackingStart ? (() => {
    const rows = [];
    for (let key = trackingStart; ; key = nextDate(key)) {
      rows.push(evaluateDay(key));
