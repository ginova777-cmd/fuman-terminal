...
async function main(){
 const valid={complete:true,status:'complete',exit_code:0,read_role:'anon',trade_date:'2026-09-17',canonical_run_id:'canonical',
  failed_checks:[],first_blocker:null,mother_pool_run_id:'one',snapshot_sequence:1};
 for(const patch of [{read_role:'service_role'},{trade_date:'2026-09-16'},{canonical_run_id:'other'},{exit_code:1},{failed_checks:['missing']},{first_blocker:'missing'},{status:'blocked'}]) {
  let committed=false;
  await assert.rejects(()=>publishWithRecovery({tradeDate:valid.trade_date,canonicalRunId:'canonical',loadIntent:()=>null,saveIntent:()=>{},
   build:()=>({trade_date:valid.trade_date,canonical_run_id:'canonical',run_id:'one',snapshot_sequence:1}),validate:()=>true,
   send:()=>({ok:true,run_id:'one',snapshot_sequence:1}),readback:()=>({...valid,...patch}),commit:()=>{committed=true;}}),/READBACK_FAILED/);
 ...
 for(const fault of ['rpc_after_commit','local_disk','ack_intent']) {
  ...
  const snapshot={trade_date:'2026-09-17',canonical_run_id:'canonical',run_id:'one',snapshot_sequence:1};
  const options={tradeDate:snapshot.trade_date,canonicalRunId:'canonical',validate:()=>true,
   ...
   readback:s=>({complete:true,status:'complete',exit_code:0,read_role:'anon',trade_date:s.trade_date,canonical_run_id:s.canonical_run_id,
    failed_checks:[],first_blocker:null,mother_pool_run_id:s.run_id,snapshot_sequence:s.snapshot_sequence}),
