...
const active = taskDisabled(ACTIVE_TASK);
const activeActionOk = active.exists
  && /powershell(?:\.exe)?/i.test(active.xml)
  && /run-strategy2-live-v2\.ps1/i.test(active.xml)
  && !active.disabled;
...
const legacy = LEGACY_TASKS.map((entry) => {
  const state = taskDisabled(entry.name);
...
const unattendedReady = dataPathIsolated && !interactiveOnly && !batteryBlocked;
const issues = [];
if (!activeActionOk) issues.push("strategy2_v2_guarded_task_missing_or_wrong_action");
for (const entry of legacy) {
  if (!entry.isolated) issues.push(`legacy_task_can_still_write:${entry.task}`);
}
...
const report = {
  ok: unattendedReady,
  status: unattendedReady ? "READY_FOR_UNATTENDED" : dataPathIsolated ? "DATA_PATH_ISOLATED_BUT_WINDOWS_UNATTENDED_BLOCKED" : "STRATEGY2_V2_ISOLATION_FAILED",
