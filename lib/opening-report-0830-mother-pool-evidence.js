...
function mergeSingleStageEvidence(previousEvidence, payloads) {
  ...
  return {
    contract: CONTRACT,
    date,
    report_time: first.report_time || "08:20",
    stage: first.stage || "us_0820",
...
function mergeOpeningReportEvidence(previousEvidence, payloads) {
  const inputs = Array.isArray(payloads) ? payloads : [payloads];
  const id = inputs[0]?.stage || "us_0820";
  require("./opening-report-stage-contract").stage(id);
  const date = String(inputs[0]?.date || "").slice(0,10);
  const previous = previousEvidence?.date === date ? previousEvidence : null;
  const stages = {...(previous?.stages || {})};
  if (previous && !previous.stages) stages[previous.stage || "us_0820"] = previous;
  const merged = mergeSingleStageEvidence(stages[id], inputs);
...
function validateEvidenceForPayload(evidence, expectedPayload, expectedReportRunId = "") {
  ...
  if (row.report_time !== (expected.report_time || "08:20")) issues.push("evidence_report_time_mismatch");
