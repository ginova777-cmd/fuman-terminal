...
function admit(row, {ratioField, directionValid=true}={}) {
 ...
 if(!Number.isInteger(row.same_minute_sample_count)||row.same_minute_sample_count<10)reasons.push('INSUFFICIENT_SAMPLE');
 if(typeof row.same_minute_baseline!=='number'||!Number.isFinite(row.same_minute_baseline)||row.same_minute_baseline<=0)reasons.push('HISTORICAL_BASELINE_INVALID_OR_ZERO');
