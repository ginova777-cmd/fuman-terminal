...
const STAGES = Object.freeze([
  {
    ...
    allowedAction: "restore_or_register_power_recovery_task_then_retry_final_audit",
  },
  {
    key: "preflight",
...
function lockHolderStatus(pid) {
  const id = Number(pid || 0);
  if (!Number.isFinite(id) || id <= 0) return { running: false, expected: false, reason: "missing_pid" };
  if (process.platform === "win32") {
    const probe = spawnSync("tasklist.exe", ["/FI", `PID eq ${id}`, "/FO", "CSV", "/NH"], { encoding: "utf8", windowsHide: true, timeout: 5000 });
    const raw = String(probe.stdout || "") + String(probe.stderr || "");
    const row = raw.split(/\r?\n/).find((line) => line.includes(`"${id}"`)) || "";
    const image = (row.match(/^"([^"]+)"/) || [])[1] || "";
    const running = Boolean(row);
    const expected = running && /^node(\.exe)?$/i.test(image);
    return { running, expected, image, reason: running ? (expected ? "expected_node_process" : "unexpected_lock_process") : "pid_not_running" };
  }
  try {
    process.kill(id, 0);
    return { running: true, expected: true, reason: "process_alive" };
  } catch {
    return { running: false, expected: false, reason: "pid_not_running" };
  }
}

function reclaimStaleLock(file, now, reason) {
  const stale = `${file}.stale-${now}-${safeName(reason, "stale")}`;
  fs.renameSync(file, stale);
  return stale;
}
function acquireOrchestratorLock({ dailyRunId, tradeDate, runtimeDir = defaultRuntimeDir(), ttlMs = 30 * 60 * 1000, ownerToken = process.env.FUMAN_ORCHESTRATOR_LOCK_OWNER || "" } = {}) {
  ...
  try {
    ...
    if (ownerToken && existing.owner_token === ownerToken && existing.daily_run_id === payload.daily_run_id && existing.trade_date === payload.trade_date) {
      return { ok: true, file, payload: existing, reentrant: true, ownerToken };
    }
    const holderStatus = lockHolderStatus(existing.pid);
    const expiresAt = Date.parse(existing.expires_at || "");
    if (!holderStatus.running || holderStatus.expected === false || (Number.isFinite(expiresAt) && expiresAt <= now)) {
      try {
        const stale = reclaimStaleLock(file, now, holderStatus.reason || "expired");
        const next = acquireOrchestratorLock({ dailyRunId, tradeDate, runtimeDir, ttlMs, ownerToken });
        if (next.ok) next.reclaimed = { stale, holderStatus, previous: existing };
        return next;
      } catch (staleError) {
        return { ok: false, file, reasonCode: "orchestrator_lock_stale_but_not_reclaimable", holder: existing, holderStatus, error: String(staleError.message || staleError) };
      }
    }
    return { ok: false, file, reasonCode: "orchestrator_lock_held", holder: existing, holderStatus };
  }
}

function releaseOrchestratorLock(lock) {
  if (!lock?.ok || !lock.file) return { ok: true, released: false };
  if (lock.reentrant === true) return { ok: true, released: true, reentrant: true, owner_retained: true, file: lock.file };
...
function reasonCodeFor(stage, parsed, output = "", fallback = "stage_not_ready") {
  ...
  const stageDefaults = {
    market_calendar: "market_calendar_not_ready",
        power_recovery: "power_recovery_not_verified",
