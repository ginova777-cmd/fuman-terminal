...
function buildStatusFromArtifacts(artifacts, source) {
  ...
  return sanitizeOpsStatusDiagnostics({
    ...
    action: finalAuditBlocksOps ? finalAuditAllowedAction : (decision.action || policy.decision?.action || waitCondition?.allowedAction || ""),
    reason: effectiveReason,
    reasonCode: finalAuditBlocksOps ? finalAuditReasonCode : (reasonCodeSummary.primaryCode || ""),
    firstBlocker: finalAuditBlocksOps ? finalAuditBlocker : (blockers[0] || ""),
    allowedAction: finalAuditBlocksOps ? finalAuditAllowedAction : (blockers.length ? (reasonCodeSummary.actions[0] || "manual_investigate_first_blocker") : (waitCondition?.allowedAction || reasonCodeSummary.actions[0] || "continue_autonomous_monitoring")),
    waitCondition,
    rawBlockers,
    blockers,
...
function isExpectedPendingBlocker(value) {
  const text = String(value || "").toLowerCase();
  return isPendingNotDueText(text) || text === "runid_closure_not_ok";
}
