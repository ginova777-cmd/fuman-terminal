...
function buildStatusFromArtifacts(artifacts, source) {
  ...
  const readinessPendingNotDue = readinessReport.status === "PENDING_NOT_DUE"
    || (Array.isArray(readinessReport.blockers) && readinessReport.blockers.some((row) => String(row || "").startsWith("pending_not_due")));
  ...
  const modules = moduleRows(manifest, controlPlane).map(enrichReasonClassification);
  ...
  const blockers = readinessPendingNotDue
    ? compactComputeBlockers(readinessReportBlockers.length ? readinessReportBlockers : ["pending_not_due"])
  ...
  const gates = {
    ...
    dailyManifest: enrichGate(readinessPendingNotDue ? { ok: true, status: "PENDING_NOT_DUE", reason: blockers[0], pendingNotDue: true } : (controlPlane.dailyManifest || { ok: manifest.ok, status: manifest.ok ? "GREEN" : "BLOCKED", reason: manifest.blocker })),
    ...
    runIdClosure: enrichGate(readinessPendingNotDue ? { ok: true, status: "PENDING_NOT_DUE", reason: blockers[0], pendingNotDue: true } : controlPlane.runIdClosure),
  ...
  if (readinessPendingNotDue) {
    ...
    outputActionMatrix.formalScan = { ...(outputActionMatrix.formalScan || {}), allowed: false, mode: "wait_schedule", reason: blockers[0] || "pending_not_due" };
    outputActionMatrix.publish = { ...(outputActionMatrix.publish || {}), allowed: false, mode: "preserve_previous_good", reason: blockers[0] || "pending_not_due" };
    outputActionMatrix.terminalDisplay = { ...(outputActionMatrix.terminalDisplay || {}), allowed: true, mode: "previous_good_hold", reason: blockers[0] || "pending_not_due" };
