...
function canonicalRunIdsFromModules(modules = []) {
  const byKey = new Map();
  for (const row of Array.isArray(modules) ? modules : []) {
    const key = String(row?.key || "").trim();
    const runId = String(row?.runId || "").trim();
    if (key && runId) byKey.set(key, runId);
  }
  return byKey;
}

function redactUnexpectedRunIds(value, canonicalByKey) {
  if (!canonicalByKey?.size) return value;
  ...
  if (Array.isArray(value)) return value.map((item) => redactUnexpectedRunIds(item, canonicalByKey));
  if (value && typeof value === "object") {
    const next = {};
    for (const [key, item] of Object.entries(value)) next[key] = redactUnexpectedRunIds(item, canonicalByKey);
    return next;
  }
  return value;
}

function sanitizeOpsStatusDiagnostics(status = {}) {
  const modules = Array.isArray(status.modules) ? status.modules : [];
  const canonicalByKey = canonicalRunIdsFromModules(modules);
  if (!canonicalByKey.size) return status;
  const sanitized = redactUnexpectedRunIds(status, canonicalByKey);
...
function buildStatusFromArtifacts(artifacts, source) {
  ...
  const gates = {
    ...
    stateMachine: enrichGate(readinessPendingNotDue ? { ok: true, status: "PENDING_NOT_DUE", reason: pendingReason, pendingNotDue: true } : controlPlane.stateMachine),
    ...
    finalAudit: enrichGate(readinessPendingNotDue
      ? { ok: true, status: "PENDING_NOT_DUE", reason: pendingReason, pendingNotDue: true }
      : finalAudit),
  };
  ...
  if ((ok || readinessPendingNotDue) && reasonCodeSummary.ok === true && Number(reasonCodeSummary.unknownEntries || 0) === 0) {
