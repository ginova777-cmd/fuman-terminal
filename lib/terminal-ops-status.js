...
function buildStatusFromArtifacts(artifacts, source) {
  ...
  const readinessRootCauseOk = readinessReport?.rootCauseSummary?.ok === true && Number(readinessReport?.rootCauseSummary?.unknownBlockers || 0) === 0;
  const finalAuditForBlockers = readinessRootCauseOk
    ? { ...finalAudit, issues: (Array.isArray(finalAudit?.issues) ? finalAudit.issues : []).filter((row) => (row?.issue || row?.code || String(row)) !== "ops_status_root_cause_summary_not_ok") }
    : finalAudit;
  const blockers = compactComputeBlockers(compactBlockers({
    decision,
    manifest,
    controlPlane,
    finalAudit: finalAuditForBlockers,
  ...
  const effectiveState = rollForwardAuthBlocked ? rollForwardDecision.state : (decision.state || decision.opsState || controlPlane.stateMachine?.overallState || orchestrator.overallState || "");
  const effectiveReason = rollForwardAuthBlocked ? (rollForwardDecision.reason || "backend_service_token_auth_blocked") : (decision.reason || policy.decision?.reason || manifest.blocker || blockers[0] || "");
