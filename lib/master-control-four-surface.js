...
const ROUTE88_NONEMPTY_FIELDS = ["strategy", "tradeDate", "sourceDate", "runId", "startedAt", "finishedAt", "qualityStatus", "evidenceStatus", "desktopStatus", "mobileStatus", "scorecardUpdatedAt", "reasonCode"];
const ROUTE88_COUNT_FIELDS = ["universeCount", "scannedCount", "resultCount"];
const ROUTE88_BOOLEAN_FIELDS = ["fallbackUsed", "publishAllowed"];
...
function route88FieldIssues(strategy, row) {
  const issues = [];
  if (!row || typeof row !== "object") return [`${strategy}:route88_row_missing`];
  for (const field of ROUTE88_FIELDS) if (!(field in row)) issues.push(`${strategy}:route88_field_missing:${field}`);
  for (const field of ROUTE88_NONEMPTY_FIELDS) {
    if (field in row && String(row[field] ?? "").trim() === "") issues.push(`${strategy}:route88_field_empty:${field}`);
  }
  for (const field of ROUTE88_COUNT_FIELDS) {
    if (field in row && (number(row[field]) === null || number(row[field]) < 0)) issues.push(`${strategy}:route88_field_invalid_count:${field}`);
  }
  for (const field of ROUTE88_BOOLEAN_FIELDS) if (field in row && typeof row[field] !== "boolean") issues.push(`${strategy}:route88_field_invalid_boolean:${field}`);
  return issues;
}
...
async function auditFourSurfaces({ baseUrl, runtimeDir, headers = {}, timeoutMs = 45000, dueByStrategy = {}, tradeDate }) {
  ...
  for (const [strategy, config] of Object.entries(STRATEGIES)) {
    ...
    if (due) issues.push(...route88FieldIssues(strategy, route88Row));
...
module.exports = { auditFourSurfaces, compare, route88FieldIssues, ROUTE88_FIELDS };
