...
function identity(payload) {
  ...
  return {
    tradeDate: compactDate(first(payload, ["tradeDate", "trade_date", "scanDate", "scan_date", "sourceDate", "usedDate", "marketDate", "payload.tradeDate"])),
...
function readReceipt(runtimeDir, strategy, tradeDate) {
  ...
  for (const pattern of STRATEGIES[strategy].receipts) {
    ...
    try { const payload = JSON.parse(fs.readFileSync(file, "utf8")); return { file, payload, ...identity(payload), complete: payload.complete === true && String(payload.status || "").toLowerCase() === "complete", fallbackUsed: payload.fallback === true || payload.fallbackUsed === true }; } catch {}
  }
...
function compare(strategy, receipt, surfaces, due) {
  const issues = [];
...
async function auditFourSurfaces({ baseUrl, runtimeDir, headers = {}, timeoutMs = 45000, dueByStrategy = {}, tradeDate }) {
  ...
  const scorecard = await request(baseUrl, "/api/scorecard", headers, timeoutMs);
  const blockers = [];
  ...
  for (const [strategy, config] of Object.entries(STRATEGIES)) {
    ...
    issues.push(...compare(strategy, receipt, surfaces, due));
...
module.exports = { auditFourSurfaces, ROUTE88_FIELDS };
