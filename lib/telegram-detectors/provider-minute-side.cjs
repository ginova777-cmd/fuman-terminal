...
function build({trade_date,stock_id,trades,journal,as_of,volume_unit}){
 ...
 const rows=[],gaps=[],grouped=new Map(),seen=new Map();let cumulative=0;
 ...
 for(const x of sorted){
  ...
  if(x.volume!==cumulative+x.size)throw Error('INCOMPLETE_NATIVE_TRADE_SEQUENCE');
  const key=Math.floor(t/60000)*60000;let g=grouped.get(key);if(!g){g={before:cumulative,last:x,volume:0,count:0};grouped.set(key,g);}g.volume+=x.size;g.last=x;g.count++;cumulative=x.volume;
 }
