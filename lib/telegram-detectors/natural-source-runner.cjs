...
function produce({runtimeRoot,now=new Date().toISOString(),previousOutside={}}){
 ...
 for(const [symbol,current]of group){
  ...
  for(const [row,type,yes]of [[v,'VOLUME_ANOMALY_EVENT',v?.volume_anomaly_event],[p,'PRICE_UP_ANOMALY_EVENT',p?.price_up_anomaly_event],[o,'RAW_OUTSIDE_STRENGTH_EVENT',o?.raw_event],[o,'DYNAMIC_OUTSIDE_STRENGTH_EVENT',o?.dynamic_event]])if(yes)events.push({...row,event_type:type,source_event_at:type.includes('OUTSIDE')?sideRows.at(-1).side_volume_timestamp:eventAt});
