...
for(let i=0;i<33;i++){
 ...
 candles.push({symbol,market:'TSE',tradeDate:date,candleTime:new Date(time).toISOString(),candleSeenAt:new Date(evt).toISOString(),synthetic:false,websocketRow:true,restRepairRow:false,sourceChannel:'candles',volumeStrategyUsable:true,intradayOddLot:false,open:100+i,high:100+i,low:100+i,close:100+i,volume:size});
...
write(path.join(root,'cache/intraday/fugle-daytrade-ws-quotes-v2.json'),{quotes:[{code:symbol,totalVolumeSourceEventAt:now,prevClose:99}]});assert.equal(produce({runtimeRoot:root,now}).events.length,3);
...
write(path.join(root,'cache/intraday/fugle-daytrade-ws-candles-v2.json'),{candles:candles.map(x=>({...x,restRepairRow:true}))});assert.equal(produce({runtimeRoot:root,now}).proof.complete,false);
