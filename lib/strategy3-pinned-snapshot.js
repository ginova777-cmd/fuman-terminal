...
function readPinnedSnapshot(tradeDate,identity,root=process.env.FUMAN_RUNTIME_DIR||'C:/fuman-runtime'){
 ...
 if(typeof identity.generation!=='string'||!identity.generation.trim()||identity.trade_date!==tradeDate||!Number.isInteger(identity.snapshot_sequence)||identity.snapshot_sequence<1)throw Error('strategy3_pinned_snapshot_identity_invalid');
 ...
 if(!checked.ok||fields.some(k=>!same(k,checked.snapshot[k],identity[k]))||hash(checked.symbols)!==identity.symbols_sha256)throw Error('strategy3_pinned_snapshot_evidence_mismatch');
 return checked;
}
function verifySnapshotRows(rows,identity){
 const symbols=rows.map(r=>String(r.symbol));
 const bad=!identity||typeof identity.generation!=='string'||!identity.generation.trim()||rows.length===0||new Set(symbols).size!==rows.length||hash(symbols)!==identity.symbols_sha256||rows.some(row=>fields.some(k=>!same(k,row[k],identity[k]))||row.contract_version!=='4.1.0'||row.complete!==true||row.status!=='complete'||row.membership_status==='REMOVED');
