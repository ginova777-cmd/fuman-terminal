...
// Existing higher-priority rows may retain their rank without a second boost.
function validMorningBoostRank(boost) {
  const rank = boost?.applied_priority_rank;
  return Number.isInteger(rank) && rank > 0 && (rank >= 41 || (
    boost.duplicate_boost_skipped === true && boost.boost_once === true &&
    boost.boost === 0 && boost.previous_priority_rank === rank && boost.existed_before_handoff === true
  ));
}
module.exports={preserveMorningWatchRows,validMorningBoostRank};
