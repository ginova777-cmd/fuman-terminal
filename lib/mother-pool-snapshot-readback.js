...
function verifySnapshotReadback(snapshot,rows,context={}) {
  ...
  if(typeof snapshot.generation!=='string'||!snapshot.generation.trim()||snapshot.generation!==snapshot.run_id)failed.push('snapshot_generation_invalid');
  ...
  const seen=new Set();
  const scalar=['generation','contract','contract_version','trade_date','canonical_run_id','snapshot_sequence','snapshot_type','status','complete','symbol_count','exit_code'];
  ...
  return {contract:'mother_pool_snapshot_anon_readback_v1',trade_date:snapshot.trade_date,
    canonical_run_id:snapshot.canonical_run_id,mother_pool_run_id:snapshot.run_id,generation:snapshot.generation,snapshot_sequence:snapshot.snapshot_sequence,
