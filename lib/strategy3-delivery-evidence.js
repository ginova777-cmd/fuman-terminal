...
function verifyDelivery({ scan, tri, collection, current, line, date, ui }) {
  ...
  const lineException = require("./strategy3-line-exception").valid(line, scan, date);
  if (!lineException) {
  ...
  return { ok: !issues.length, issues, firstBlocker: issues[0] || null, lineStatus: lineException ? "SKIPPED_QUOTA_EXHAUSTED" : (line?.status || "missing"), lineException };
}
