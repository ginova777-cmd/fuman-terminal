...
function buildFormalEvidence(input = {}) {
  const payloadEvidence = input.payload && typeof input.payload === "object" && input.payload.evidence && typeof input.payload.evidence === "object"
    ? input.payload.evidence
    : {};
  const directEvidence = input.evidence && typeof input.evidence === "object" ? input.evidence : {};
  const evidence = { ...payloadEvidence, ...directEvidence };
  const fieldNames = [
    "source_status", "sourceStatus", "gate_grade", "gateGrade", "canonical_gate_grade", "canonicalGateGrade",
    "gate_status", "gateStatus", "canonical_gate_status", "canonicalGateStatus",
    "formal_entry_allowed", "formalEntryAllowed", "formal_entry_speed_verdict", "formalEntrySpeedVerdict",
    "run_id", "runId", "trade_date", "tradeDate",
  ];
  for (const field of fieldNames) {
    if (evidence[field] === undefined && input[field] !== undefined) evidence[field] = input[field];
  }
  for (const nestedKey of ["canonical_gate", "canonicalGate"]) {
    const nested = evidence[nestedKey];
    if (!nested || typeof nested !== "object") continue;
    for (const field of fieldNames) {
      if (evidence[field] === undefined && nested[field] !== undefined) evidence[field] = nested[field];
    }
  }
  return evidence;
}

function validateFormalSourceEvidence(row) {
  const source = text(row.source).toLowerCase();
  const formalSource = source.includes("fugle") || source.includes("daytrade") || source.includes("live_detected");
  if (!formalSource) return [];
  const evidence = row.evidence && typeof row.evidence === "object" ? row.evidence : {};
  const sourceStatus = text(evidence.source_status ?? evidence.sourceStatus).toLowerCase();
  const gateGrade = text(evidence.gate_grade ?? evidence.gateGrade ?? evidence.canonical_gate_grade ?? evidence.canonicalGateGrade).toUpperCase();
  ...
  const formalAllowed = booleanOrNull(evidence.formal_entry_allowed ?? evidence.formalEntryAllowed);
  const missing = [];
  if (!sourceStatus) missing.push("source_status");
  if (!gateGrade) missing.push("gate_grade");
  if (!gateStatus) missing.push("gate_status");
  if (!verdict) missing.push("formal_entry_speed_verdict");
  if (formalAllowed === null) missing.push("formal_entry_allowed");
  const issues = [];
  if (missing.length) issues.push(`formal_source_evidence_missing:${missing.join("|")}`);
  if (sourceStatus && !["ok", "ready"].includes(sourceStatus)) issues.push(`source_status_not_ready:${sourceStatus}`);
  if (gateGrade && gateGrade !== "A") issues.push(`gate_grade_not_a:${gateGrade}`);
  if (gateStatus && gateStatus !== "ready") issues.push(`gate_status_not_ready:${gateStatus}`);
  if (verdict && verdict !== "YES") issues.push(`formal_entry_speed_verdict_not_yes:${verdict}`);
  if (formalAllowed === false) issues.push("formal_entry_not_allowed");
...
function normalizeRow(input = {}, options = {}) {
  ...
  return {
    ...
    evidence: buildFormalEvidence(input),
...
function validateRow(row, options = {}) {
  ...
  issues.push(...validateFormalSourceEvidence(row));
