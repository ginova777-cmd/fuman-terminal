...
function normalizeRow(input = {}, options = {}) {
  ...
  return {
    ...
    detect_time: normalizeTime(input.detect_time ?? input.detectTime ?? input.entry_time ?? input.entryTime ?? input.time ?? input.updated_at ?? input.updatedAt),
    ...
    strategy: text(input.strategy ?? input.strategy_label ?? input.strategyLabel ?? "七策略"),
...
function rowHasReplay(row) {
  return [row.source, row.strategy, row.signal_type].some((value) => text(value).toLowerCase().includes("replay"));
}
