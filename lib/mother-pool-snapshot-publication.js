...
async function publishWithRecovery({tradeDate,canonicalRunId,loadIntent,saveIntent,build,send,readback,commit,validate}) {
  async function publish(snapshot) {
    if(typeof snapshot.generation!=='string' || !snapshot.generation.trim() || snapshot.generation!==snapshot.run_id || snapshot.trade_date!==tradeDate || snapshot.canonical_run_id!==canonicalRunId || !validate(snapshot)) {
    ...
    if(evidence?.complete!==true || evidence.status!=='complete' || evidence.exit_code!==0
       ...
       || evidence.generation!==snapshot.generation || evidence.snapshot_sequence!==snapshot.snapshot_sequence)throw Error('MOTHER_POOL_SNAPSHOT_READBACK_FAILED');
