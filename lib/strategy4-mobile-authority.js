...
function strategy4MobileAuthority(payload, today) {
  ...
  const valid = /^\d{8}$/.test(day)
    ...
    && require("./strategy4-v4-evidence").strategy4V4Issues(p, rows).length === 0
    ...
    && Number(p.count) === rows.length && rows.length === count
  ...
  return { key: "strategy4", runId, tradeDate: day, sourceDate: day,
    ...
    displayBlockReason: "", pendingNotDue: false, evidenceStatus: "complete", publishAllowed: true, preservePreviousGood: false, fallback: false };
}
