...
const { readSecretText, serverSupabaseKey, serviceRoleKey, terminalSupabaseKey, terminalSupabaseUrl } = require("./server-supabase-key");
...
const DEFAULT_WRITE_ATTEMPTS = Math.max(1, Number(process.env.FUMAN_SNAPSHOT_WRITE_ATTEMPTS || 3) || 1);
...
function supabaseUrl() {
  return String(
    ...
    || readSecretText(path.join(RUNTIME_DIR, "secrets", "terminal-supabase-url.txt"))
    || readSecretText(path.join(ROOT, "secrets", "supabase-url.txt"))
    || readSecretText(path.join(RUNTIME_DIR, "secrets", "supabase-url.txt"))
    || terminalSupabaseUrl({ root: ROOT, runtimeDir: RUNTIME_DIR })
  ).replace(/\/+$/, "");
}

function readKey() {
  return process.env.FUMAN_SNAPSHOT_SUPABASE_KEY
    || process.env.DESKTOP_SNAPSHOT_SUPABASE_KEY
    || process.env.FUMAN_SNAPSHOT_SUPABASE_SERVICE_ROLE_KEY
    ...
    || terminalSupabaseKey({ root: ROOT, runtimeDir: RUNTIME_DIR })
...
async function readSnapshot(snapshotKey, options = {}) {
  ...
  if (SNAPSHOT_TABLE === "market_snapshots") {
    ...
    const maxAttempts = Math.max(1, Math.min(3, Number(options.maxAttempts || 3) || 1));
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      try {
        const rows = await fetchRows(query, key, options.timeoutMs || DEFAULT_TIMEOUT_MS);
        const snapshot = normalizeSnapshotRow(Array.isArray(rows) ? rows[0] : null);
...
async function upsertSnapshot(snapshotKey, payload, options = {}) {
  ...
  if (!supabaseUrl() || !key || !snapshotKey || !payload || typeof payload !== "object") {
    return {
      ok: false,
      skipped: true,
      reason: !supabaseUrl() ? "snapshot_supabase_url_missing" : !key ? "snapshot_write_key_missing" : "snapshot_write_payload_invalid",
      key: snapshotKey,
    };
  }
  ...
  if (SNAPSHOT_TABLE === "market_snapshots") {
    ...
    for (let attempt = 1; attempt <= DEFAULT_WRITE_ATTEMPTS; attempt += 1) {
      ...
      try {
        ...
        return { ok: true, key: snapshotKey, tradeDate, attempts: attempt };
      } catch (error) {
        lastError = error.message || String(error);
        if (attempt < DEFAULT_WRITE_ATTEMPTS) await sleep(1000 * attempt);
      } finally {
        timeout.clear();
      }
    }
    return { ok: false, error: lastError, key: snapshotKey, tradeDate, attempts: DEFAULT_WRITE_ATTEMPTS };
  }
  const row = {
    key: snapshotKey,
