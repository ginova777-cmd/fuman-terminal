...
const FUGLE_WS_QUOTES_FILE = process.env.FUGLE_WS_QUOTES_FILE || path.join(CACHE_DIR, "intraday", `${CACHE_STEM}-quotes.json`);
const FUGLE_WS_CANDLES_FILE = process.env.FUGLE_WS_CANDLES_FILE || path.join(CACHE_DIR, "intraday", `${CACHE_STEM}-candles.json`);
const FUGLE_WS_STATUS_FILE = process.env.FUGLE_WS_STATUS_FILE || path.join(STATE_DIR, DAYTRADE_CACHE ? "fugle-daytrade-websocket-status.json" : "fugle-websocket-status.json");
