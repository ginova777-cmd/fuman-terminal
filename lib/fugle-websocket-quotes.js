...
function normalizeFugleCandle(payload) {
  ...
  // Missing/invalid volume is not a natural zero-volume observation.
  const rawVolume = data.volume;
  const parsedVolume = ['number', 'string'].includes(typeof rawVolume)
    && String(rawVolume).trim() !== '' ? Number(rawVolume) : NaN;
