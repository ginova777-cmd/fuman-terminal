...
function hasProductionVerificationAccess(request, scope) {
  if (scope !== "mobile-boot" && !String(scope || "").startsWith("mobile-fragment:")) return false;
  const expected = String(process.env.FUMAN_PRODUCTION_VERIFY_TOKEN || "").trim();
  const provided = String(getHeader(request, "x-fuman-production-verify") || "").trim();
...
async function verifyRequestEntitlement(request, options = {}) {
  ...
  if (hasProductionVerificationAccess(request, scope)) {
    return { ok: true, scope, productionVerify: true, user: { email: "production-verify@fuman.local" }, access: { status: "production_verify" } };
  }
