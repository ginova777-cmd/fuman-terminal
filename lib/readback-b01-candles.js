...
async function readbackCandles(expected,{readRpc,deadlineMs,now=Date.now}={}) {
  ...
  if(expected.items.some(x=>!x||typeof x.symbol!=='string'||!/^\d{4}$/.test(x.symbol)||!['READY','DATA_GAP'].includes(x.status)))throw Error('B01_EXPECTED_ITEMS_INVALID');
  ...
  for(let offset=0;offset<symbols.length;offset+=100){
    ...
    try{
      ...
      if(!Array.isArray(result)||result.length>selected.length*3||result.some(x=>!x||!selected.includes(x.symbol)))throw Error('INVALID_RESPONSE');
      rows.push(...result);
  ...
  if(Date.parse(readbackAt)>deadlineMs)errors.push('B01_READBACK_BUDGET_EXHAUSTED');
