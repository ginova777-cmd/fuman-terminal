...
function completeHours(rows, signalDate) {
  ...
  for(const r of rows){
    ...
    // Only four full 60-minute bars; do not label the 13:00-13:30 tail as 60m.
    if(day>signalDate||h<9||h>12)continue;
    const key=day+"T"+String(h).padStart(2,"0"),g=groups.get(key)||new Map();g.set(m,r);groups.set(key,g);
  }
  return [...groups].sort(([a],[b])=>a.localeCompare(b)).filter(([,g])=>g.size===60).map(([key,g])=>{
    const b=[...g].sort(([a],[b])=>a-b).map(([,r])=>r);
    return {date:key.slice(0,10),timestamp:key,open:num(b[0].open),high:Math.max(...b.map(r=>num(r.high))),low:Math.min(...b.map(r=>num(r.low))),close:num(b.at(-1).close)};
  });
}
