...
function statusLooksReady(value) {
  return /^(complete|ok|ready|pass|passed|healthy)$/i.test(String(value || "").trim());
}

function realtimeRadarAfterCloseReady(payload = {}, now = new Date()) {
  if (isMarketDetectionWindow(now)) return false;
  if (payloadTradeDate(payload) !== taipeiDateKey(now)) return false;
  ...
  const staleQuoteCount = firstFiniteNumber(
    payload.staleQuoteCount,
    payload.stale_quote_count,
    runQuality.staleQuoteCount,
    runQuality.stale_quote_count
  );
  const failedBatchCount = firstFiniteNumber(
    payload.failedBatchCount,
    payload.failed_batch_count,
    sourceStatus.failedBatchCount,
    sourceStatus.failed_batch_count
  );
  const qualityReady = statusLooksReady(payload.evidenceStatus)
    || statusLooksReady(runQuality.status)
    || statusLooksReady(sourceStatus.status)
    || runQuality.ready === true
    || sourceStatus.ready === true
    || cleanNumber(runQuality.rowsChecked) > 0;
  return Boolean(
    rowCount > 0
    && (failedBatchCount === null || failedBatchCount === 0)
    && (staleQuoteCount === null || staleQuoteCount === 0)
    && qualityReady
  );
}
...
function isRealtimeRadarSnapshotStale(payload, nowMs = Date.now()) {
  ...
  if (!isMarketDetectionWindow(now)
    && payloadTradeDate(payload) === taipeiDateKey(now)
    && (realtimeRadarCoverageReady(payload) || realtimeRadarAfterCloseReady(payload, now))) {
    return false;
  }
