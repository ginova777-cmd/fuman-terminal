...
function ensureWarmup({runtime,root,tradeDate,symbols,apply,now=new Date()}){
 ...
 const launchFile=path.join(dir,'launch.json');try{const x=read(launchFile);previousLaunch=x;if(Date.now()-Date.parse(x.started_at)<180000)return {started:false,reason:'LAUNCH_COOLDOWN'};if(x.pid){try{process.kill(x.pid,0);return {started:false,reason:'WARMUP_RUNNING'};}catch{}}
   // A launch count alone is not evidence that A16 completed. Failed or
   // incomplete warmups must remain retryable during preopen; a verified
   // writer-summary is terminal (handled by readReferences above).
   if(Number(x.attempts)>=2 && Date.now()-Date.parse(x.started_at)<1800000)return {started:false,reason:'RETRY_BACKOFF'};
 }catch{}
