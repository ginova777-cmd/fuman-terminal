...
const CONTRACT='institution-candidate90-daily-last60-up-v1',MIN_COVERAGE=.9;
...
async function readTechnicalSources(candidates,tradeDate){
 const url=secret('supabase-url.txt'),key=secret('supabase-service-role-key.txt'),fugleKey=secret('fugle-api-key.txt');
...
function evaluateCandidates(candidates,sources,tradeDate,extraIssues={}){const selected=[],missing=[],notBullish=[],ready=[];for(const row of candidates){const s=sources[row.code]||{errors:['technical_source_missing']},daily=timeframeEvidence(s.daily,tradeDate),hourly60=timeframeEvidence(s.hourly60,tradeDate,true);const reasons=[...(extraIssues[row.code]||[]),...(s.errors||[])];for(const f of ['code','name','market','foreign','trust','dealer','total','foreignStreak','trustStreak','jointStreak'])if(row[f]===null||row[f]===undefined||String(row[f]).trim()==='')reasons.push('missing_'+f);if(!(row.close>0&&row.tradeVolume>0&&row.fiveDayAvgVolume>0))reasons.push('required_trading_fields_missing');if(!daily.available)reasons.push('daily_'+daily.reason);if(!hourly60.available)reasons.push('hourly60_'+hourly60.reason);
 if(reasons.length){missing.push({code:row.code,reasons:[...new Set(reasons)]});continue;}ready.push(row.code);const technicalTrend={contract:CONTRACT,indicatorContract:INDICATOR_CONTRACT,daily,hourly60,pass:daily.trendUp&&hourly60.trendUp};if(!technicalTrend.pass){notBullish.push(row.code);continue;}selected.push({...row,technicalTrend});}
