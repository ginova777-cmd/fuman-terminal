...
function recoverDeliveryHistory({records,targets,runId,date,messages}) {
 ...
 for(const target of targets){
  ...
  const found=records.filter(r=>r.channel==='line'&&r.target===target&&r.idempotencyKey===key&&r.payloadHash===payloadHash&&day(r.recordedAt)===date&&['sent','failed'].includes(r.status)).at(-1);
