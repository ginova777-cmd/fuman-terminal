...
const DISPLAY_ARRAY_FIELDS = [
  "rows", "records", "entries", "matches", "events", "items", "data", "signals", "results",
  "volumeMatches", "singleSignals", "list",
];

function payloadDate(payload = {}) {
  const direct = [
    payload.tradeDate, payload.trade_date, payload.sourceDate, payload.source_date,
    payload.usedDate, payload.used_date, payload.latestDate, payload.latest_date,
    payload.marketDate, payload.market_date,
  ];
  for (const value of direct) {
    const normalized = compactDate(value);
    if (normalized) return normalized;
  }
  const runId = String(payload.runId || payload.run_id || payload.latestRunId || payload.latest_run_id || "");
  const match = runId.match(/(?:^|[-_])(20\d{6})(?:[-_]|$)/);
  return match ? match[1] : "";
}

function suppressStaleDisplayPayload(payload, expectedDate) {
  if (!payload || typeof payload !== "object" || !expectedDate) return payload;
  const actualDate = payloadDate(payload);
  const staleNested = [];
  const stripStaleNested = (value, fieldPath = "payload") => {
    if (Array.isArray(value)) {
      return value.filter((item, index) => {
        const itemDate = payloadDate(item);
        if (itemDate && itemDate !== expectedDate) {
          staleNested.push({ field: `${fieldPath}[${index}]`, date: itemDate, runId: item?.runId || item?.run_id || "" });
          return false;
        }
        return true;
      }).map((item, index) => stripStaleNested(item, `${fieldPath}[${index}]`));
    }
    if (!value || typeof value !== "object") return value;
    const copy = { ...value };
    for (const [key, child] of Object.entries(copy)) {
      if (key === "marketCalendar") continue;
      copy[key] = stripStaleNested(child, `${fieldPath}.${key}`);
    }
    return copy;
  };
  if (!actualDate || actualDate === expectedDate) {
    const nestedClean = stripStaleNested(payload);
    if (!staleNested.length) return payload;
    const nestedReason = `nested_payload_date_mismatch:${[...new Set(staleNested.map((row) => row.date))].join(",")}!=${expectedDate}`;
    return {
      ...nestedClean,
      ok: false,
      qualityStatus: "blocked_stale_date",
      evidenceStatus: "insufficient",
      unattendedStatus: "NO",
      publishAllowed: false,
      preservePreviousGood: true,
      staleDataSuppressed: true,
      staleNestedRowsSuppressed: staleNested.length,
      staleNestedRunIds: staleNested.map((row) => row.runId).filter(Boolean),
      dateGateStatus: "blocked",
      dateGateReason: nestedReason,
      blockedReason: nestedReason,
      scanner_block_reason: nestedReason,
      displayTradeDate: expectedDate,
    };
  }
  const reason = `payload_date_mismatch:${actualDate}!=${expectedDate}`;
  const gated = {
    ...payload,
    ok: false,
    qualityStatus: "blocked_stale_date",
    ...
    staleDataSuppressed: true,
    dateGateStatus: "blocked",
    dateGateReason: reason,
    blockedReason: reason,
    scanner_block_reason: reason,
    displayTradeDate: expectedDate,
    staleRunId: payload.runId || payload.run_id || payload.latestRunId || payload.latest_run_id || "",
    count: 0,
    returnedCount: 0,
    resultCount: 0,
    readbackCount: 0,
  };
  for (const field of DISPLAY_ARRAY_FIELDS) {
    if (Array.isArray(gated[field])) gated[field] = [];
  }
  return gated;
}
...
function attachMarketCalendar(payload, marketCalendar) {
  ...
  const gatedPayload = shouldPreserve
    ? suppressStaleDisplayPayload(payload, marketCalendar.displayTradeDate)
    : payload;
  return {
    ...gatedPayload,
    marketCalendar,
