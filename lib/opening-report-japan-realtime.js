...
function inWindow(ms, date) { return Number.isFinite(ms) && ms >= Date.parse(`${date}T08:00:00+08:00`) && ms <= Date.parse(`${date}T08:20:59.999+08:00`); }
...
function evidenceCheck(e, symbol, date) {
  ...
  if(!inWindow(quoteMs,date) || openMs>quoteMs) return "japan_realtime_quote_outside_0800_0820_window";
  if(!inWindow(fetchedMs,date) || quoteMs>fetchedMs) return "japan_realtime_capture_outside_0800_0820_window";
...
function parseQuote(html, symbol, date, fetchedAt) {
  const base={ok:false,source:SOURCE,source_url:`https://finance.yahoo.co.jp/quote/${symbol}`,ticker:symbol,source_provider:PROVIDER,source_fields:[...FIELDS],session_contract:"08:00-08:20 Asia/Taipei"};
...
async function snapshot(leader,date) {
  ...
  if(!cache.has(key)) cache.set(key,(async()=>{
    ...
    if(!inWindow(Date.now(),date)) return {...base,reason_code:"japan_realtime_capture_outside_0800_0820_window"};
    const attempts=[];
