...
function expectedUniverse(snapshot, resource, fallbackResource) {
  return firstMetric(resource, [
    ...
    ?? firstMetric(fallbackResource || {}, ["expected", "active_symbols", "activeSymbols", "sourceUniverseCount"])
...
function auditRunTimeSourceSnapshotQuality(payload, options = {}) {
  ...
  if (resourceIsRequired(preopenDaily)) {
    const daily = isObject(preopenDaily.dailyVolume) ? preopenDaily.dailyVolume : {};
...
function attachRunTimeSourceEvidence(payload, options = {}) {
  ...
  return {
    ...
    unattended: {
      ...
      evidenceStatus,
      reason: audit.ok && qualityAudit.ok ? "run_time_source_snapshot_complete" : quality.reason,
    },
