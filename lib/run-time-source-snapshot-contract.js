...
function auditRunTimeSourceSnapshotQuality(payload, options = {}) {
  ...
  if (resourceIsRequired(intraday)) {
    ...
    const staleLimit = firstMetric(intraday, ["max_stale_seconds", "maxStaleSeconds", "allowed_stale_seconds", "allowedStaleSeconds"]) ?? thresholds.intraday1mStaleSeconds;
    if (stale === null) issues.push("intraday_1m_readiness_at_run_missing_stale_seconds");
    else if (stale > staleLimit) issues.push(`intraday_1m_readiness_at_run_stale_seconds_${stale}_above_${staleLimit}`);
  }
  ...
  const formalFallbackUsed = formalSourceFallbackUsed(quality);
  if (quality.publishAllowed === false) issues.push("run_quality_publishAllowed_false");
  if (formalFallbackUsed) issues.push("run_quality_fallback_used_not_formal_publishable");
  if (formalFallbackUsed && quality.fallbackAllowed !== true) issues.push("run_quality_unallowed_fallback_used");
  const blankCounts = isObject(quality.blankCounts) ? quality.blankCounts : {};
  for (const [key, value] of Object.entries(blankCounts)) {
    const count = cleanNumber(value, 0);
    if (count > 0) issues.push(`run_quality_blankCounts_${key}_${count}`);
  }
...
function formalSourceFallbackUsed(quality = {}) {
  if (!isObject(quality)) return false;
  if (quality.formalSourceFallbackUsed === true) return true;
  const scope = Array.isArray(quality.fallbackScope) ? quality.fallbackScope : [];
  if (scope.some((item) => /^(source|formal[-_]?source|formal_source_chain)$/i.test(String(item || "").trim()))) return true;
  const details = Array.isArray(quality.fallbackDetails) ? quality.fallbackDetails : [];
  return details.some((item) => {
    if (!isObject(item)) return false;
    if (item.formalSource === true) return true;
    return /^(source|formal[-_]?source|formal_source_chain)$/i.test(String(item.scope || item.source || "").trim());
  });
}
...
function fallbackDisclosure(payload = {}, quality = {}) {
  ...
  const fallbackContract = firstNonBlank(payload.fallbackContract, payload.fallback_contract, quality.fallbackContract)
    || (fallbackUsed ? "fallback-disclosure-required" : "fallback-not-used");
  return { fallbackUsed, fallbackScope, fallbackAllowed, fallbackDetails, fallbackContract };
}
...
function attachRunTimeSourceEvidence(payload, options = {}) {
  ...
  const insufficientQuality = {
    ...
    fallbackContract: fallback.fallbackContract,
    reason: "run_time_source_snapshot_missing_or_incomplete",
    blockedReason: firstNonBlank(existingQuality.blockedReason, existingQuality.scanner_block_reason, "run_time_source_snapshot_missing_or_incomplete"),
    scanner_block_reason: firstNonBlank(existingQuality.scanner_block_reason, existingQuality.blockedReason, "run_time_source_snapshot_missing_or_incomplete"),
  };
  const sourceEvidenceIssues = audit.ok
    ? qualityAudit.issues
    : [`run_time_source_snapshot_insufficient:${audit.missingFields.join(",")}`];
  const evidenceStatus = audit.ok && !qualityAudit.ok && options.evidenceStatusOnQualityFail
    ? String(options.evidenceStatusOnQualityFail)
    : audit.ok ? qualityAudit.status : audit.status;
  ...
  const quality = audit.ok && qualityAudit.ok ? {
    ...
    blockedReason: firstNonBlank(
      existingQuality.blockedReason,
      existingQuality.scanner_block_reason,
      audit.ok ? `run_time_source_snapshot_quality_failed:${qualityAudit.issues.join(";")}` : "run_time_source_snapshot_missing_or_incomplete"
    ),
    scanner_block_reason: firstNonBlank(
      existingQuality.scanner_block_reason,
      existingQuality.blockedReason,
      audit.ok ? `run_time_source_snapshot_quality_failed:${qualityAudit.issues.join(";")}` : "run_time_source_snapshot_missing_or_incomplete"
    ),
  ...
  const computedUnattendedStatus = audit.ok
    ...
    && !formalSourceFallbackUsed(quality)
    ? "YES"
    : "NO";
  ...
  const explicitUnattendedText = explicitUnattendedStatus ? String(explicitUnattendedStatus) : "";
  const explicitYesBlockedByComputedNo = explicitUnattendedText.toUpperCase() === "YES" && computedUnattendedStatus !== "YES";
  const unattendedStatus = explicitUnattendedText && !explicitYesBlockedByComputedNo
    ? explicitUnattendedText
    : computedUnattendedStatus;
  const unattendedReason = explicitYesBlockedByComputedNo
    ? (quality.reason || "payload_unattended_yes_blocked_by_source_quality")
    : explicitUnattendedStatus
    ? (payloadUnattended.reason || (unattendedStatus === "YES" ? "payload_unattended_status_explicit" : quality.reason || "payload_unattended_yes_blocked_by_source_quality"))
  ...
  return {
    ...
    run_quality_at_publish: quality,
    fallbackContract: quality.fallbackContract || fallback.fallbackContract,
    blockedReason: quality.blockedReason || "",
    scanner_block_reason: quality.scanner_block_reason || "",
    evidenceStatus,
    sourceEvidenceStatus: qualityAudit.status || evidenceStatus,
    ...
    publishAllowed: audit.ok && qualityAudit.ok && !formalSourceFallbackUsed(quality) ? quality.publishAllowed !== false : false,
    degradedBlocksLatest: audit.ok && qualityAudit.ok ? quality.degradedBlocksLatest === true || formalSourceFallbackUsed(quality) : true,
    preservePreviousGood: audit.ok && qualityAudit.ok ? quality.preservePreviousGood === true || formalSourceFallbackUsed(quality) : true,
...
function buildRunQualityAtPublish({
  ...
  return {
    ...
    fallbackAllowed: fallback.fallbackAllowed,
    fallbackDetails: fallback.fallbackDetails,
    fallbackContract: fallbackContract || fallback.fallbackContract,
    ...
    blockedReason: blockedReason || payload.blockedReason || payload.blocked_reason || "",
    scanner_block_reason: scannerBlockReason || payload.scanner_block_reason || payload.scannerBlockReason || blockedReason || "",
  };
}
