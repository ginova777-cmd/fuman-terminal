...
function auditRunTimeSourceSnapshotQuality(payload, options = {}) {
  ...
  if (quality.fallbackUsed === true) issues.push("run_quality_fallback_used_not_formal_publishable");
...
function attachRunTimeSourceEvidence(payload, options = {}) {
  ...
  return {
    ...
    publishAllowed: audit.ok && qualityAudit.ok && quality.fallbackUsed !== true ? quality.publishAllowed !== false : false,
    degradedBlocksLatest: audit.ok && qualityAudit.ok ? quality.degradedBlocksLatest === true || quality.fallbackUsed === true : true,
    preservePreviousGood: audit.ok && qualityAudit.ok ? quality.preservePreviousGood === true || quality.fallbackUsed === true : true,
