...
function normalizeEntry(input = {}, options = {}) {
  ...
  return {
    ...
    signal_type: text(input.signal_type || input.signalType || "formal"),
...
function validateEntry(entry, options = {}) {
  ...
  if (text(entry.signal_type || "formal").toLowerCase() !== "formal") issues.push(`signal_type_not_formal:${entry.signal_type || "missing"}`);
