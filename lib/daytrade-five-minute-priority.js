...
function coverageOnlyReceipt(receipt) {
  const failures = receipt?.failed_checks;
  const coverageFailure = value => typeof value === "string" && /^(?:latest_view_)?effective_coverage_below_\d+pct:\d+\/\d+$/.test(value);
  return receipt?.status === "blocked" && receipt.complete === false && receipt.exit_code === 1 &&
    receipt.anon_http_status === 200 && receipt.ssl_ok === true &&
    Array.isArray(failures) && failures.length > 0 && failures.every(coverageFailure) &&
    coverageFailure(receipt.first_blocker) && failures.includes(receipt.first_blocker) &&
    Array.isArray(receipt.diagnostic_summary?.source_errors) && receipt.diagnostic_summary.source_errors.length === 0;
}
...
function applyFiveMinutePriority(pool, bars, receipt, snapshot, now = Date.now()) {
  ...
  const evidence = { contract: "daytrade_intraday_5m_priority_v1", trade_date: tradeDate || null,
    ...
    mother_pool_run_id: snapshot?.runId || null, status: "unavailable", role: "bonus_only", blocks_mother_pool: false, coverage_role: "diagnostic_only", coverage_warnings: [], promoted_symbols: [],
  ...
  const fullyVerified = receipt.complete === true && receipt.status === "complete" && receipt.exit_code === 0 && !receipt.first_blocker;
  if (!fullyVerified && !coverageOnlyReceipt(receipt)) return finish("five_minute_receipt_not_verified");
  if (!fullyVerified) evidence.coverage_warnings = receipt.failed_checks.slice();
...
module.exports = { applyFiveMinutePriority, intradayWindow, coverageOnlyReceipt };

function verifyPriorityPublication(expected, actual) {
  const failed = [], evidence = expected?.fiveMinutePriorityEvidence;
  const noBonus = evidence?.role === "bonus_only" && evidence.blocks_mother_pool === false &&
    ["unavailable", "not_due"].includes(evidence.status) && Array.isArray(evidence.promoted_symbols) && evidence.promoted_symbols.length === 0;
  if (!evidence || (evidence.status !== "evaluated" && !noBonus)) failed.push(evidence?.first_blocker || "priority_not_evaluated");
  ...
  return { contract: "daytrade_intraday_5m_priority_publication_receipt_v1", scope: "priority_artifact_readback_only",
    ...
    role: "bonus_only", bonus_status: evidence?.status || "unknown", bonus_unavailable_reason: noBonus ? evidence.first_blocker : null,
    coverage_warnings: evidence?.coverage_warnings || [], blocks_mother_pool: false,
