...
function deliveryPayload(mode,observations,night){return night ? {contract:"opening-report-delivery-v3",mode,observations,night_futures:night} : {contract:CONTRACT,mode,observations};}
function contentHash(mode,observations,night){return crypto.createHash("sha256").update(JSON.stringify(deliveryPayload(mode,observations,night))).digest("hex");}
