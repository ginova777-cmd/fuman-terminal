...
const MIN_AFTER1300_ROWS = Math.max(1, Number(process.env.STRATEGY3_TV_MIN_AFTER1300_ROWS || 5));
...
function candleQuality(candles) {
  const rows = (candles || []).filter((row) => cleanNumber(row.close) > 0);
  ...
  const after1300Rows = rows.filter((row) => {
    const minutes = candleMinutes(row);
    return minutes != null && minutes >= 13 * 60 && minutes <= 13 * 60 + 30;
  }).length;
...
function needsFullOhlcFallback(quality) {
  return cleanNumber(quality.rows) < MIN_FULL_ROWS
    ...
    || cleanNumber(quality.after1300Rows) < MIN_AFTER1300_ROWS;
}

function qualityReason(quality) {
  return `rows=${quality.rows}; degenerate=${quality.degenerateRows}/${quality.rows}; after1300=${quality.after1300Rows}`;
}

function betterQuality(candidate, current) {
  if (!candidate || !candidate.rows) return false;
  if (!current || !current.rows) return true;
  if (candidate.after1300Rows > current.after1300Rows && candidate.rows >= current.rows) return true;
...
async function fetchStrategy3TvCandles(code, limit = 160) {
  const symbol = normalizeCode(code);
  let supabase = null;
  try {
