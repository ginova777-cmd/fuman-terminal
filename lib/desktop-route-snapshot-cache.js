...
const DEFAULT_MEMORY_TTL_MS = Number(process.env.DESKTOP_ROUTE_SNAPSHOT_MEMORY_TTL_MS || process.env.FUMAN_DESKTOP_ROUTE_SNAPSHOT_MEMORY_TTL_MS || 60000);
const ROUTE_SNAPSHOT_KEYS = Object.freeze({
  strategy3: `${DESKTOP_ROUTE_SNAPSHOT_KEY}_strategy3`,
  strategy4: `${DESKTOP_ROUTE_SNAPSHOT_KEY}_strategy4`,
  strategy5: `${DESKTOP_ROUTE_SNAPSHOT_KEY}_strategy5`,
  institution: `${DESKTOP_ROUTE_SNAPSHOT_KEY}_institution`,
});

function routeSnapshotKey(route) {
  return ROUTE_SNAPSHOT_KEYS[String(route || "").trim().toLowerCase()] || "";
}

let memorySnapshot = null;
let memorySnapshotCachedAt = 0;
let pendingSnapshotRead = null;
...
async function readDesktopRouteSnapshot(options = {}) {
  const snapshot = await readDesktopRouteSnapshotRaw(options);
...
async function readDesktopRouteSnapshotForRoute(route, options = {}) {
  const key = routeSnapshotKey(route);
  if (!key) return null;
  ...
  return {
    ...
    payload: {
      ...snapshot.payload,
      source: "terminal-fast-bundle",
      cacheSource: "supabase:desktop_route_snapshot:route",
      snapshotHit: true,
      snapshotFresh: freshness.fresh,
      snapshotStale: freshness.stale,
      snapshotAgeMs: Number.isFinite(freshness.ageMs) ? freshness.ageMs : null,
      ...
      snapshot: {
        ...
        snapshotId: snapshot.snapshotId || "",
        updatedAt: snapshot.updatedAt || "",
        source: snapshot.source || "supabase:desktop_route_snapshot:route",
        fresh: freshness.fresh,
        stale: freshness.stale,
        ageMs: Number.isFinite(freshness.ageMs) ? freshness.ageMs : null,
        maxAgeMs: freshness.maxAgeMs,
...
async function readDesktopRouteSnapshotRaw(options = {}) {
  const memoryTtlMs = Math.max(0, Number(options.memoryTtlMs ?? DEFAULT_MEMORY_TTL_MS) || 0);
  if (memorySnapshot?.payload && (!memoryTtlMs || Date.now() - memorySnapshotCachedAt <= memoryTtlMs)) {
    return memorySnapshot;
  }
  if (pendingSnapshotRead) return pendingSnapshotRead;
  pendingSnapshotRead = readSnapshot(DESKTOP_ROUTE_SNAPSHOT_KEY, {
    allowLatestFallback: true,
    timeoutMs: Number(options.timeoutMs || DEFAULT_READ_TIMEOUT_MS),
  }).then((snapshot) => {
    if (snapshot?.payload && typeof snapshot.payload === "object") {
      memorySnapshot = snapshot;
      memorySnapshotCachedAt = Date.now();
    }
    return snapshot;
  }).finally(() => {
    pendingSnapshotRead = null;
  });
  return pendingSnapshotRead;
}

async function writeDesktopRouteSnapshot(payload, options = {}) {
  const result = await upsertSnapshot(DESKTOP_ROUTE_SNAPSHOT_KEY, payload, {
  ...
  if (result?.ok !== false) {
    memorySnapshot = null;
    memorySnapshotCachedAt = 0;
  }
  return result;
}

async function writeDesktopRouteSnapshotForRoute(route, payload, options = {}) {
  const key = routeSnapshotKey(route);
  if (!key || !payload || typeof payload !== "object") {
    return { ok: false, skipped: true, reason: "desktop_route_snapshot_route_invalid", key };
  }
  return upsertSnapshot(key, payload, {
    source: "desktop-route-snapshot-route",
    reason: options.reason || "desktop-route-precomputed-route",
    timeoutMs: options.timeoutMs || DEFAULT_WRITE_TIMEOUT_MS,
    snapshotId: options.snapshotId || payload.runId || "",
  });
}
...
module.exports = {
  ...
  readDesktopRouteSnapshotRaw,
  writeDesktopRouteSnapshot,
  writeDesktopRouteSnapshotForRoute,
