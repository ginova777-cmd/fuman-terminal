...
function evaluate(candidates, sources, tradeDate) {
  ...
  for (const row of candidates) {
    ...
    const daily = {...timeframeEvidence(source.daily, tradeDate), source:'supabase:'+DAILY_TABLE};
...
async function readSources(candidates, tradeDate) {
  return readTechnicalSources(candidates, tradeDate, { optionalHourly: true, dailyTable: DAILY_TABLE });
}
