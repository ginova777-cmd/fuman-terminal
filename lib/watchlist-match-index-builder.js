...
const SOURCES = [
  ...
  { key: "strategy2", label: "策略2-當沖雷達", endpoint: "/api/latest-strategy?key=strategy2", fields: ["events", "records", "matches", "rows"] },
...
function isPublishedDisplayHold(payload, meta, rows) {
  const cacheSource = String(meta?.cacheSource || meta?.source || payload?.cacheSource || payload?.source || "");
  const hasRows = Array.isArray(rows) && rows.length > 0;
  const ok = Boolean(payload) && payload?.ok !== false && meta?.ok !== false;
  const serverSideSource = /supabase|complete-run|published/i.test(cacheSource);
  const disallowedSource = /static|local|json|snapshot-soft-fallback/i.test(cacheSource);
  return ok && hasRows && serverSideSource && !disallowedSource;
}

function buildSourceEvidence(source, payload, rows, capturedAt) {
  const meta = payloadMeta(payload) || {};
  const fallbackMarker = hasFallbackMarker(payload, meta);
  const cacheSource = String(meta.cacheSource || meta.source || payload?.cacheSource || payload?.source || "");
  const ok = Boolean(payload) && payload?.ok !== false && meta?.ok !== false;
  const count = rows.length;
  const acceptedDisplayHold = fallbackMarker && isPublishedDisplayHold(payload, meta, rows);
  const fallbackUsed = fallbackMarker && !acceptedDisplayHold;
  ...
  const reason = !payload
    ...
    : !ok
      ...
      : fallbackUsed
        ...
        : acceptedDisplayHold
          ? "published_previous_good_accepted_for_watchlist_index"
