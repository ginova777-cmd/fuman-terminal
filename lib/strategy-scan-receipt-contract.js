...
function dateToken(value) {
  ...
  const compact = text.match(/(?:^|[^0-9])(20[0-9]{6})(?:[^0-9]|$)/);
  if (compact) return compact[1];
  const dashed = text.match(/(20[0-9]{2})-([0-9]{2})-([0-9]{2})/);
...
function normalizeStrategyScanReceipt(row, options = {}) {
  ...
  const receiptDateToken = dateToken(row.targetDate || row.tradeDate || row.expectedDate || row.startedAt || row.finishedAt || row.checkedAt || row.createdAt);
  const runIdDateToken = dateToken(runId);
  const runIdDateMismatch = Boolean(receiptDateToken && runIdDateToken && runIdDateToken !== receiptDateToken);
  const blocked = status !== "complete" || complete !== true || exitCode !== 0 || runIdDateMismatch || blockingTextIncludes(row, /blocked|preserve|previous good|source[_ -]?root[_ -]?not[_ -]?ready|not_ready|failed/);
  const preservePreviousGood = Boolean(row.preservePreviousGood === true || preservedLatest || row.publishBlocked === true || rawFallback || blocked);
  const candidatePublishAllowed = typeof row.publishAllowed === "boolean"
    ? row.publishAllowed
    : Boolean(status === "complete" && complete && exitCode === 0);
  const publishAllowed = Boolean(candidatePublishAllowed && status === "complete" && complete && exitCode === 0 && !rawFallback && !preservePreviousGood && !runIdDateMismatch);
  ...
  const normalizedStatus = publishAllowed ? "complete" : (preservePreviousGood ? "blocked_preserved" : (exitCode !== 0 ? "failed" : "blocked"));
  const normalizedComplete = publishAllowed === true;
  const normalizedQualityStatus = publishAllowed ? "complete" : (preservePreviousGood ? "preserved_latest" : (row.qualityStatus || "blocked"));
  const runQuality = {
    ...
    evidenceStatus,
    unattendedStatus,
    blockedReason: row.blockedReason || row.blockingReason || row.scanner_block_reason || (runIdDateMismatch ? `runId_date_mismatch:${runIdDateToken || "missing"}:receipt:${receiptDateToken}` : ""),
    scanner_block_reason: row.scanner_block_reason || row.blockingReason || row.blockedReason || (runIdDateMismatch ? `runId_date_mismatch:${runIdDateToken || "missing"}:receipt:${receiptDateToken}` : ""),
