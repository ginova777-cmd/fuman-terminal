...
const DOWNSTREAM_MODULES = Object.freeze([  { key: "power_recovery", label: "Power Outage Recovery", dueTime: "00:00", class: "recovery", source: "state/power-recovery.json", adapter: "power_recovery", command: { cwd: ".", executable: "node", args: ["scripts/verify-terminal-power-recovery.js"] }, verifier: "scripts/verify-terminal-power-recovery.js", allowedAction: "repair_or_register_start_when_available_final_audit_task_then_rerun" },
  ...
  { key: "mother_pool", label: "Dynamic Mother Pool", dueTime: "09:00", class: "pool", source: "state/daytrade-unattended-gate-watchdog.json", adapter: "watchdog_mother_pool", command: { cwd: ".", executable: "node", args: ["scripts/verify-daytrade-mother-pool-closed-loop.js"] }, verifier: "scripts/verify-daytrade-mother-pool-closed-loop.js", allowedAction: "restore_dynamic_mother_pool_closed_loop_then_rerun" },
  { key: "top40", label: "Priority Pool (TOP40 compatibility only)", dueTime: "09:00", class: "pool", source: "state/daytrade-unattended-gate-watchdog.json", adapter: "watchdog_top40", command: { cwd: ".", executable: "node", args: ["scripts/verify-daytrade-mother-pool-closed-loop.js"] }, verifier: "scripts/verify-daytrade-mother-pool-closed-loop.js", allowedAction: "restore_dynamic_priority_pool_and_fresh_quote_coverage_then_rerun" },
  ...
  { key: "strategy3", label: "Strategy3 V2", dueTime: "13:15", class: "strategy", source: "data/scan-receipts/strategy3.json", adapter: "scan_receipt", verifier: "scripts/verify-strategy3-v2-full-closure.js", allowedAction: "run_strategy3_v2_recovery_then_reverify" },
...
function validateModule(module, source, tradeDate) {
  ...
  const dateOptional = ["verification_command", "power_recovery", "contract_verification_command"].includes(module.adapter);
  ...
  if (!["verification_command", "power_recovery", "contract_verification_command"].includes(module.adapter) && !runId) issues.push("module_receipt_run_id_missing");
