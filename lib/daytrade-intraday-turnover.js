...
function nativeVolume(data = {}, source) {
  ...
  const micros = number(data.total?.time);
  const event = micros !== null && micros > 0 ? new Date(micros / 1000) : null;
  return {
    value: number(data.total?.tradeVolume),
    unit: data.intradayOddLot === true ? 'shares' : ['TSE', 'TWSE', 'OTC', 'TPEX', 'TIB'].includes(market) ? 'lots' : null,

    event_at: event && Number.isFinite(event.getTime()) ? event.toISOString() : null,
    source, is_synthetic: false,
  };
}

function evaluateTurnover({ symbol, master = {}, volume = {}, tradeDate, now }) {
  if(!master || typeof master!=='object' || Array.isArray(master)) master={};
  ...
  if (shares === null) reasons.push('MISSING_ISSUED_COMMON_SHARES');
  else if (!(shares > 0)) reasons.push('INVALID_ISSUED_COMMON_SHARES');
  if (volume.value === null || volume.value === undefined || volume.value === '') reasons.push('MISSING_VOLUME');
  else if (amount === null || amount < 0) reasons.push('INVALID_VOLUME');
  if (!['lots', 'shares'].includes(volume.unit)) reasons.push('UNKNOWN_VOLUME_UNIT');
  if (volume.is_synthetic === true) reasons.push('SYNTHETIC_VOLUME');
  else if (volume.is_synthetic !== false || !volume.source) reasons.push('NATURAL_VOLUME_UNPROVEN');
  const age = (Date.parse(now) - Date.parse(volume.event_at)) / 1000;
  if (!Number.isFinite(Date.parse(volume.event_at))) reasons.push('MISSING_VOLUME_EVENT_TIME');
  else {
    if (dateOf(volume.event_at) !== tradeDate) reasons.push('TRADE_DATE_MISMATCH');
    if (!Number.isFinite(age) || age < 0 || age > 120) reasons.push('STALE_VOLUME_EVENT');
  }

  const volumeShares = amount === null ? null : amount * (volume.unit === 'lots' ? 1000 : 1);
  if(volumeShares!==null && !Number.isFinite(volumeShares)) reasons.push('VOLUME_CONVERSION_OVERFLOW');
  if(!reasons.length && !Number.isFinite(volumeShares / shares * 100)) reasons.push('TURNOVER_CALCULATION_OVERFLOW');
  return { symbol, trade_date: tradeDate, status: reasons.length ? 'DATA_GAP' : 'ready', reasons,
    ...
    cumulative_volume: amount, volume_unit: volume.unit || null, cumulative_volume_shares: ['lots', 'shares'].includes(volume.unit) && Number.isFinite(volumeShares) ? volumeShares : null,
...
function rankTurnover(rows, { tradeDate, canonicalRunId, now }) {
  ...
  return { contract: CONTRACT, trade_date: tradeDate, canonical_run_id: canonicalRunId, calculated_at: now,
    ...
    data_gaps: rows.filter(row => row.status !== 'ready'),
    // Temporary read compatibility only; canonical consumers use data_gaps.
    gaps: rows.filter(row => row.status !== 'ready'), creates_formal_candidate: false, publish_allowed: false };
