...
const STRATEGY2_SOURCE_STATUS_NAME = process.env.STRATEGY2_SOURCE_STATUS_NAME || "fugle_daytrade_source";
...
function buildCoverage(sourceStatusRow = {}) {
  ...
  const priorityFreshQuotes120s = cleanNumber(payload.priority_fresh_quotes_120s || payload.priorityFreshQuotes120s);
  const priorityFreshQuoteCoverage120s = cleanNumber(payload.priority_fresh_quote_coverage_120s || payload.priorityFreshQuoteCoverage120s);
  const motherPoolSymbols = priorityPoolSymbols || cleanNumber(payload.mother_pool_symbols, cleanNumber(payload.active_symbols, cleanNumber(payload.eligible_symbols)));
  ...
  const readyGe35 = cleanNumber(
    payload.ready_ge_35_symbols,
    cleanNumber(
      payload.ready_ge_35,
      cleanNumber(payload.ready_ma35_continuous_symbols, cleanNumber(payload.ready_ma35_continuous))
    )
  );
  const readyGe20 = cleanNumber(payload.ready_ge_20_symbols, cleanNumber(payload.ready_ge_20, cleanNumber(payload.ready_ma20_continuous_symbols, cleanNumber(payload.ready_ma20_continuous))));
  const dailyVolumeReady = cleanNumber(
    payload.daily_volume_ready_symbols,
    cleanNumber(payload.daily_volume_rows, cleanNumber(payload.daily_volume_avg_rows, cleanNumber(payload.avg_volume5_eligible)))
  );
  ...
  const scannerCanRunOpening = boolValue(payload.scanner_can_run_opening ?? payload.scannerCanRunOpening);
  const formalEntryAllowed = boolValue(payload.formal_entry_allowed ?? payload.formalEntryAllowed);
  const quoteAgeSeconds = cleanNumber(payload.quote_age_seconds ?? payload.priority_quote_age_p95_seconds ?? payload.priority_fresh_max_quote_age_seconds, 999999);
  const selectedSymbolsFreshOk = boolValue(payload.selected_symbols_fresh_ok ?? true);
  const offSession = boolValue(payload.off_session ?? payload.offSession);
  const dedicatedDaytradeReady = ["A", "READY"].includes(daytradeGateGrade)
    && (offSession || (scannerCanRunOpening && formalEntryAllowed))
    && selectedSymbolsFreshOk
    && freshQuoteCoverage120s >= cleanNumber(process.env.STRATEGY2_SOURCE_GATE_MIN_FRESH_QUOTE_COVERAGE_120S, 0.95)
    && quoteAgeSeconds <= cleanNumber(process.env.STRATEGY2_SOURCE_GATE_MAX_QUOTE_AGE_SECONDS, 90);
  return {
    sourceStatus: String(sourceStatusRow?.status || payload.status || payload.sourceStatus || "").toLowerCase(),
    quoteStatus: String(payload.quote_status || payload.quoteStatus || "").toLowerCase(),
    intraday1mStatus: String(payload.intraday_1m_status || payload.intraday1mStatus || "").toLowerCase(),
    preopenStatus: String(payload.preopen_status || payload.preopenStatus || "").toLowerCase(),
    dailyVolumeStatus: String(payload.daily_volume_status || payload.dailyVolumeStatus || "").toLowerCase(),
...
function evaluateStrategy2SourceGate({ sourceStatusRow, readinessRow, now = new Date(), strict = true } = {}) {
  ...
  const minFreshQuoteCoverage120s = cleanNumber(process.env.STRATEGY2_SOURCE_GATE_MIN_FRESH_QUOTE_COVERAGE_120S, 0.95);
  const maxQuoteAgeSeconds = cleanNumber(process.env.STRATEGY2_SOURCE_GATE_MAX_QUOTE_AGE_SECONDS, 90);
  ...
  if (!sourceStatusRow) issues.push(`source_status ${STRATEGY2_SOURCE_STATUS_NAME} missing`);
  ...
  if (!coverage.dedicatedDaytradeReady && !coverage.quotesOk && !statusOk(coverage.quoteStatus)) issues.push(`quote_status=${coverage.quoteStatus || "not_ready"}`);
  if (!coverage.dedicatedDaytradeReady && coverage.freshQuoteCoverage120s < minFreshQuoteCoverage120s) {
    issues.push(`fresh_quote_coverage_120s ${coverage.freshQuoteCoverage120s.toFixed(4)} < ${minFreshQuoteCoverage120s}`);
  }
  if (!coverage.dedicatedDaytradeReady && coverage.quoteAgeSeconds > maxQuoteAgeSeconds) {
    issues.push(`quote_age_seconds ${coverage.quoteAgeSeconds} > ${maxQuoteAgeSeconds}`);
  }
  ...
  if (minute >= 9 * 60 + 1) {
    if (!coverage.dedicatedDaytradeReady && !coverage.latestCandleTime) issues.push("latest_candle_time missing after 09:01");
    if (!coverage.dedicatedDaytradeReady && coverage.intraday1mStaleSeconds > criticalStaleSeconds) issues.push(`intraday_1m_stale_seconds ${coverage.intraday1mStaleSeconds} > ${criticalStaleSeconds} critical`);
    else if (!coverage.dedicatedDaytradeReady && coverage.intraday1mStaleSeconds > maxStaleSeconds) issues.push(`intraday_1m_stale_seconds ${coverage.intraday1mStaleSeconds} > ${maxStaleSeconds}`);
  }
  if (!coverage.dedicatedDaytradeReady && minute >= 9 * 60 + 5 && coverage.today1mSymbols <= 0) issues.push("09:05 first batch fugle_intraday_1m missing");
  if (!coverage.dedicatedDaytradeReady && minute >= 9 * 60 + 10 && coverage.motherPoolSymbols > 0) {
  ...
  if (!coverage.dedicatedDaytradeReady && minute >= 9 * 60 + 30 && coverage.motherPoolSymbols > 0) {
...
async function readStrategy2SourceGate(config) {
  ...
  const sourceRows = await fetchRows(
    config,
    "source_status",
    `source_name=eq.${encodeURIComponent(STRATEGY2_SOURCE_STATUS_NAME)}&select=source_name,status,updated_at,message,payload&order=updated_at.desc&limit=1`
  );
