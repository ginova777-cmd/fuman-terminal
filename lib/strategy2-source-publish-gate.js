...
function evaluateStrategy2SourceGate({ sourceStatusRow, readinessRow, now = new Date(), strict = true } = {}) {
  ...
  if (readinessReady && issues.length) {
    warnings.push(`strategy2_readiness_100_authoritative; instantaneous source gate downgraded: ${issues.join("; ")}`);
    issues.length = 0;
  }
...
async function readStrategy2SourceGate(config) {
  ...
  if (readinessRow?.strategy2_ready_100 === true) {
    return evaluateStrategy2SourceGate({
      sourceStatusRow: null,
      readinessRow,
    });
  }
  const sourceRows = await fetchRows(
    config,
    "source_status",
    "source_name=eq.fugle_shared_source&select=source_name,status,updated_at,message,payload&limit=1"
  );
  return evaluateStrategy2SourceGate({
