...
function buildCoverage(sourceStatusRow = {}) {
  ...
  const quoteAgeSeconds = cleanNumber(payload.quote_age_seconds, 999999);
  const dedicatedDaytradeReady = ["A", "READY"].includes(daytradeGateGrade)
    && scannerCanRunOpening
    && formalEntryAllowed
    && freshQuoteCoverage120s >= cleanNumber(process.env.STRATEGY2_SOURCE_GATE_MIN_FRESH_QUOTE_COVERAGE_120S, 0.9)
