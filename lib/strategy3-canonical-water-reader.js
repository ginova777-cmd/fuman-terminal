...
const fs = require("fs");
const { readMotherPoolSnapshot } = require("./daytrade-mother-pool-snapshot");
const { readPinnedSnapshot, verifySnapshotRows } = require("./strategy3-pinned-snapshot");
const { CONTRACT: FIELD_CONTRACT, volumeEvidence, quoteEvidence, snapshotEvidence } = require("./strategy3-source-field-contract");
...
function quoteTimestamp(row = {}) {
  return quoteEvidence(row, String(row.trade_date || "")).canonical_quote_time;
}
...
async function readAllRows(key, target, params = {}, options = {}) {
  ...
  const overflow = await readRows(key, target, { ...params, limit: 1, offset: rows.length }, options);
  if (overflow.length) throw new Error('canonical_water_pagination_limit_exceeded:' + target);
  return rows;
}
...
async function readCanonicalDaytradeWater(options = {}) {
  ...
  const baseReceipt = {
    ...
    // Strategy3 is an independent consumer.  The shared canonical/unattended
    // gates may be degraded by FutOpt or another strategy, but that global
    // condition is not authoritative for Strategy3.  Strategy3 still checks
    // source identity, Mother Pool receipt, and per-symbol quote/1m coverage.
    strategy3_gate_isolation: strategy3Consumer ? "canonical_unattended_futopt_gate_non_authoritative" : null,
  ...
  try {
    ...
    if (!bypassLiveSessionGate && !strategy3Consumer) {
    ...
    const pinnedIdentity = options.motherPoolSnapshotIdentity || null;
    const snapshotBefore = readPinnedSnapshot(tradeDate, pinnedIdentity);
    const allPoolRows = await readAllRows(key, MOTHER_POOL_VIEW, {
      select: "*",
      trade_date: `eq.${tradeDate}`,
    ...
    const poolRows = pinnedIdentity ? allPoolRows.filter(row => snapshotBefore.symbols.has(String(row.symbol))) : allPoolRows;
    {
      const fixedIdentity = pinnedIdentity || snapshotEvidence(snapshotBefore, snapshotBefore, poolRows).identity;
      if (typeof fixedIdentity.generation !== 'string' || !fixedIdentity.generation.trim()) throw new Error('mother_pool_snapshot_generation_missing');
      const fixedRows = await readAllRows(key, 'v_fugle_daytrade_mother_pool_snapshot_v4_1', {
        select: '*', trade_date: 'eq.' + tradeDate, canonical_run_id: 'eq.' + fixedIdentity.canonical_run_id,
        mother_pool_run_id: 'eq.' + fixedIdentity.mother_pool_run_id,
        generation: 'eq.' + fixedIdentity.generation,
        snapshot_sequence: 'eq.' + fixedIdentity.snapshot_sequence, membership_status: 'neq.REMOVED', order: 'symbol.asc'
      }, {pageSize: 200, maxRows: 5000, timeout: 30000});
      failures.push(...verifySnapshotRows(fixedRows, fixedIdentity));
    }
    ...
    for (let index = 0; index < poolRows.length; index += 1) {
      ...
      if (strategy3Consumer && symbol) {
        const volume = volumeEvidence(row, tradeDate);
        for (const gap of volume.gaps) addSymbolGap(symbol, gap);
        const { gaps, ...volumeFields } = volume;
        Object.assign(normalized, volumeFields);
    ...
    if (strategy3Consumer) {
      for (const symbol of poolSymbols) {
        ...
        const quoteValid = Boolean(quote)
          ...
          && quoteEvidence(quote, tradeDate).gaps.length === 0;
    ...
    const snapshotCheck = snapshotEvidence(snapshotBefore, readPinnedSnapshot(tradeDate, pinnedIdentity), poolRows);
    failures.push(...snapshotCheck.gaps);
    ...
    const receipt = {
      ...
      source_field_contract: FIELD_CONTRACT,
      mother_pool_snapshot: snapshotCheck,
