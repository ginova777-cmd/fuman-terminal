  ...
  try {
    ...
    if (pinnedIdentity) {
      const fixedRows = await readAllRows(key, 'v_fugle_daytrade_mother_pool_snapshot_v4_1', {
        select: '*', trade_date: 'eq.' + tradeDate, mother_pool_run_id: 'eq.' + pinnedIdentity.mother_pool_run_id,
        snapshot_sequence: 'eq.' + pinnedIdentity.snapshot_sequence, order: 'symbol.asc'
      }, {pageSize: 200, maxRows: 5000, timeout: 30000});
      failures.push(...verifySnapshotRows(fixedRows, pinnedIdentity));
    }
