...
const DEFINITIONS = [
  ...
  {
    code: "AUTH_PROTECTED_READBACK_ARTIFACT_MISSING",
    layer: "auth_readback",
    action: "generate_or_install_protected_readback_credential_artifact",
    severity: "critical",
    retryable: false,
    priority: 11.5,
    test: ({ text }) => /protected_readback_credential_artifact_missing|protectedreadbackcredential.*artifact_missing/.test(text),
  },
  {
    code: "SERVICE_TOKEN_SCHEDULE_NOT_OK",
    layer: "schedule_service_token",
    action: "repair_windows_task_service_token_and_rerun_strict_schedule_audit",
    severity: "critical",
    retryable: true,
    priority: 12.5,
    test: ({ text }) => /service_token_schedule_not_ok|windowstaskandservicetokenaudit_not_ok|windows_task.*service_token.*not_ok/.test(text),
  },
  {
    code: "OPS_GATE_ARTIFACT_NOT_REPORTED",
    layer: "ops_artifact",
    action: "generate_missing_ops_artifact_before_final_audit",
    severity: "critical",
    retryable: true,
    priority: 14,
    test: ({ text }) => !String(text || "").trim() || /^reason:\s*\|\s*status:\s*$/i.test(text) || /^failures:\s*$/i.test(text) || /artifact_missing/.test(text),
  },  {
