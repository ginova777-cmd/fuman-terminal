...
const DEFINITIONS = [
  ...
  },
  {
    ...
    retryable: false,
    priority: 3,
    test: ({ text, input }) => (input?.ok === true && /protectedreadbackcredential|protected_readback|reason:ok|status:ok/.test(text)) || /protectedreadbackcredential.*reason:ok|reason:ok \| failures:$/.test(text),
  },  {
