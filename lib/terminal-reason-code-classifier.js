...
const DEFINITIONS = [
  {
    code: "PREDICTIVE_PREFLIGHT_NOT_READY",
    layer: "predictive_preflight",
    action: "write_or_recheck_predictive_preflight",
    severity: "critical",
    retryable: true,
    priority: 34,
    test: ({ text }) => /predictivepreflight_artifact_missing|predictive_preflight_artifact_missing/.test(text),
  },
  {
    code: "REASON_CODE_MISSING",
    layer: "reason_code_classifier",
    action: "populate_reason_code_before_publish",
    severity: "critical",
    retryable: true,
    priority: 86,
    test: ({ text }) => /reason:\s*\|\s*status:\s*$|reason_code_classifier_contract_missing|reason_code_classifier_codes_missing/.test(text),
  },
  {
    code: "OPS_STATUS_SNAPSHOT_REFRESH_FAILED",
    layer: "control_plane",
    action: "refresh_terminal_ops_status_snapshot",
    severity: "critical",
    retryable: true,
    priority: 87,
    test: ({ text }) => /refresh_failed:ops_status_snapshot/.test(text),
  },
  {
    code: "BACKEND_SERVICE_TOKEN_SCHEDULE_NOT_READY",
    layer: "backend_auth",
    action: "repair_service_token_schedule_without_membership_login",
    severity: "critical",
    retryable: false,
    priority: 14,
    test: ({ text }) => /windowstaskandservicetokenaudit_not_ok|service_token_schedule_not_ok/.test(text),
  },
  {
    code: "CLOSURE_GREEN",
    layer: "closure",
  ...
  },
  {
    ...
    retryable: false,
    priority: 2,
    test: ({ text, input }) => input?.marketClosed === true || /market_closed|market closed|weekend|holiday/.test(text),
  },
  {
  ...
  },
  {
    ...
    retryable: false,
    priority: 10,
    test: ({ text }) => /protected_readback_credential_not_armed|authenticated_protected_readback_not_armed|authenticated readback required|protected_surface_needs_authenticated_readback_token|token not armed|missing_bearer_token|resource_chain_authenticated_readback_missing_or_not_armed/.test(text),
  },
  {
    ...
    retryable: false,
    priority: 11,
    test: ({ text }) => /protectedreadbackcredential_not_ok|protected_readback_credential_not_ok|protected_readback_login_failed|protected_readback_login_error|protected_readback_unauthorized|protected_readback_membership_required|protected_readback_credential_next_actions_missing|protected_readback_timeout|protected_readback_request_error|^failures:\s*$/.test(text),
  },
  {
  ...
  },
  {
    ...
    retryable: true,
    priority: 13,
    test: ({ text }) => /protected_readback_timeout|protected readback timeout|scorecard:protected_readback_timeout|source_reports:protected_readback_timeout|^failures:\s*$/.test(text),
  },
  {
  ...
  },
  {
    ...
    retryable: true,
    priority: 30,
    test: ({ text, input }) => input?.waterRoot?.ok === false || input?.canonicalGate?.canonicalGateStatus === "not_ready" || (input?.reason === "" && input?.status === "" && input?.sourceMessage === "" && input?.sourceStatus === "" && input?.sourceGate === "") || /blocked_source|waterroot_not_ok|water_root_not_ok|canonical_gate_not_a|source_root_not_ready|source_status_not_ok|source_status=degraded|source_status_not_connected|source_not_ready|coverage|stale|priority.*0|sourceMessage:\s*\|\s*sourceStatus:\s*\|\s*sourceGate:\s*$|not connected/.test(text),
  },
  {
  ...
  },
  {
    ...
    retryable: true,
    priority: 32,
    test: ({ text }) => /warmup_\d{4}_not_green|natural_(?:0700|0845|0900)_not_green|natural_warmup_not_ok/.test(text),
  },
  {
  ...
  },
  {
    ...
    retryable: true,
    priority: 70,
    test: ({ text, input }) => input?.runIdClosureOk === false || /failed_display|blocked_runid_closure|runid_closure_not_ok|runid.*mismatch|runid\s*!=|scorecard.*latest pointer|latest pointer.*runid|missing.*runid|productionapi":""|mobile":""|scorecard88":""/.test(text),
  },
  {
  ...
  },
  {
    ...
    retryable: true,
    priority: 82,
    test: ({ text }) => /resourcechain_not_ok|resource_chain_not_ok|terminal_resource_chain_unattended_failed|resource_chain_readback|resource-chain-readback|terminal-resource-chain:unattended_exit_1|active_module_missing_from_resource_chain|resource_chain_authenticated_readback_missing_or_not_armed/.test(text),
  },
  {
  ...
  },
  {
    ...
    retryable: true,
    priority: 84,
    test: ({ text }) => /autorollforward_not_ok|auto_roll_forward_not_ok|auto_roll_forward_idempotency_contract_missing|auto_roll_forward_idempotency_invariant_missing/.test(text),
  },
  {
  ...
  },
  {
    ...
    retryable: true,
    priority: 83,
    test: ({ text }) => /live_schedule_gate_failed|live_task_missing|autonomous_schedule.*not_ready|schedule_gate_failed|windowstaskandservicetokenaudit_not_ok|service_token_schedule_not_ok/.test(text),
  },];
