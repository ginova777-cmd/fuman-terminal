...
const DEFINITIONS = [
  ...
  },
  {
    ...
    retryable: false,
    priority: 12,
    test: ({ text }) => /blocked_auth|backend_service_token|service token|service_token_schedule|windowstaskandservicetokenaudit|401|unauthorized/.test(text) && !/scorecardstatus":401|scorecardstatus:401|membership-protected|authenticated readback/.test(text),
  },
  {
  ...
  },
  {
    ...
    retryable: true,
    priority: 30,
    test: ({ text, input, context }) => input?.waterRoot?.ok === false || context?.waterRoot?.ok === false || input?.canonicalGate?.canonicalGateStatus === "not_ready" || /blocked_source|waterroot_not_ok|water_root_not_ok|canonical_gate_not_a|source_root_not_ready|source_status_not_ok|source_status=degraded|source_status_not_connected|source_not_ready|coverage|stale|priority.*0|not connected/.test(text),
  },
  {
  ...
  },
  {
    ...
    retryable: true,
    priority: 70,
    test: ({ text, input }) => input?.runIdClosureOk === false || /failed_display|blocked_runid_closure|runid_closure_not_ok|runid.*mismatch|runid != latest pointer|row\/sourcereport runid != latest pointer|scorecard \/88 row\/sourcereport runid|latest pointer|runId.*mismatch|missing.*runid|productionapi":""|mobile":""|scorecard88":""/.test(text),
  },
  {
    ...
    retryable: true,
    priority: 71,
    test: ({ text }) => /tradedate_mismatch|sourcedate_mismatch|rundate_mismatch|date mismatch|latestdate_mismatch|scorecard_latestdate_mismatch/.test(text),
  },
  {
  ...
  },
  {
    ...
    retryable: true,
    priority: 83,
    test: ({ text }) => /productionliveopsreadback_not_ok|production_live_not_ok|production_live_issue|production_live_passes_missing|production_live_authenticated_readback_required_for_ready|refresh_failed:production_live_readback/.test(text),
  },
  {
    code: "READINESS_REPORT_INCOMPLETE",
    layer: "readiness_report",
    action: "repair_readiness_report_sections",
    severity: "critical",
    retryable: true,
    priority: 86,
    test: ({ text }) => /nonproduction_passes_missing|runbook_too_short|remaining_conditions_too_short|runbook_missing/.test(text),
  },
  {
    ...
    retryable: true,
    priority: 85,
    test: ({ text }) => /reasoncodeclassifier_not_ok|reason_code_classifier_not_ok|reason_code_classifier_unknown_entries|ops_status_reason_code_summary_not_ok|refresh_failed:reason_code_classifier/.test(text),
  },
  {
  ...
  },
  {
    ...
    retryable: true,
    priority: 84,
    test: ({ text }) => /autorollforward_not_ok|auto_roll_forward_not_ok|auto_roll_forward_idempotency_invariant_missing|auto_roll_forward_idempotency_contract_missing|idempotency_contract_missing/.test(text),
  },
  {
    ...
    retryable: true,
    priority: 110,
    test: ({ text }) => /trading_day_after_formal_source_window|trading_day_wait_source_window|resume_window_due/.test(text),
  },
  {
    ...
    retryable: true,
    priority: 81,
    test: ({ text }) => /finalaudit_not_ok|final_audit_not_ok|final_audit_issue|artifact_missing|reason:\s*\|\s*status:|terminal_autonomous_completion_audit|completion_audit|ops_status_root_cause_summary|ops_status_root_cause_recovery_plan/.test(text),
  },
  {
