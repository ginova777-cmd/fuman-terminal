...
const num=s=>{const v=String(s).replace(/[, %\s]/g,"").replace(/^▲\+?/,"+").replace(/^▼-?/,"-");return /^[+-]?\d+(?:\.\d+)?$/.test(v)?Number(v):NaN;};
...
function parse(html,date){
 ...
 if(Date.parse(v.session_end)>Date.parse(date+"T08:20:00+08:00")||Date.parse(v.session_start)>=Date.parse(v.session_end))throw Error("night_futures_session_not_completed");
 const calendar=require("../data/contracts/morning_tx_settlement_calendar_2026.json");
 if(Number(date.slice(0,4))!==calendar.year) throw Error("morning_settlement_calendar_year_missing");
 v.settlement_reminder=require("./opening-report-stage-contract").settlementReminder(date,calendar.dates);
 v.settlement_calendar_source=calendar.source;
 return v;
}
...
function summary(e){return e?.ok?`台指期夜盤：${e.change_percent>=0?"+":""}${e.change_percent.toFixed(2)}%${e.settlement_reminder ? "｜"+e.settlement_reminder : ""}`:"台指期夜盤：資料未確認";}
