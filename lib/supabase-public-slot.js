...
const STRATEGY2_READY_VIEW = process.env.STRATEGY2_SUPABASE_READY_VIEW || "v_strategy2_intraday_ready";
const STRATEGY2_READY_RPC = process.env.STRATEGY2_SUPABASE_READY_RPC || "get_strategy2_intraday_ready";
const STRATEGY2_INTRADAY_1M_RPC = process.env.STRATEGY2_SUPABASE_1M_RPC || "get_strategy2_intraday_1m_latest_n";
...
function normalizeStrategy2ReadyRow(row) {
  ...
  return {
    ...
    readyGe35: row?.ready_ge_35 === true || cleanNumber(row?.today_candle_count) >= 35,
    ready_ge_35: row?.ready_ge_35 === true || cleanNumber(row?.today_candle_count) >= 35,
...
async function fetchStrategy2Intraday1mCoverage(options = {}) {
  ...
  const rows = await fetchRows(INTRADAY_1M_STATUS_VIEW, {
    select: "symbol,market,latest_candle_time,today_candle_count,candle_count,has_today_data,ready_ge_35,updated_at",
  ...
  rows.forEach((row) => {
    const count = cleanNumber(row.today_candle_count ?? row.candle_count ?? row.rows_today);
    if (row.has_today_data === false && count <= 0) return;
    if (count <= 0) return;
    totalCandles += count;
    symbolCount += 1;
    if (row.ready_ge_35 === true || count >= 35) readyGe35 += 1;
...
async function getStrategy2SourceHealth(options = {}) {
  try {
    ...
    const readbackHealthyStatus = quoteOk
      && intradayOk
      && ["stopped", "stale", "unknown", "idle"].includes(statusText);
    ...
    return {
      ...
      reason: ok ? "" : [
        ...
        !payloadQuotesOk && !coverageOk && quotes < minQuotes ? `quotes=${quotes}<${minQuotes}` : "",
...
async function fetchActiveCommonStockQuotes(options = {}) {
  ...
  if (!health.ok && !options.allowWarmupSource) {
  ...
  let quotes = rows
    ...
    .map((quote) => ({ ...quote, sourceHealthOk: health.ok, quoteReadySource: source, readyViewError }));
  if (quotes.length < Number(options.minQuotes || 0) && (health.ok || options.allowWarmupSource)) {
    quotes = rows
      .map(normalize)
      .filter(Boolean)
      .filter((quote) => quote.is_active !== false && quote.is_etf !== true && quote.is_warrant !== true && quote.is_cb !== true && quote.is_blacklisted !== true && quote.is_daytrade_unsuitable !== true)
      .map((quote) => ({ ...quote, acceptedBySourceHealth: health.ok, sourceHealthOk: health.ok, quoteReadySource: source, readyViewError }));
  }
  return {
...
async function fetchIntraday1m(code, limit = 240, options = {}) {
  ...
  if (!health.ok && !options.allowWarmupSource) return { ok: false, error: health.reason || "source_status unhealthy", rows: [], candles: [], sourceHealthy: false, sourceAgeSeconds: health.sourceAgeSeconds };
  let rows = [];
  let source = STRATEGY2_INTRADAY_1M_RPC;
  try {
    rows = await fetchRpcRows(STRATEGY2_INTRADAY_1M_RPC, {
      symbols: [symbol],
      bars_per_symbol: Math.max(35, Number(limit || 240)),
    }, { timeout: options.timeout || 30000 });
  } catch {
  ...
  candles.sort((a, b) => Date.parse(a.candleTime || a.time || "") - Date.parse(b.candleTime || b.time || ""));
  return { ok: candles.length >= 35, error: candles.length >= 35 ? "" : `candle_count=${candles.length}<35`, rows: candles, candles, source, sourceHealthy: health.ok, sourceAgeSeconds: health.sourceAgeSeconds };
}
  ...
  try {
    if (normalized.length) {
      ...
      readyRows.forEach((row) => byCode.set(normalizeCode(row.symbol), {
        ...
        ma35_available: row.ready_ge_35 === true && cleanNumber(row.today_candle_count) >= 35,
    ...
    for (const group of groups) {
      const params = {
        select: "symbol,market,latest_candle_time,today_candle_count,ready_ge_35,intraday_1m_status_updated_at",
      ...
      try {
        ...
        rows = await fetchRows(INTRADAY_1M_STATUS_VIEW, {
          select: "symbol,market,latest_candle_time,today_candle_count,candle_count,ready_ge_35,ready_ge_80,ready_ge_200,has_today_data,updated_at",
          limit: group.length || 5000,
      ...
      rows.forEach((row) => byCode.set(normalizeCode(row.symbol), {
        ...
        today_candle_count: cleanNumber(row.today_candle_count ?? row.candle_count ?? row.rows_today),
        ma35_available: row.ready_ge_35 === true && cleanNumber(row.today_candle_count ?? row.candle_count ?? row.rows_today) >= 35,
      }));
    }
