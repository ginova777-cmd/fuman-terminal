...
async function getStrategy2SourceHealth(options = {}) {
  try {
    ...
    const quoteAge = cleanNumber(quoteHealth.quote_age_seconds) || cleanNumber(payload.quote_age_seconds) || secondsSince(payload.last_quote_at || status?.updated_at);
    const quotes = cleanNumber(quoteHealth.quote_count) || cleanNumber(payload.quotes || payload.quote_count);
    ...
    const activeSymbols = cleanNumber(quoteHealth.active_symbols) || cleanNumber(payload.active_symbols || payload.eligible_symbols || payload.symbols);
    ...
    const intradayOk = options.requireIntraday1m
      ? intradayRows > 0
        ...
        && (!intradayAge || intradayAge <= maxIntradayAge)
        && (latestCandleAge == null || latestCandleAge <= maxIntradayAge)
      : true;
    const anonRead = options.verifyAnonReadAccess ? await verifyAnonReadAccess() : { ok: true, checks: [], failed: [] };
    const statusOk = !status || String(status?.status || "").toLowerCase() === "ok";
    const ok = Boolean(healthRows[0] && statusOk && quoteOk && intradayOk && anonRead.ok);
    return {
      ok,
      healthy: ok,
      sourceHealthy: ok,
      ...
      reason: ok ? "" : [
        healthRows[0] ? "" : `${QUOTES_HEALTH_VIEW} missing`,
