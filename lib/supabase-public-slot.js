...
const STRATEGY3_INTRADAY_1M_READINESS_RPC = process.env.STRATEGY3_SUPABASE_1M_READINESS_RPC || "get_strategy3_intraday_1m_readiness";
...
async function fetchStrategy3QuoteLatestReady(options = {}) {
  const rows = [];
  const pageSize = Number(options.latestPageSize || process.env.STRATEGY3_QUOTES_LATEST_PAGE_SIZE || 250);
  ...
  for (let offset = 0; offset < limit; offset += pageSize) {
    const page = await fetchRows(STRATEGY3_QUOTES_TABLE, {
      ...
      limit: Math.min(pageSize, limit - offset),
      offset,
    }, { timeout: options.latestTimeout || options.timeout || 45000 });
    rows.push(...page);
    if (page.length < pageSize) break;
  }
  const quotes = rows
    ...
    .filter(Boolean)
    .filter((quote) => !options.maxQuoteAgeSeconds || quoteIsFresh(quote, options.maxQuoteAgeSeconds));
  const status = options.skipIntradayStatus
    ? { ok: true, byCode: new Map(), source: "skipped_strategy3_fast_readiness_probe" }
    : await fetchStrategy3Intraday1mStatus(quotes.map((quote) => quote.code));
  ...
  const merged = quotes.map((quote) => {
    ...
    return {
      ...
      quoteReadySource: options.skipIntradayStatus
        ? `${STRATEGY3_QUOTES_TABLE}+${STRATEGY3_INTRADAY_1M_READINESS_RPC}+${STRATEGY3_DAILY_VOLUME_TABLE}`
        : `${STRATEGY3_QUOTES_TABLE}+${STRATEGY3_INTRADAY_1M_STATUS_VIEW}+${STRATEGY3_DAILY_VOLUME_TABLE}`,
    };
  });
  return {
    ...
    quotes: merged,
    source: options.skipIntradayStatus
      ? `${STRATEGY3_QUOTES_TABLE}+${STRATEGY3_INTRADAY_1M_READINESS_RPC}+${STRATEGY3_DAILY_VOLUME_TABLE}`
      : `${STRATEGY3_QUOTES_TABLE}+${STRATEGY3_INTRADAY_1M_STATUS_VIEW}+${STRATEGY3_DAILY_VOLUME_TABLE}`,
  };
}
...
async function fetchStrategy3Intraday1mReadiness(codes = [], options = {}) {
  const symbols = [...new Set((codes || []).map(normalizeCode).filter((code) => /^\d{4}$/.test(code)))];
  const rows = await fetchRpcRows(STRATEGY3_INTRADAY_1M_READINESS_RPC, {
    p_symbols: symbols,
    p_trade_date: options.tradeDate || undefined,
    p_min_candles: Number(options.minCandles || 35),
    p_min_symbols: Number(options.minSymbols || 1000),
  }, { timeout: options.timeout || 30000 });
  const row = rows[0] || {};
  return {
    ok: true,
    source: STRATEGY3_INTRADAY_1M_READINESS_RPC,
    tradeDate: row.trade_date || options.tradeDate || "",
    requestedSymbols: cleanNumber(row.requested_symbols),
    today1mSymbols: cleanNumber(row.today_1m_symbols),
    today1mRows: cleanNumber(row.today_1m_rows),
    readyGe35: cleanNumber(row.ready_ge_35),
    latestCandleTime: row.latest_candle_time || "",
    intraday1mStaleSeconds: cleanNumber(row.intraday_1m_stale_seconds, null),
