...
async function fetchStarPreopenReadback(options = {}) {
  const rows = await fetchRows("v_fugle_daytrade_star_preopen_readback", {
    select: "*",
    order: "star_final_ok.desc,futopt_change_percent.desc,relative_to_txf_percent.desc,futopt_total_volume.desc,symbol.asc",
    limit: options.limit || 5000,
  }, { timeout: options.timeout || 20000 });
  return { ok: true, rows, count: rows.length, source: "v_fugle_daytrade_star_preopen_readback" };
}
