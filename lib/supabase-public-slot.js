...
const STRATEGY3_MOTHER_POOL_TABLE = process.env.STRATEGY3_SUPABASE_MOTHER_POOL_TABLE || STOCK_UNIVERSE_TABLE;
...
async function fetchStrategy3DaytradeMotherPoolMap(options = {}) {
  const limit = Math.max(1, Number(options.limit || process.env.STRATEGY3_MOTHER_POOL_LIMIT || 500));
  const rows = await fetchRows(STRATEGY3_MOTHER_POOL_TABLE, {
    select: "symbol,name,market,priority_rank,priority_reason,source,updated_at,payload",
  ...
  rows.forEach((row) => {
    ...
    const ruleHits = Array.isArray(payload.motherPoolRuleHits) ? payload.motherPoolRuleHits : [];
    ...
    byCode.set(code, {
      ...
      name: row.name || "",
      market: row.market || "",
      rank: cleanNumber(row.priority_rank),
      reason: row.priority_reason || "",
      source: row.source || STRATEGY3_MOTHER_POOL_TABLE,
      updatedAt: row.updated_at || "",
      ruleVersion: payload.motherPoolRuleVersion || "",
      ruleHits,
      metrics,
    });
    if (row.updated_at && (!latestUpdatedAt || String(row.updated_at) > String(latestUpdatedAt))) latestUpdatedAt = row.updated_at;
  });
  return {
    ok: byCode.size > 0,
    byCode,
    rows: byCode.size,
    source: STRATEGY3_MOTHER_POOL_TABLE,
...
async function fetchActiveCommonStockQuotes(options = {}) {
  ...
  try {
    ...
    const universe = await fetchUniverseMap().catch(() => new Map());
    ...
    rows = rows
      .map(normalizeQuote)
      .filter(Boolean)
      .filter((quote) => universe.size <= 0 || universe.has(quote.code))
...
function normalizeFutoptQuote(row) {
  ...
  return {
    ...
    symbol,
    underlying_symbol: normalizeCode(row?.underlying_symbol || row?.payload?.underlying_symbol || row?.payload?.underlyingSymbol) || (String(row?.product || "").toUpperCase() === "TXF" || symbol.startsWith("TXF") ? "TXF" : ""),
    underlying_name: String(row?.underlying_name || row?.payload?.underlying_name || "").trim(),
    source_name: String(row?.source_name || "").trim(),
    is_realtime: row?.is_realtime === true,
...
async function fetchDaytradeFutoptQuotesLive(options = {}) {
  const rows = await fetchRows("fugle_daytrade_futopt_quotes_live", {
    select: "future_symbol,underlying_symbol,updated_at,last_price,open_price,high_price,low_price,previous_close,change_percent,total_volume,product,session,source_name,is_realtime,is_fallback,is_formal_entry_eligible,payload",
    order: "updated_at.desc",
    limit: options.limit || 5000,
  }, { timeout: options.timeout || 20000 });
  const bySymbol = new Map();
  rows.map((row) => normalizeFutoptQuote({
    ...row,
    product: row?.product || row?.payload?.product || row?.payload?.product_type || "STOCK_FUTURE",
  })).filter(Boolean).forEach((quote) => bySymbol.set(quote.future_symbol, quote));
  return {
    ok: true,
    rows,
    bySymbol,
    txf: [...bySymbol.values()].find((quote) => quote.future_symbol === "TXF") || null,
    count: bySymbol.size,
