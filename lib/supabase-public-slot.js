...
const STRATEGY3_INTRADAY_1M_STATUS_VIEW = process.env.STRATEGY3_SUPABASE_1M_STATUS_VIEW || "v_strategy2_intraday_ready";
...
async function fetchStrategy3Intraday1mLatestN(code, limit = 160) {
  ...
  if (candles.length < 35) {
    const latestTradeDate = await fetchStrategy3LatestIntradayTradeDate();
    if (latestTradeDate) {
