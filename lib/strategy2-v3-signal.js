...
// Strategy2 is the seven-strategy read-only scanner. Mother Pool determines
...
function candidateFromWaterRow(row, candles, options = {}) {
  ...
  return {
    ...
    score: primary?.priority || 0, strategy: primary?.label || "七策略未命中", signalId: primary?.id || "seven_strategy_no_match",
    signalLine: primary?.reason || "資料完整但七策略未命中", reason: dataGapReasons.length ? `DATA_GAP:${dataGapReasons.join(",")}` : chaseBlocked ? "剔除：追高限制" : primary?.reason || "七策略未命中",
