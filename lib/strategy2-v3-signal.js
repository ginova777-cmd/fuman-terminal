...
const { evaluateJiangCore } = require("./jiang-intraday-signal-core");
const { attachBurstObservation } = require("./daytrade-intraday-burst-readback");
...
function candidateFromWaterRow(row, candles, options = {}) {
  ...
  const jiang = evaluateJiangCore(ordered, row);
  const jiangPrimary = jiang.primarySignal;
  const strategyLabel = jiangPrimary ? "Jiang｜" + jiangPrimary.label : "V3量價突破";
  const stateLabel = qualifies ? "LIVE候選" : jiangPrimary ? "觀察｜策略命中" : "資料完整，未達訊號";
  const stateId = qualifies ? "candidate" : "watch";
  const signalId = jiangPrimary ? jiangPrimary.id : "s2_v3_1m_trend_volume_breakout";
  const signalLine = jiangPrimary
    ? jiangPrimary.label + "；三關價 " + jiang.threeGate.mode + " A " + jiang.threeGate.upper + " / B " + jiang.threeGate.middle + " / C " + jiang.threeGate.lower
    : "V3量價突破";
  ...
  const candidate = {
    ...
    signalId,
    signalLine,
    reason: jiangPrimary ? jiangPrimary.reason : "正式 Fugle 1分K 量價趨勢突破",
    state: stateLabel,
    stateId,
    ...
    evidence: {
      ...
      nearSessionHigh,
      jiang,
      strategyHits: jiang.hits,
      primarySignal: jiangPrimary,
      secondaryLabels: jiang.secondaryLabels,
      threeGate: jiang.threeGate,
      nLargeChaseGuard: jiang.guards.some((guard) => guard.id === "jiang_n_large_chase_guard"),
    },
    jiang,
    strategyHits: jiang.hits,
    primarySignal: jiangPrimary,
  ...
  return attachBurstObservation(candidate, options.burstReadback || row.burstReadback);
}
