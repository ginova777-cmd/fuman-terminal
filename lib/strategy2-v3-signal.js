...
function stochastic(candles, period = 5) {
...
function candidateFromWaterRow(row, candles, options = {}) {
  ...
  const technical = sharedTrend(ordered); const kdNow = technical.kdK; const kdPrev = technical.kdPrevK; const kdUp = technical.available && technical.kdTrendUp;
  const rsiNow = technical.rsi3; const rsiPrev = technical.rsi3Prev; const rsiUp = technical.available && technical.rsiTrendUp;
  ...
  return {
    ...
    ma: Object.fromEntries(Object.entries(ma).map(([key, value]) => [key, round(value)])), indicators: { indicatorContract: technical.contract, kdD: technical.kdD, rsi3: technical.rsi3, rsi6: technical.rsi6, kd: round(kdNow), kdUp, rsi: round(rsiNow), rsiUp, macdDif: round(dif.at(-1)), macdUp, macdRed },
