...
function buildModuleAudit(module, receipt, expectedDate, now) {
  ...
  const receiptDate = module.key === "institution"
    ...
    : executionDate;
  const current = (module.key === 'institution' ? receiptDate : module.key === 'strategy5' ? dateFromTimestamp(receipt?.startedAt) || executionDate : executionDate) === expectedDate;
  ...
  if (current) {
    if (module.key === 'strategy3' && validStrategy3Recovery(receipt, expectedDate)) {
...
function buildScanAudit({ runtimeDir = process.env.FUMAN_RUNTIME_DIR || "C:/fuman-runtime", now = new Date() } = {}) {
  ...
  const modules = MODULES.map((module) => {
    ...
    return buildModuleAudit(module, receipt, marketDate, now);
  });
  return {
    ok: true,
