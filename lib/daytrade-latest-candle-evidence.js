...
function latestCandleEvidence(symbols, rows, nowMs) {
  ...
  const items = requested.map(symbol => {
    ...
    // Freshness for a completed candle is measured from its bar end, not its
    // opening timestamp.  Using bar_start adds an artificial 60-second delay
    // and can reject a naturally completed bar that is still within the
    // configured 120-second freshness window.
    const end = start + 60000;
    const age = (nowMs - end) / 1000;
    const complete = end <= nowMs;
    ...
    return { symbol, status: fresh ? 'READY' : 'DATA_GAP', reason: qualityReason || (!complete ? 'CANDLE_NOT_COMPLETED' : fresh ? null : 'CANDLE_STALE'),
      ...
      bar_end: new Date(end).toISOString(), candle_age_seconds: age,
  ...
  return { contract: 'daytrade_latest_candle_write_evidence_v1',
    checked_at: new Date(nowMs).toISOString(), freshness_reference: 'bar_end', freshness_limit_seconds: 120,
