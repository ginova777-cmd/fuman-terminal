...
const STRATEGY3_ENTRY_PRICE_SOURCES = new Set([
  "intraday_1m_1300",
  "intraday_1m_1300_exact",
  "intraday_1m_entry_window_tolerance",
  "intraday_1m_tail_volume_confirmed",
]);

function strategy3EntrySourceOk(row = {}) {
  ...
  return STRATEGY3_ENTRY_PRICE_SOURCES.has(source);
}

function strategy3UsesLatestQuoteFallback(row = {}) {
  return /fugle_quotes_latest|最高價補值/i.test(cleanText(row.reason));
}

function verifiedStrategy3Zero(payload = {}) {
  const r = Array.isArray(payload.sourceReports) && payload.sourceReports.length === 1 ? payload.sourceReports[0] : null;
  return payload.refreshScope === 'strategy3-only' && Boolean(payload.refreshRunId)
    && /^\d{4}-\d{2}-\d{2}$/.test(payload.latestDate || '')
    && r?.key === 'strategy3' && r.strategy === '策略3隔日沖成績單'
    && r.ok === true && r.publishAllowed === true && r.evidenceStatus === 'complete'
    && r.runId === payload.refreshRunId && r.date === payload.latestDate && r.expectedDisplayDate === payload.latestDate
    && r.count === 0 && r.emittedRows === 0 && r.suppressedRows === 0
    && r.fallbackUsed === false && !r.marketClosedLastGood && !r.rowSuppressionReason
    && Array.isArray(r.rowSuppressionBlockers) && r.rowSuppressionBlockers.length === 0;
}
...
function verifyScorecardStrategyRules(payload = {}, options = {}) {
  ...
  addCheck(checks, rows.length > 0 || verifiedStrategy3Zero(payload), `${source}-strategy-rules-rows`, `${source} has rows for strategy rule lock`, { rows: rows.length });
  ...
  const strategy3BadEntrySource = strategy3Rows
    .filter((row) => !strategy3EntrySourceOk(row))
    .map((row) => ({
      ticker: cleanText(row.ticker),
      entry_time: cleanText(row.entry_time),
      entry_price: row.entry_price,
      entry_price_source: metadataOf(row).entry_price_source,
    }));
  const strategy3BadLatestQuoteFallback = strategy3Rows
    .filter(strategy3UsesLatestQuoteFallback)
    .map((row) => ({
      ticker: cleanText(row.ticker),
      entry_price: row.entry_price,
      high_price: row.high_price,
      reason: cleanText(row.reason).slice(0, 220),
  ...
  addCheck(checks, strategy3BadEntrySource.length === 0, `${source}-strategy3-entry-price-source`, `${source} strategy3 13:00 entry_price must come from formal Fugle 1m entry evidence, not generic price/latest quote`, { bad: strategy3BadEntrySource });
  addCheck(checks, strategy3BadLatestQuoteFallback.length === 0, `${source}-strategy3-no-fugle-latest-high-fallback`, `${source} strategy3 scorecard must not use fugle_quotes_latest as historical high fallback`, { bad: strategy3BadLatestQuoteFallback });  addCheck(checks, strategy3BadSource.length === 0, `${source}-strategy3-source-report-date`, `${source} strategy3 source_date must be present and match the Strategy3 source report date`, { bad: strategy3BadSource, sourceReportDates: [...sourceReportDates(payload, "strategy3")] });
