...
const WINDOW_RULES = [
  ...
  { strategy: "策略2成績單", start: 9 * 60, end: 12 * 60, label: "09:00-12:00" },
];
...
function metadataOf(row = {}) {
  ...
  return {
    ...
    followup_days: cleanNumber(row.followup_days || hydrated.followup_days),
    followup_rule: cleanText(row.followup_rule || hydrated.followup_rule),
    followup_status: cleanText(row.followup_status || hydrated.followup_status),
    followup_due_date: cleanText(row.followup_due_date || hydrated.followup_due_date),
  };
}
...
function verifyScorecardStrategyRules(payload = {}, options = {}) {
  ...
  const strategy3BadSource = strategy3Rows
    .filter((row) => {
      ...
      return !sourceDate || sourceDate === cleanText(row.record_date);
    })
  ...
  addCheck(checks, strategy3BadSource.length === 0, `${source}-strategy3-previous-source`, `${source} strategy3 must keep previous trading-day source marker`, { bad: strategy3BadSource });
