...
// Daily trend is mandatory; completed 60m RSI can add five score points only.
...
async function readStrategy3TechnicalTrends({ symbols, tradeDate, poolBySymbol, evaluatedAt = new Date() }) {
  ...
  const pairs = await mapConcurrent(symbols, 4, async (symbol) => {
    try {
      ...
      if (dailyRead.status === 'rejected') throw dailyRead.reason;
      const dailyRowsSource = dailyRead.value;
