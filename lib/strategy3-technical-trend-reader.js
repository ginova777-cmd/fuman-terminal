...
// Daily and completed 60m signals are evidence for independent optional bonuses.
function evaluateStrategy3Trend(hourly, daily) {
  const hourlyPass = Boolean(hourly?.ok && hourly.rsi3_over_rsi6 && hourly.rsi_trend_up);
  const dailyPass = Boolean(daily?.ok && daily.signal_pass);
  return {
    ok: dailyPass, source_ready: daily?.ok === true,
    reason: !daily?.ok ? 'daily_' + (daily?.reason || 'unavailable') : !dailyPass ? 'daily_kd_rsi_trend_not_all_up' : '',
    hourly60: hourly, daily,
    hourly_strategy3_pass: hourlyPass, daily_strategy3_pass: dailyPass,
    contract: TREND_CONTRACT.contract, hourly60_required: TREND_CONTRACT.hourly60Required, hourly60_bonus_points: hourlyPass ? TREND_CONTRACT.hourly60BonusPoints : 0,
    hourly60_bonus_status: !hourly?.ok ? 'unavailable' : hourlyPass ? 'awarded' : 'not_awarded',
  };
}
...
async function readStrategy3TechnicalTrends({ symbols, tradeDate, poolBySymbol, evaluatedAt = new Date() }) {
  ...
  const pairs = await mapConcurrent(symbols, 4, async (symbol) => {
    try {
      const [dailyRead, hourlyRead] = await Promise.allSettled([
      ...
      const dailyRowsSource = dailyRead.status === 'fulfilled' ? dailyRead.value : [];
      const hourlyBars = aggregateCompleted60m(hourlyRead.status === 'fulfilled' ? hourlyRead.value : [], tradeDate, evaluatedAt);
      const dailySeries = dailyBars(dailyRowsSource, tradeDate, poolBySymbol.get(symbol));
      const hourly = indicatorTrend(hourlyBars);
      const daily = dailyRead.status === 'fulfilled' ? indicatorTrend(dailySeries) : {ok:false,reason:'daily_source_read_failed'};
      return [symbol, {
        ...evaluateStrategy3Trend(hourly, daily),
...
module.exports = { evaluateStrategy3Trend, readStrategy3TechnicalTrends, aggregateCompleted60m, indicatorTrend, dailyBars, KD_PERIOD, RSI_FAST_PERIOD, RSI_SLOW_PERIOD, MIN_INDICATOR_BARS, DAILY_TABLE, INTRADAY_TABLE };
