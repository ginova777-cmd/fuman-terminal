...
const WATCHLIST_WARRANT_COVERAGE_QUERY = { compact: "1", shell: "1", limit: "500" };
const SNAPSHOT_WRITE_TIMEOUT_MS = Number(process.env.DESKTOP_ROUTE_SNAPSHOT_WRITE_TIMEOUT_MS || process.env.FUMAN_DESKTOP_ROUTE_SNAPSHOT_WRITE_TIMEOUT_MS || 60000);
...
function desktopRouteTasks() {
  return [
    ["/api/terminal-home", terminalHome, {}, 18000],
    ["/api/market", market, compactQuery(24), 12000],
    ["/api/stocks", stocks, { limit: "120", compact: "1", shell: "1" }, 22000],
    ["/api/open-buy-latest", openBuyLatest, compactQuery(60), 22000],
    ["/api/strategy3-latest", strategy3Latest, compactQuery(60), 12000],
    ["/api/strategy4-latest", strategy4Latest, compactQuery(70), 12000],
    ["/api/strategy5-latest", strategy5Latest, compactQuery(70), 12000],
    ["/api/latest-signals?strategy=strategy4", latestSignals, { strategy: "strategy4", compact: "1", shell: "1", limit: "70" }, 12000],
    ["/api/realtime-radar-latest", realtimeRadarLatest, { compact: "1", shell: "1", limit: "50" }, 12000],
    ...
    ["/api/institution-latest", institutionLatest, compactQuery(60), 12000],
    ["/api/cb-detect-latest", cbDetectLatest, compactQuery(60), 12000],
    ["/api/warrant-flow-latest", warrantFlowLatest, compactQuery(60), 22000],
  ];
}
...
async function buildAndWriteDesktopRouteSnapshot(request = {}, options = {}) {
  ...
  const watchlistPayload = payload.endpoints?.[WATCHLIST_MATCH_INDEX_ENDPOINT];
  const watchlistWrite = await upsertSnapshot("watchlist_match_index", watchlistPayload, {
