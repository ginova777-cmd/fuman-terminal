...
const { readDesktopRouteSnapshot, writeDesktopRouteSnapshot, writeDesktopRouteSnapshotForRoute } = require("./desktop-route-snapshot-cache");
...
const ROUTE_ENDPOINT_PATHS = Object.freeze({
...
function buildRouteSnapshotPayload(route, payload = {}) {
  const endpointPath = ROUTE_ENDPOINT_PATHS[route] || "";
  if (!endpointPath) return null;
  const endpoints = Object.fromEntries(Object.entries(payload.endpoints || {}).filter(([endpoint]) => String(endpoint || "").startsWith(endpointPath)));
  if (!Object.keys(endpoints).length) return null;
  const summary = Object.fromEntries(Object.entries(payload.summary || {}).filter(([endpoint]) => Object.prototype.hasOwnProperty.call(endpoints, endpoint)));
  const timings = Object.fromEntries(Object.entries(payload.timings || {}).filter(([endpoint]) => Object.prototype.hasOwnProperty.call(endpoints, endpoint)));
  const misses = (payload.misses || []).filter((endpoint) => String(endpoint || "").startsWith(endpointPath));
  const previousFilled = (payload.previousFilled || []).filter((endpoint) => String(endpoint || "").startsWith(endpointPath));
  return JSON.parse(JSON.stringify({
    ...payload,
    cacheSource: "desktop-route-snapshot-route-build",
    route,
    routeSnapshot: true,
    endpoints,
    summary,
    ...
    misses,
    previousFilled,
  }, (key, value) => key === "topBranches" ? undefined : value));
}
...
function desktopRouteTasks() {
  return [
    ...
    ["/api/strategy2-latest", strategy2Latest, { ...liveCompactQuery(80), today: "1" }, 12000],
    ["/api/strategy3-latest", strategy3Latest, liveCompactQuery(60), 30000],
    ["/api/strategy4-latest", strategy4Latest, liveCompactQuery(70), 12000],
    ["/api/strategy5-latest", strategy5Latest, liveCompactQuery(140), 12000],
    ...
    ["/api/institution-latest", institutionLatest, liveCompactQuery(60), 30000],  ];
}

function callJson(label, handler, request, query = {}, timeoutMs = 5500) {
  return new Promise((resolve) => {
    const startedAt = Date.now();
    const minTimeoutMs = Number(process.env.FUMAN_DESKTOP_ROUTE_SNAPSHOT_ENDPOINT_TIMEOUT_MS || process.env.DESKTOP_ROUTE_SNAPSHOT_ENDPOINT_TIMEOUT_MS || 0);
    const effectiveTimeoutMs = Math.max(Number(timeoutMs) || 5500, Number.isFinite(minTimeoutMs) ? minTimeoutMs : 0);
...
function buildSoftSnapshotFallback(endpoint, result, via) {
  ...
  return {
    ...
    transport: { ...(original.transport || {}), source: "fast-bundle", gate: "snapshot-soft-fallback", endpoint, originalStatusCode: result?.statusCode || 0, via, fetchedAt: new Date().toISOString() },
  };
}
...
function isMiss(item, endpoints = {}) {
  ...
  if (isSoftSnapshotEndpoint(item.label)) {
    return !endpoints[item.label] && (Number(item.statusCode || 0) >= 400
...
async function buildWatchlistSourceEndpoints(request, results) {
  const sourceEndpoints = {};
  for (const [endpoint, result] of Object.entries(results || {})) {
    if (!endpoint.startsWith("/api/strategy2-latest")) continue;
    if (Number(result?.statusCode || 0) >= 500) continue;
    if (!result?.payload || typeof result.payload !== "object") continue;
    sourceEndpoints.strategy2 = result.payload;
  }
...
async function buildAndWriteDesktopRouteSnapshot(request = {}, options = {}) {
  ...
  const routeWrites = {};
  for (const route of Object.keys(ROUTE_ENDPOINT_PATHS)) {
    const routePayload = buildRouteSnapshotPayload(route, payload);
    routeWrites[route] = routePayload
      ? await writeDesktopRouteSnapshotForRoute(route, routePayload, {
        reason: options.reason || "desktop-route-precomputed-bundle",
        timeoutMs: options.timeoutMs || SNAPSHOT_WRITE_TIMEOUT_MS,
      })
      : { ok: false, skipped: true, reason: "route_endpoint_missing" };
  }
  if (watchlistPayload) {
...
module.exports = {
  ...
  buildRouteSnapshotPayload,
  desktopRouteTasks,
  ROUTE_ENDPOINT_PATHS,
