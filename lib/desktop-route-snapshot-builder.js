...
function publicEndpointMap(results) {
  ...
  for (const [endpoint, result] of Object.entries(results)) {
    ...
    if (!isSoftSnapshotEndpoint(endpoint) && result.payload?.cacheSource === "snapshot-friendly-empty") continue;
...
async function buildDesktopRouteSnapshot(request = {}) {
  ...
  const concurrency = request?.query?.concurrency
    || process.env.DESKTOP_ROUTE_SNAPSHOT_CONCURRENCY
    || process.env.FUMAN_DESKTOP_ROUTE_SNAPSHOT_CONCURRENCY
    || 2;
...
async function buildAndWriteDesktopRouteSnapshot(request = {}, options = {}) {
  ...
  const write = await writeDesktopRouteSnapshot(payload, {
    ...
    writeEndpointSnapshots: options.writeEndpointSnapshots,
  });
