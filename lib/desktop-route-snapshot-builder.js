...
function buildSoftSnapshotFallback(endpoint, result, via) {
  ...
  const source = isWarrant ? "supabase:warrant_flow_scan_results" : isStrategy2 ? "supabase:strategy2_scan_results" : "supabase:cb_detect_cache";
...
async function buildWatchlistSourceEndpoints(endpoints, request) {
  ...
  if (!findEndpointPayload(sourceEndpoints, "/api/strategy2-latest").payload) {
    const strategy2Result = await callJson(
    ...
    if (Number(strategy2Result.statusCode || 0) < 400 && strategy2Result.payload?.ok !== false && strategy2Result.payload?.cacheSource !== "snapshot-friendly-empty") {
      sourceEndpoints[buildEndpoint("/api/strategy2-latest", liveCompactQuery(80))] = strategy2Result.payload;
    }
  }

  const { endpoint, payload } = findEndpointPayload(sourceEndpoints, "/api/warrant-flow-latest");
  if (!needsFullWarrantCoverage(payload)) return sourceEndpoints;
