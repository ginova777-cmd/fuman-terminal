...
async function readCanonicalDaytradeWater(options = {}) {
  ...
  try {
    ...
    const poolRows = await readAllRows(key, MOTHER_POOL_VIEW, {
      select: "trade_date,symbol,name,market,mother_pool_rank,priority_reason,pool_source,pool_layer,entry_score,upgrade_score,source_flags,source_run_ids,priority_reasons,source_updated_at,source_freshness,price,open_price,previous_close,change_percent,total_volume,trade_value,quote_seen_at,quote_age_seconds,last_trade_time,last_trade_age_seconds,latest_candle_time,intraday_1m_stale_seconds,inside_volume,outside_volume,side_volume_total,side_volume_unit,side_volume_threshold_lots,side_volume_threshold_met,side_volume_ge_2000_lots,side_volume_source,side_volume_source_event_at,side_volume_trade_date,side_volume_canonical_run_id,side_volume_definition,side_volume_includes_odd_lot,side_volume_includes_opening_auction_first_trade,side_volume_includes_unclassified_trades,side_volume_difference_from_total,outside_inside_ratio,side_volume_available,outside_volume_ge_inside_times_2,outside_volume_gt_inside_times_2,ma5,ma10,ma20,ma5_ma10_ma20_bullish,contract_version,canonical_run_id,updated_at",
