...
const {
  ...
  normalizeTimestamp: normalizeSideVolumeTimestamp,
  taipeiDateFrom,
...
function quoteTradeDate(row = {}) {
  ...
  const explicitTradeDate = String(
    row?.trade_date
  ...
  const sourceTimestamp = payload?.sideVolumeSourceEventAt
    || payload?.side_volume_source_event_at
    || payload?.total?.time
    || raw?.total?.time
    || quoteTimestamp(row);
  const normalizedSourceTimestamp = normalizeSideVolumeTimestamp(sourceTimestamp);
  return normalizedSourceTimestamp ? taipeiDateFrom(normalizedSourceTimestamp) : "";
}
