...
const SIDE_VOLUME_RECEIPT_VIEW = "v_fugle_daytrade_side_volume_verification_readback";
const SIDE_VOLUME_SYMBOL_VIEW = "v_fugle_daytrade_side_volume_symbol_readback";
...
async function readCanonicalDaytradeWater(options = {}) {
  ...
  const snapshotEvidence = telegramObservation ? require("./daytrade-mother-pool-snapshot").readMotherPoolSnapshot(tradeDate) : null;
  const eventReadinessHardGate = options.eventReadinessHardGate !== false;
  const requestedEventSymbols = [...new Set((options.symbols || []).map(normalizeSymbol).filter(Boolean))];
  const failures = snapshotEvidence && !snapshotEvidence.ok ? [...snapshotEvidence.failedChecks] : [];
  const key = anonKey();
  const baseReceipt = {
    ...
    ...(snapshotEvidence ? require("./daytrade-mother-pool-snapshot").snapshotIdentity(snapshotEvidence) : {}),
    ...
    reader_mode: telegramObservation ? "telegram_observation_per_symbol_fail_closed" : "formal_gate_fail_closed",
    ...
    sources: {
      ...
      side_volume_receipt: SIDE_VOLUME_RECEIPT_VIEW,
      side_volume_symbols: SIDE_VOLUME_SYMBOL_VIEW,
    },
  };
  ...
  try {
    ...
    if (!telegramObservation && String(sourceRow?.status || "").toLowerCase() !== "ok") failures.push("canonical_water_source_status_not_ok");
    ...
    const poolRows = await readAllRows(key, MOTHER_POOL_VIEW, {
      select: "trade_date,symbol,name,market,mother_pool_rank,priority_reason,pool_source,pool_layer,entry_score,upgrade_score,source_flags,source_run_ids,priority_reasons,source_updated_at,source_freshness,price,open_price,previous_close,change_percent,total_volume,trade_value,quote_seen_at,quote_age_seconds,last_trade_time,last_trade_age_seconds,latest_candle_time,intraday_1m_stale_seconds,ma5,ma10,ma20,ma5_ma10_ma20_bullish,contract_version,canonical_run_id,updated_at",
    ...
    const normalizedPoolRows = poolRows.map(normalizeMotherPoolRow).filter(row => !snapshotEvidence || snapshotEvidence.symbols.has(row.symbol));
    if (snapshotEvidence) {
      for (const symbol of snapshotEvidence.symbols) if (!normalizedPoolRows.some(row=>row.symbol===symbol)) failures.push("mother_pool_snapshot_symbol_missing_in_source");
      for (const row of normalizedPoolRows) Object.assign(row, snapshotEvidence.membership.get(row.symbol) || {});
    }
    // Side-volume verification is an independent radar branch.  Consume the
    // latest immutable verifier batch and overlay only rows that passed its
    // per-symbol quality contract; a missing branch must not block the general
    // 1m/5m radar.
    let sideVolumeReadback = { status: "unavailable", verification_run_id: null, read_rows: 0, applied_rows: 0, reason: "no_current_verification_batch" };
    try {
      const sideReceipts = await readRows(key, SIDE_VOLUME_RECEIPT_VIEW, {
        select: "verification_run_id,trade_date,canonical_run_id,status,complete,verified_at,source_common_valid,first_blocker",
        trade_date: `eq.${tradeDate}`,
        canonical_run_id: `eq.${sideVolumeCanonicalRunId(tradeDate)}`,
        source_common_valid: "eq.true",
        status: "in.(complete,partial)",
        order: "verified_at.desc",
        limit: 1,
      }, { timeout: 15000 });
      const sideReceipt = sideReceipts[0] || null;
      if (sideReceipt?.verification_run_id) {
        const sideRows = await readAllRows(key, SIDE_VOLUME_SYMBOL_VIEW, {
          select: "verification_run_id,trade_date,canonical_run_id,symbol,name,in_mother_pool,source_common_valid,quality_ok,quality_status,inside_volume,outside_volume,side_volume_total,side_volume_unit,side_volume_available,side_volume_threshold_lots,side_volume_ge_2000_lots,side_volume_source,side_volume_source_event_at,side_volume_trade_date,side_volume_canonical_run_id,total_matches_inside_plus_outside,source_fresh_120s_at_verification",
          verification_run_id: `eq.${sideReceipt.verification_run_id}`,
          order: "symbol.asc",
        }, { pageSize: 500, maxRows: 5000, timeout: 15000 });
        let appliedRows = 0;
        const verifiedBySymbol = new Map(sideRows.map((row) => [normalizeSymbol(row?.symbol), row]));
        const verifiedSymbols = normalizedPoolRows
          .map((row) => row.symbol)
          .filter((symbol) => {
            const verified = verifiedBySymbol.get(symbol);
            return verified?.in_mother_pool === true && verified?.source_common_valid === true && verified?.quality_ok === true;
          });
        const liveSideQuotes = [];
        for (const group of chunks(verifiedSymbols, 200)) {
          liveSideQuotes.push(...await readRows(key, QUOTE_TABLE, {
            select: "symbol,trade_date,name,quote_seen_at,last_trade_time,updated_at,total_volume,cumulative_bid_volume,cumulative_ask_volume,cumulative_bid_ask_volume,payload",
            trade_date: `eq.${tradeDate}`,
            symbol: `in.(${group.join(",")})`,
            limit: group.length,
          }, { timeout: 15000 }));
        }
        const liveSideQuoteBySymbol = new Map(liveSideQuotes.map((row) => [normalizeSymbol(row?.symbol), row]));
        for (const row of normalizedPoolRows) {
          const verified = verifiedBySymbol.get(row.symbol);
          const liveQuote = liveSideQuoteBySymbol.get(row.symbol);
          if (!verified || verified.in_mother_pool !== true || verified.source_common_valid !== true || verified.quality_ok !== true || !liveQuote) continue;
          const live = deriveDaytradeSideVolumeContract({ quote: liveQuote, expectedTradeDate: tradeDate, now: new Date(nowMs) });
          const inside = numberValue(live.insideVolume, null);
          const outside = numberValue(live.outsideVolume, null);
          const sourceFresh = live.sideVolumeSourceEventAgeSeconds !== null && live.sideVolumeSourceEventAgeSeconds <= 120;
          Object.assign(row, {
            inside_volume: inside,
            outside_volume: outside,
            side_volume_total: numberValue(live.sideVolumeTotal, null),
            side_volume_unit: String(live.sideVolumeUnit || "").toLowerCase(),
            side_volume_threshold_lots: numberValue(live.sideVolumeThresholdLots, SIDE_VOLUME_THRESHOLD_LOTS),
            side_volume_threshold_met: live.sideVolumeGe2000Lots === true,
            side_volume_ge_2000_lots: live.sideVolumeGe2000Lots === true,
            side_volume_source: String(live.sideVolumeSource || ""),
            side_volume_source_event_at: String(live.sideVolumeSourceEventAt || ""),
            side_volume_trade_date: String(live.sideVolumeTradeDate || ""),
            side_volume_canonical_run_id: String(live.sideVolumeCanonicalRunId || ""),
            side_volume_available: live.sideVolumeAvailable === true && sourceFresh,
            outside_inside_ratio: inside > 0 && outside !== null ? outside / inside : (outside > 0 ? Infinity : null),
          ...
          if (live.sideVolumeAvailable === true && sourceFresh) appliedRows += 1;
        }
        sideVolumeReadback = { status: "ready", mode: "latest_live_quote_revalidated_against_immutable_verifier_batch", verification_run_id: String(sideReceipt.verification_run_id), verified_at: String(sideReceipt.verified_at || ""), receipt_status: String(sideReceipt.status || ""), read_rows: sideRows.length, live_quote_rows: liveSideQuotes.length, applied_rows: appliedRows, reason: appliedRows ? null : "no_quality_ready_current_rows" };
      }
    } catch (error) {
      sideVolumeReadback = { status: "data_gap", verification_run_id: null, read_rows: 0, applied_rows: 0, reason: failureFromError(error), detail: String(error?.message || error) };
    }
    ...
    const hydrationSymbols = telegramObservation ? requestedEventSymbols : poolSymbols;
    ...
    // Full-pool synchronization is the general-radar branch health signal.
    // It must not suppress independently verified side-volume events; every
    // actual general event is still revalidated per symbol below.
    const generalRadarPoolSyncReady = marketEventSyncCoverage >= TELEGRAM_POOL_FRESH_COVERAGE_MIN;
    if (!telegramObservation && quoteCoverage < 0.95) failures.push("canonical_water_mother_pool_quote_coverage_below_095");
    if ((!telegramObservation || requestedEventSymbols.length > 0) && !intradayStatusRows.length) failures.push("canonical_water_intraday_1m_status_empty");
    ...
    const receipt = {
      ...
      side_volume_readback: sideVolumeReadback,
      ...
      telegram_branch_readiness: telegramObservation ? {
        general_radar: {
          ready: generalRadarPoolSyncReady,
          status: generalRadarPoolSyncReady ? "ready" : "data_gap",
          reason: generalRadarPoolSyncReady ? null : "canonical_water_telegram_market_event_sync_coverage_below_090",
        },
        side_volume_radar: {
          ready: sideVolumeReadback.applied_rows > 0,
          status: sideVolumeReadback.applied_rows > 0 ? "ready" : "data_gap",
          reason: sideVolumeReadback.applied_rows > 0 ? null : sideVolumeReadback.reason,
        },
      } : null,
...
// Strategy3 snapshot rules are isolated from shared live observation gates.
const sharedReadCanonicalDaytradeWater = module.exports.readCanonicalDaytradeWater;
module.exports.readCanonicalDaytradeWater = function(options = {}) {
  if (options.strategy3Consumer === true || options.consumerName === "strategy3_v2") {
    return require("./strategy3-canonical-water-reader").readCanonicalDaytradeWater(options);
  }
  return sharedReadCanonicalDaytradeWater(options);
};
