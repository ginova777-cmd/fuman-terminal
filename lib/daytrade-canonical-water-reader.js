  ...
  try {
    ...
    for (const group of chunks(poolSymbols, 200)) {
      ...
      quoteRows.push(...await readRows(key, QUOTE_TABLE, {
        select: "symbol,name,market,price,open_price,previous_close,high_price,low_price,change_percent,total_volume,trade_value,quote_seen_at,updated_at,trade_date",
        symbol: filter,
        trade_date: `eq.${tradeDate}`,
        limit: group.length,
      }, { timeout: 15000 }));
    ...
    const quoteBySymbol = new Map(quoteRows.map((row) => [normalizeSymbol(row?.symbol), row]).filter(([symbol]) => symbol));
    ...
    const freshQuoteSymbols = poolSymbols.filter((symbol) => {
      ...
      const seenAt = quote?.quote_seen_at || quote?.updated_at;
    ...
    for (const symbol of requestedEventSymbols) {
      ...
      const quoteAge = ageSeconds(quote?.quote_seen_at || quote?.updated_at, nowMs);
    ...
    const latestQuoteTime = quoteRows.map((row) => String(row?.quote_seen_at || row?.updated_at || "")).sort().pop() || "";
