...
function quoteTradeDate(row = {}) {
  const payload = row?.payload && typeof row.payload === "object" ? row.payload : {};
  const explicitTradeDate = String(row?.trade_date || payload?.trade_date || payload?.tradeDate || "").slice(0, 10);
  if (/^\d{4}-\d{2}-\d{2}$/.test(explicitTradeDate)) return explicitTradeDate;
  const timestamp = quoteTimestamp(row);
  return timestamp ? taipeiDate(new Date(timestamp)) : "";
}
  ...
  try {
    ...
    for (const group of chunks(poolSymbols, 200)) {
      ...
      quoteRows.push(...await readRows(key, QUOTE_TABLE, {
        select: "symbol,name,market,price,open_price,previous_close,high_price,low_price,change_percent,total_volume,trade_value,quote_seen_at,last_trade_time,updated_at,source,source_name,is_realtime,is_fallback,is_formal_entry_eligible,payload",
        symbol: filter,
        limit: group.length,
      }, { timeout: 15000 }));
